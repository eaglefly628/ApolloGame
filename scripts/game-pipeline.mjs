...
export const REVIEW_CHECKLISTS = {
  ...
  S4: ['走查测试断言的是行为而非常量（假信心自查：故意改坏被测逻辑应变红）', '核心循环闭环：开局→行动→反馈→终局→可重开', '失败路径有测试（非法输入被拒/终局判定不误报）', '确定性：同 seed 同结果有断言', '验收剧本作者=GD 非 PE（git blame docs/design/<game>/acceptance/*.scenario.jsonc 抽查·PE 自写剧本=FAIL·REQ-ACCEPT 循环律）', '附真浏览器试玩截图序列（开局→N 步→终局→重开·非仅 CLI 绿）', '自证对齐单抽样重走 ≥3 条（含 ⚠降格行的裁决去向核对）+ 好玩三问已作答非敷衍（docs/playbooks/self-check.md）', 'UI 货架**选型**逛过（house 主题 / @ui/starters / 按品类挑成熟件）——选型定信息层级属结构归 S4，观感精修归 S5（owner 2026-08-07·两层 1:1 律）'],
