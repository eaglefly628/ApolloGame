...
function teachRow(cols: number, rows: number, r: number, cs: number[], ch: string): string[] {
  return layer(cols, rows, (c, rr) => (rr === r && cs.includes(c) ? ch : '.'));
}
...
for (let n = 1; n <= 6; n++) {
  const board = n === 3 ? overlay(blank(7, 8), teachRow(7, 8, 4, [1, 2, 4], '0')) : undefined;
...
for (let k = 0; k < 6; k++) {
  ...
  if (no === 7) { board = layer(7, 8, (c, r) => ((r === 3 && (c === 2 || c === 4)) || (c === 3 && (r === 2 || r === 4)) ? '1' : '.')); note = '教学：L/T 交叉生成朱印珠'; }
  if (no === 12) { board = overlay(blank(7, 9), teachRow(7, 9, 4, [1, 2, 3, 5], '0')); note = '教学：5 连生成太极丸 + 鹤师父考验'; }
  I.push({ no, chapter: 1, type: 'collect', cols: 7, rows, kinds, color: no % kinds, d0: 24, board, note });
}
// 蛇 13-18 jelly
...
function medianStepsAtNominal(it: Intent): number {
  const lv = materialize(it, HIGH_MOVES, it.d0);
  const steps: number[] = [];
  for (let i = 0; i < CAL_SEEDS; i++) steps.push(playLevel(lv, it.seed !== undefined ? SEED_OF(it.no) + i : i, HIGH_MOVES).stepsToWin);
  steps.sort((a, b) => a - b);
  return steps[Math.floor(steps.length / 2)] || HIGH_MOVES;
}
...
function stars(it: Intent, moves: number, d: number): [number, number, number] {
  ...
  let s1 = it.type === 'score' && scoreGoal ? scoreGoal.n! : rnd(pc(0.05));
...
function main(): void {
  ...
  for (const it of I) {
    ...
    const moves = Math.min(40, Math.max(15, Math.round(medianStepsAtNominal(it) * margin)));
