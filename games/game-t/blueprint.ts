...
//   输入       = clickable 点选两格交换 + t2-match3-drag-swap 拖拽滑动手势（已验收·产同形信号·sim 不知输入形态）
...
import {
  ...
  INK_TINTS, INK_LABELS, INK_SKIN_KEYS, TINT_JELLY, TINT_PORCELAIN, TINT_STONE, TINT_EMPTY, cellCenter,
} from './theme.js';
...
export function buildLevelBlueprint(spec: LevelSpec): WorldBlueprint {
  ...
  entities.board = {
    MatchBoard: {
      ...
      selectAction: 'pick',
      movesResource: 'moves',
      kindSkinEntities: inkIds.map((_, k) => `skin-ink${k}`), // 皮肤定义实体（S6 真图落 index 即全盘换装）
  ...
  // ── 皮肤定义实体（art-pipeline 三行接入②：一种一实体持 Sprite key·view-sync 写到 BoardCell）──
  //   Color=该品墨珠的身份色：让台账推导按 tint 区分六品各成一行（也是生成 prompt 的色锚数据）。
  for (let k = 0; k < spec.kinds; k++) {
    entities[`skin-ink${k}`] = {
      Sprite: { textureKey: INK_SKIN_KEYS[k] ?? '', anchorX: 0.5, anchorY: 0.5, zOrder: 0 },
      Color: { tint: INK_TINTS[k] ?? 0, alpha: 1 },
    };
  }
  ...
  for (let i = 0; i < L.cells.length; i++) {
    ...
    entities[`cell-${i}`] = {
      ...
      // 皮肤槽（与 Shape 并存·贴图未就绪回退色块=观感零变）；view-sync 按 kindSkinEntities 换 key
      Sprite: { textureKey: '', anchorX: 0.5, anchorY: 0.5, zOrder: 0 },
      Color: { tint, alpha: 1 },
  ...
  return {
    capabilities: [
      transformCapability, shapeCapability, colorCapability,
      resourceCapability, flagCapability, randomCapability,
      clickableCapability, match3DragSwapCapability,
