...
function hudTree(fps: number, stats: RenderStats | null, showProfiler: boolean, picked?: string | null): LayoutNode {
  const children: LayoutNode[] = [
    ...
    { type: 'Label', id: 'gz-show', props: { text: '🌉 五台分布：A=追逐场 ｜ Two=手感·物理 ｜ Three=材质·渲染 ｜ Four=物理沙盘 ｜ Five=批渲·实例化', size: 'sm', color: 'jade' } },
    { type: 'Label', id: 'gz-show2', props: { text: '🎮 相机=C 软跟随 ｜ 点任意物件=B 震屏+E 闪白 ｜ 拖拽旋转 · 滚轮缩放', size: 'sm', color: 'gold' } },
    { type: 'Label', id: 'gz-show3', props: { text: '🚀 调试面板「传送」按钮 → Camera3D.tween 循环飞越 A→Two→Three→Four→Five→A 看各台特性', size: 'sm', color: 'jade' } },
...
export function mount(container: HTMLElement): () => void {
  ...
  //   A=追逐场(follow 跟狐狸)、Two=手感/物理、Three=材质/渲染、Four=物理沙盘、Five=批渲/实例化（皆 orbit 定机位·从 diorama 导出）。
  const CYCLE = ['A', 'two', 'three', 'four', 'five'] as const;
  const PLAT_CAM = { two: PLATFORM_TWO_CAM, three: PLATFORM_THREE_CAM, four: PLATFORM_FOUR_CAM, five: PLATFORM_FIVE_CAM } as const;
  const PLAT_LABEL = { A: 'Platform A（追逐场）', two: 'Platform Two（手感·物理）', three: 'Platform Three（材质·渲染）', four: 'Platform Four（物理沙盘）', five: 'Platform Five（批渲·实例化）' } as const;
  let platIdx = 0;
  let tweenN = 0;
  const teleport = (): void => {
    const c = cam(); if (!c) return;
