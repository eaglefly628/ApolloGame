// game-g 平台触点（Steam / 假 Steam · sim 外）。把战役结果接到平台端口：胜利解成就、传战役进度
// 排行榜、更新富状态；并把本游戏自有 localStorage 存档 blob 镜像上云（game-g 用自家 Save 而非
// 引擎 SaveSystem 快照，故走云桥直写文件）。全部经工厂选实现：原生壳真 Steam / 开关下假 Steam /
// 否则 Null 静默 —— 游戏零分支、不可用绝不影响玩法。
import { createPlatformPort, type PlatformPort } from '@services/platform/index.js';
import { resolveCloudBridge } from '@services/storage/index.js';
...
const CLOUD_FILE = 'gameG-save-v1.json';
