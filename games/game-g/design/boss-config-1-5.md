...
- **✅✅ 核心大改后重标（design G 2026-07-04·三行为自由混 + Boss自由混 + 机关门退役 全落地后跑 `simulate-balance.ts`）**：
  - **对称核变后关1 稳在目标**：终极 Player-AI（前向推演 skill5·标定基准）· bossDelta=0 →
    - **单场 Boss(列奥尼达) 胜率 95.5%**（对齐旧 expert 96%）
    - **5战全胜通关率 73.5%**（对齐旧 74.7%）→ **≈ 70% 目标·无需 bossDelta 加减**。
  - **🎯 结论：三大对称改（动作模型/Boss自由混/退役机关门）没把公平点推离 70%**——正是公平铁律要的（双方同规则 → 难度只来自明牌 kit·核变不偏心）。**关1 @ bossDelta=0 = 定稿候选**（待下方 loader 接入后终扫）。
  - **裸镜像 98%**（skill5·双方同牌同规则）：公平底层成立·玩家 AI 够强（skill1<3<5 单调 81→93→98）。
  - **⚠ 三点仍需 owner/程序A 拍**：
    1. **贪心地板 vs 专家顶**：同 bossDelta=0，**贪心(真新手)通关仅 24%**（单场 78%·但 5战全胜 0.78⁵≈0.29 复利砸下来）↔ 专家 73.5%。**关1="5战全胜"的复利** = 新手体验硬。**设计杠杆**（owner 定）：关1 是否该 5 战？还是目标改按**单场 Boss 胜率**量（专家 95.5%/贪心 78%·更符教学关手感）？
    2. **天罡 loadout 在 stage-sim 消融 +0%**（裸装 23.2% = +天罡 loadout 23.2%·完全无效）——stage-favorBias 路径**没吃天罡 buff**。疑 stage-sim 临时模型不建模天罡（或 bug）→ 请程序A 核 `runStageSim` 是否漏应用 tengang loadout。（battle-sim 路径吃天罡·故单场数可信。）
    3. **仍是临时 favorBias 模型**（同上 b）：Boss 16写死牌组 + dishaScale 未接 loader（`REQ-G-Boss写死明牌天罡`/doc27 §六·程序A）→ **接入后 design G 终扫定稿**。现 73.5% = 高可信"接近定稿"·非终值。
