...
describe('overlays pilot · 数据驱动浮层', () => {
  ...
  it('buildOverlay 按 open 返回对应浮层（none→null·settings→Modal·shop→居中 Modal）', () => {
    ...
    expect(shop?.type).toBe('Modal'); // 照抄原版居中弹窗（非右侧抽屉）
  ...
  it('mountOverlays 双宿主：点「设置」→ Modal 现·点完成→消失；点「商城」→ 居中 Modal', () => {
    ...
    expect(host.textContent).toContain('天罡卡池');           // 商城 Modal 出现
