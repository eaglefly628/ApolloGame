    ...
    children: [
      { type: 'Label', id: 'q-title', props: { text: '◈ NEON SIEGE', font: 'scifi', size: 'xl', color: 'jade', glow: true } },
      {
        ...
        children: [
          ...
          stat('q-threat', '☣', `${s.remaining} left`, s.remaining > 0 ? 'warn' : 'dim'),
        ],
      },
    ],
  };
...
function buildButton(key: 'pulse' | 'cannon', s: HudState): LayoutNode {
  ...
  // 有任何 pending → 两钮都禁（防重复买扣金）；否则按可负担。
  const disabled = s.pending !== null || !affordable;
  return {
    type: 'Button',
    id: `q-buy-${key}`,
    props: { label: `${def.name}  ⬡${def.cost}`, kind: 'primary', disabled, action: `buy_${key}` },
...
export function buildBottomBar(s: HudState): LayoutNode {
  const hint = s.pending
    ? `▶ Tap a glowing pad to deploy ${TOWERS[s.pending].name}`
    : 'Buy a tower, then tap a build pad by the lane';
  return {
    type: 'Panel', id: 'q-bottombar', props: { glass: true },
    layout: { direction: 'row', align: 'center', justify: 'center', gap: 14, padding: 12 },
    children: [
      buildButton('pulse', s),
      buildButton('cannon', s),
      { type: 'Label', id: 'q-hint', props: { text: hint, size: 'sm', color: s.pending ? 'jade' : 'sub' } },
...
export function buildOverlay(s: HudState): LayoutNode {
  ...
  return {
    ...
    children: [
      { type: 'Label', id: 'q-over-title', props: { text: win ? 'VICTORY' : 'BASE LOST', font: 'scifi', size: 'xxxl', bold: true, color: win ? 'ok' : 'danger', glow: true } },
      { type: 'Label', id: 'q-over-sub', props: { text: win ? 'All waves repelled. The grid holds.' : 'The swarm overran your core.', size: 'md', color: 'sub' } },
