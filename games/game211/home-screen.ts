...
// 红线（同 game-i）：本文件**只产数据 + 接信号**，不写一行 HTML/DOM 模板。证明 game211 UI 可落「纯数据 + 引擎固定解释器」。
//
...
export function buildHomeScreen(view: LobbyView): LayoutNode {
  ...
  const duel: LayoutNode = {
    ...
    children: [
      ...
      { type: 'CoinFlip', id: 'duel-roll', props: {
        ...
        ...(textureOverrideUri('game211/tex/coin-heads') ? { headsArt: textureOverrideUri('game211/tex/coin-heads')! } : {}),
        ...(textureOverrideUri('game211/tex/coin-tails') ? { tailsArt: textureOverrideUri('game211/tex/coin-tails')! } : {}),
      }, layout: { anim: 'glow', sheen: true } },
      // 红牌背 backPattern:'checker'（原版 .dback i 棋盘格条纹质感）；牌背贴图槽（台账 game211/tex/card-back·
      // 批29 引擎 backArt prop 落地）：真图=整面 cover 替代纹样·无=原棋盘格（观感零变）。
      tiltFloat('duel-back', {
        rank: 'A', suit: '♠', face: 'light', faceUp: false, back: '❖', backPattern: 'checker',
        ...(textureOverrideUri('game211/tex/card-back') ? { backArt: textureOverrideUri('game211/tex/card-back')! } : {}),
      }, 9),
    ],
  };
  ...
  //   台账槽 `game211/tex/home-backdrop` 放了真图 → 这层直接吃**整图 cover**当自己的贴图（容器自己的贴图·route B），
  //   透明色本就支持——透明/半透明底图连同上面 bare 子面板一起透出（此前放图放在 Screen 上被这层绿呢盖住·看不到=owner 撞的 bug）。
  //   无真图 → 回退原绿呢渐变 + 钱币纹（观感零变）。
  const homeBackdrop = textureOverrideUri('game211/tex/home-backdrop');
      ...
      { type: 'Panel', id: 'home-center', props: { bare: true }, layout: { direction: 'column', align: 'center', gap: 20, padding: 0 },
        children: [
          { type: 'Panel', id: 'home-fortune', props: {}, layout: { direction: 'row', align: 'center', gap: 8, padding: 8 },
            children: [
              // 套装图标（07-15）：icon 覆盖在场→span.img·无=原 emoji 文本（观感零变）。token=台账 game211/icon/fortune。
  ...
  return {
    ...
    props: {
      ...
      // 主页背景板槽（台账 game211/tex/home-backdrop·07-14 全面台账化）：真图=cover 整图·无=纯主题色（观感零变）
      ...(textureOverrideUri('game211/tex/home-backdrop') ? { image: textureOverrideUri('game211/tex/home-backdrop')! } : {}),
    },
