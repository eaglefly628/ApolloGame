...
export function mountCoinFlip(host: HTMLElement, opts: CoinFlipOpts, onDone: () => void): { destroy: () => void } {
  ...
  // 硬币双面贴图槽（07-14 全面台账化：game211/tex/coin-heads|tails）：真图=cover 圆面·无覆盖=CSS 渐变（观感零变）。
  const headsArt = textureOverrideUri('game211/tex/coin-heads');
  const tailsArt = textureOverrideUri('game211/tex/coin-tails');
