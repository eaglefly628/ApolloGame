...
// 旧实时核（initLiveBattle/stepLiveBattle/…）已零调用删除；这些**纯数据件**被存活的回合制核(turn-combat.ts)+UI(game211.tsx/clash-view)+仿真台复用，故单独成模块、切断对旧核的依赖。无逻辑改动，定义与注释逐字保留。
...
export interface ClashCard { id?: string; rank: string; suit: string; general: boolean; points: number; buff: number; morale: number; tengang: number; pEff: number; tgBreak?: [string, number][]; nearDef?: number; dishaEdge?: number; phalanx?: number; wins?: number; fatiguePm?: number; rollMod?: { bonus: number; floor: number; twice: number } } // fatiguePm：该兵此刻疲劳千分比（战力损失量·owner 2026-07-06 连续疲劳条·明细里显疲劳折价·替旧 0.5^wins）；rollMod：该侧掷骰系天罡改掷(bonus 鬼手改掷+N / floor 磐石掷下界 / twice 灌铅骰多掷取高)→显式进掷骰读数(owner/GD 2026-07-04·否则掷值超 [1~P] 看着像作弊)；phalanx：地煞·斯巴达方阵→每兵按自身相邻友兵数 +确定战力（owner 2026-07-03·改逻辑为真·每兵加战力·进拆解可见）；id：该兵 unitId（owner 2026-06-29·离场动画按 id 定位被撕/光荣离场的兵）；tgBreak：天罡逐张贡献 [天罡id, 加成]（owner 2026-06-21·对决明细溯源）；nearDef：地煞·隘口守军固守 +战力；dishaEdge：地煞·招牌气势折成的确定战力（owner 2026-07-01 确定制·仅 Boss 侧）；wins：累计胜场（owner 2026-07-01·仅日志/画像·不再驱动战力）
// tie：50:50 平局如何裁定（owner）—— null=正常概率掷命(战力不等) / 'points'=战力相等·点数大者胜 / 'stamina'=点数也同·续航高者胜 / 'roll'=全同·这一掷定(重揉)。
export interface ClashEvent { tick: number; lane: number; winrate: number; roll: number; rollA?: number; rollB?: number; aWins: boolean; tie: 'points' | 'stamina' | 'roll' | 'power' | null; winStays?: boolean; warLoss?: number; winStreak?: number; fatiguePm?: number; lastStand?: boolean; a: ClashCard; b: ClashCard } // rollA/rollB：各自掷战力骰的掷值(owner 2026-07-01·双方各掷 [1,自己战力] 比大小)；winrate=预报胜率；tie=掷平如何裁定(power/points/stamina/roll) // warLoss：胜者本场对折率(0.5·显「本场−N」)；winStreak：胜者累计胜场；fatiguePm：胜者累计疲劳千分比(owner 2026-07-06·连续疲劳条·显头顶总疲劳/供恢复回看) // winStays：恒 true(胜者留场·光荣回库已删·owner 2026-07-06)；lastStand：本场触发敌主将「死战不退·首负不亡」(关1 列奥尼达地煞)→全屏通知+特写改显(owner 2026-06-21)
// 已施天罡 → 玩家侧(a)持续战斗修正（A-JOKER · cast 后整局生效·一种牌算一次不叠）。
// 聚合(aggregateTengang)在 game211 读 GAME_G_TIANGANGS 算（避免 live-combat ← blueprint 环依赖）；live-combat 只持有这份扁平修正、在 clash/deploy 钩子读。
