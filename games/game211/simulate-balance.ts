...
// Run: npx vite-node games/game211/simulate-balance.ts
//
...
import { tengangFxOf, aggregateTengang } from './game211-build.js'; // 天罡聚合改用 game211-build 注册表版（删 sim 过期 if-else 复制·零漂移·Phase1 已证注册表==if-else）
import { cardPoints, P_MAX } from './clash-resolve.js';
import { loadLevel } from './level.js';

declare const process: { exit(code: number): never };

// ── Local helpers (pure functions from game211.tsx, copied to avoid browser deps) ──
...
// seededShuffle / tengangFxOf / aggregateTengang 已收敛——洗牌→atoms、天罡聚合→game211-build（删本地过期复制·见上方 import）。
// 注：game211-build.aggregateTengang 内部走 TIANGANG_BY_ID（与原 sim 同源）→ 行为一致。
