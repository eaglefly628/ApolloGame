...
// 主题级按钮皮（批29 owner 07-15「按键也可换」）：台账 game211/ui/btn-* 三行 → 引擎 UITheme.buttonSkins，
...
function ggButtonSkins(): UITheme['buttonSkins'] {
  const hero = textureOverrideUri('game211/ui/btn-hero');
  const primary = textureOverrideUri('game211/ui/btn-primary');
  const ghost = textureOverrideUri('game211/ui/btn-ghost');
...
// game211 古风 UI 主题（玄铁·暗）：暖墨底 + 羊皮纸字 + 淡金点睛。
// 喂引擎 components 的 renderNode/mountUI 即「换皮」——同一份 LayoutNode 数据、零改解释器。
// 这就是「数据驱动 UI·主题作数据」的 game211 那份令牌：最弱 LLM 也只是填颜色/字体字符串，不写 CSS/DOM。
