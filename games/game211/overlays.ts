...
const HELP_MANUAL: Record<'easy' | 'mid' | 'hard', { title: string; color: 'ok' | 'warn' | 'danger'; paras: string[] }> = {
  easy: { title: '🟢 初级 · 打赢第一场', color: 'ok', paras: [
    ...
    '回合制·每回合：回合开始 +1 召唤源泉，然后**三行为自由混（只要源泉够，随便混、不互斥）**：\n· 抽（天罡/扑克）\n· 打（部署兵 / 施天罡）\n· 换（选一张手牌弃换一张·每回合 1 次·免费）。\n源泉稀缺=天然节流。做完 → 棋盘走一格，两军碰头 → 掷命对决。',
  ...
  mid: { title: '🟡 中级 · 三牌组 + 经营', color: 'warn', paras: [
    ...
    '经营要点：召唤源泉紧（每回合 +1）→ 每个抉择都重要；同点数凑对子/三条加战力；田忌赛马——避开劣势路、把强牌送去优势路集中突破。',
  ] },
...
function shopModal(view: LobbyView, st: OverlayState): LayoutNode {
  ...
  const poolPanel = (pool: 'tiangang' | 'dizhi', title: string, sub: string): LayoutNode => ({
    ...
    children: [
      // 卡池 banner 槽（台账 game211/shop/banner-*·批30）：真图在场才插 Image 节点·无=原纯文案（观感零变）
      ...(textureOverrideUri(`game211/shop/banner-${pool}`) ? [
        { type: 'Image' as const, id: `pool-${pool}-banner`, props: { src: textureOverrideUri(`game211/shop/banner-${pool}`)!, fit: 'cover' as const, radius: 8 }, layout: { height: 96 } },
      ] : []),
  ...
  return {
    ...
    children: [{
      ...
      children: [
        ...
        // 逐幕插画槽（台账 game211/story/beat-N·批30）：真图在场才插 Image（16:9 cover）·无=原纯旁白（观感零变）
        ...(textureOverrideUri(`game211/story/beat-${i + 1}`) ? [
          { type: 'Image' as const, id: 'story-art', props: { src: textureOverrideUri(`game211/story/beat-${i + 1}`)!, fit: 'cover' as const, radius: 8 }, layout: { height: 200 } },
        ] : []),
