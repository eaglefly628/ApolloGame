...
import { GG_THEME_ONYX } from './ui-theme.js'; // game211 古风主题（数据·喂引擎 UI 解释器换皮）
...
// 载入 game211 美术库索引里**真图替换**的条目（source 非 procedural）→ 双通道覆盖（owner 07-13 步2·07-14 扩贴图·07-15 批29 扩 UI 皮）：
//   heroes: { 'sA': url }（立绘·portraits 覆盖键）· textures: { 'game211/tex/felt-brocade' | 'game211/ui/btn-hero': url }（贴图/按钮皮槽·全 id）
...
export { buildPickDeck, bossHeroCard, aggregateTengang, tengangFxOf } from './game211-build.js';
export { freshSave } from './game211-save.js';
...
export function mount(container: HTMLElement, shell?: { exit?: () => void }): () => void {
  ...
  function showBetween(nextLabel: string): void {
    ...
    const tree: LayoutNode = {
      type: 'Screen', id: 'gg-btw-screen', props: {
        center: true,
        // 三选一屏背景板槽（台账 game211/tex/between-backdrop·批30）：真图=cover 整图·无=原主题底（观感零变）
        ...(textureOverrideUri('game211/tex/between-backdrop') ? { image: textureOverrideUri('game211/tex/between-backdrop')! } : {}),
      } as ScreenProps,
      children: [{
    ...
    teardown = mountUI(host, tree, handlers, GG_THEME_ONYX); // 喂 game211 古风主题 → 同份数据渲成古风皮
  }
  ...
  function showTurnMatch(_formation: Formation, _myName: string, _interventions: Intervention[]): void {
    ...
    const actions: TurnBattleActions = {
      ...
      goBack: () => {
        ...
        }, GG_THEME_ONYX); // 喂 game211 古风主题 → 同一份 LayoutNode 数据渲成古风皮
  ...
  // 步2「没加载完就不画」（owner 07-13）：大厅无立绘 → 同步渲染（保运行器/集成测的同步契约）；
  // 立绘只在 牌组/收藏/改造坊 子屏出现（导航后才画）——在此之前后台载入美术库真图覆盖，等它们被打开时覆盖早已就绪，
  // 真图替换的立绘绝不先闪程序化版。载入失败/无索引=空覆盖=回退程序化（观感零变·帧回归绿）。
  showLobby();
  let disposed = false;
  loadArtOverrides('game211').then(({ heroes, textures }) => {
    if (disposed) return;
