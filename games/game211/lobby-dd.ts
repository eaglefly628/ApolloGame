...
// archLine 数据带 HTML 标签（game211.tsx 为原版 innerHTML 写的 <b>/<span>）→ Label 会 esc 成字面字符。显示前剥标签。
...
function topbar(view: LobbyView): LayoutNode {
  ...
  // 套装图标接线（批32）：icon 覆盖在场即换（emoji→美术图标）·无覆盖=原样。token 对台账 game211/icon/*。
...
export function buildLobby(view: LobbyView, st: LobbyDDState): LayoutNode {
  ...
  const TAB_BACKDROP: Record<string, string> = {
    campaign: 'game211/tex/campaign-backdrop', coll: 'game211/tex/collection-backdrop',
    decks: 'game211/tex/deck-backdrop', craft: 'game211/tex/craft-backdrop', home: 'game211/tex/home-backdrop',
  };
  const tabKey = TAB_BACKDROP[st.tab];
  const backdrop = (tabKey ? textureOverrideUri(tabKey) : null) ?? textureOverrideUri('game211/tex/lobby-backdrop');
  return {
...
/** 挂载全数据驱动大厅。签名同旧 mountLobby（host, LobbyHandlers）→ game211.tsx 零改接入。 */
