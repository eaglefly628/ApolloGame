...
## §0.4 大混战 Demo（owner 2026-08-10 定的新口径 · 送审）

**owner 原话**：24 张牌 = 1 个 Group；红蓝**各 5 组** → 共 10 组 **240 张**牌在场上移动、互相厮杀；
剩下活着的牌**重新组成小的 Group**，直到**只剩一色**为止。
（这条**替代**了 §0.1b 时期的「36 人 × 5 队 × 2 = 360」口径——规模从 360 降到 240、每组 36 降到 24。）

### 一、无头验证：这个 demo 成不成立（**先量再做**·`melee-campaign.stats.test.ts` 2000 场）

在写任何渲染之前先把战役跑了几千遍——因为「打不完 / 一边倒 / 两场就结束」这三种废掉 demo 的可能，
光看规则是看不出来的：

| 问 | 答 | 数据 |
|---|---|---|
| 打得完吗 | ✅ 2000 场全收敛，无僵局 | 上限 500 场对决的兜底一次没触发 |
| 会一边倒吗 | ✅ 对称 | 红胜 **961** · 蓝胜 **1000** · 平 **39**（2% 同归于尽） |
| 一场战役打几轮 | **中位 26 场对决**（p95 35 · 最多 55） | 直接决定 demo 时长，见下 |
| 单场对决多少刚体 | **≤ 48**（24v24） | 实测 40 体 = 1.54ms/步 → 48 体 ≈ 1.8ms |

**对称性不是巧合、是可推的**：一对牌各自独立 50/50（`duel-spike` 刻意不镜像旋转就是为这个），
四种落面 双正/正反/反正/双反 → 存活 2/1/1/0，**每对存活期望恒 1.0 且红蓝各 0.5**。
所以整场战役 = 一次公平随机游走，谁先被清零纯看运气。落面 50/50 本身已在 8000 张牌上坐实
（50.50%·CI [49.40%, 51.60%]），**这条链是闭合的**。

> ⏱ **时长预算（要 owner 拍一下）**：26 场对决 × 每场约 4s 落定 ≈ **105 秒**纯对决时间，
> 加上行军间隙，串行跑就是 **2~4 分钟**一局。三条路：① 靠 `duel-scheduler` 开 2~3 并发（48 体/对 → 3 对 = 144 体 ≈ 6.5ms/步，帧预算内）②压缩落定判定（够稳就早读数，不等全睡）③ 接受这个长度当「战役观战」。
> 我倾向 ①+②。

### 二、消费的引擎能力（registry 实名 · **零新 system**）

| 面 | 用什么现成的 | 实查留痕 |
|---|---|---|
| **群组运动 / flocking** | `t2-steering` 的 **`separation{radius,weight,tagMask}`** + `t3-aggro`（写 `Relation` 选目标）+ `t1-motion-apply` | **不用自造 flocking**：`steering.ts` 的群体分离是 REQ-SURVIVOR群体① 已落的能力；`games/game-103/blueprint.ts:347` 就是同款用法（`separation{radius: r*2.4, weight:1.6, tagMask:ENEMY}`） |
| **240 单位渲染** | **自动实例化**：同视觉签名 → 1 个 `InstancedMesh` | `src/renderer/three/batches.ts`（`REQ-3D-PBR-INSTANCING` 已 done）。**不用手写实例化**，也不必像旧口径那样自己去碰 `src/renderer/webgl/` |
| **对决本身** | `duel-spike.ts` 现成（`throwPlan`/`judgeDuel`/`upYOf`/`layoutFor` 全是导出的纯函数） | 24 lane × 2 = 48 体，正在实测过的档位内 |
| **并发/错峰** | `duel-scheduler.ts` 现成（`planDuelStarts`） | 17 例测试·含刚体预算护栏 |
