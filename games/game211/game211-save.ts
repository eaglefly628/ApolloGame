// Game G · 本地存档（纯数据 + 迁移）—— 从 game211.tsx 抽出的存档层：类型 Save/TiangangDeck + 读写迁移 + 出战牌组派生。
...
export const SAVE_KEY = 'game211-save-v1';
...
export function freshSave(): Save {
  return { materials: 120, diamond: 6, dizhiShards: 30, rechargeCount: 0, seenIntro: false, guideStep: 0, skipGuide: false, seen: {}, tiangangShards: 0, dizhiBag: { 子: [2, 0, 0], 丑: [1, 0, 0], 寅: [1, 0, 0], 卯: [1, 0, 0] }, inlays: {}, campaignMax: 1, stage: 1, deck: Array.from({ length: DECK_SIZE }, (_, i) => baselineFavor(i)), lastOfficers: [10, 10, 10], leverEnergy: LEVER_START, lives: RUN_LIVES, bossIdx: rollBoss(), ownedTiangangs: GAME_G_TIANGANGS.filter((t) => unlockStageOf(t.id) <= 1).map((t) => t.id), tiangangDecks: [{ id: 'deck1', name: '牌组 1', cards: [...DEFAULT_STARTER_TIANGANGS], pokerPicks: [] }, { id: 'deck2', name: '牌组 2', cards: [], pokerPicks: [] }, { id: 'deck3', name: '牌组 3', cards: [], pokerPicks: [] }, { id: 'deck4', name: '牌组 4', cards: [], pokerPicks: [] }], activeDeckId: 'deck1', tiangangs: [...DEFAULT_STARTER_TIANGANGS], planets: {}, foils: [], fortune: { date: '', rolls: 0, keptVal: null } }; // deck1 默认塞新手引导天罡（owner 2026-07-04·防抽不到天罡） // 44..62 起步；金币 120；钻石送 6（首充免密）；开局默认给 4 个天罡牌组让玩家去组（owner 2026-06-21）；pokerPicks 空=自动构筑一副；新存档播开场故事+引导
}
