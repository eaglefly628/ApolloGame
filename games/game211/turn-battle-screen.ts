...
import { powerRows } from './game211-clash-view.js'; // 战力逐行明细共享格式器（④ 掷命特写 + ⑥ 场上兵 hover 单一真相）
...
import { textureOverrideUri, iconUri } from './art-textures.js'; // 对战屏背景板槽（台账 art-57 game211/tex/battle-backdrop·批29 接线）+ 套装图标（07-15 dice/shield/mana 接线）
import type { ClashDie3DHandle } from './clash-dice-3d.js'; // 掷命 3D 战力骰（引擎 ThreeRenderer + Transform3D/Mesh3D/Vfx3D 数据驱动·非 CSS 3D·owner 2026-07-03·REQ-3D-骰盅）·**动态 import**（three 600KB 只在首次掷命时才拉·不压 game211 首屏）
...
/* 行军「浮起→移动→落下→滴答」（owner 2026-07-04「慢一点·浮起来·移动·落下·滴答」·四相分明）：原地抬到空中(16%)→空中平移到新格上方(60%)→落下略过冲(84%)→回弹落定(92→100%)。
...
@keyframes g-march {
  ...
  16%  { transform: translate(var(--dx), calc(var(--dy) - 54px)) scale(1.15);       filter: drop-shadow(0 30px 20px rgba(0,0,0,.55)); z-index: 30; }
  60%  { transform: translate(0, -54px) scale(1.15);                                filter: drop-shadow(0 30px 20px rgba(0,0,0,.5));  z-index: 30; }
  84%  { transform: translate(0, 5px) scale(1.02);                                  filter: drop-shadow(0 3px 4px rgba(0,0,0,.5));  z-index: 30; }
...
function clashNode(cv: TurnClashView): LayoutNode {
  ...
  //   clash-die-m/f = 掷值文本（驱动层 game211.tsx doClashRoll 就地哒哒哒滚·两相皆在场·勿改 id）。
    ...
    <div style="${st(body)}">
      <div style="${st(boardCol)}">
        <div style="${st(boardWrap)}">
          ...
          ${fortBase(view, true)}
          <div style="${st(lanesCol)}">${forr(view.lanes, (L, li) => renderNode(laneRowNode(L, li, hi === 'lane:' + li), GG_BATTLE_THEME))}</div>
          ${fortBase(view, false)}
        </div>
...
export function buildTurnBattleHTML(view: TurnBattleView): string {
  ...
  const backdrop = textureOverrideUri('game211/tex/battle-backdrop');
...
export function buildTurnBattleView(b: TurnBattle, opts: TurnViewOpts = {}): TurnBattleView {
  ...
  return {
    ...
    water: b.a.mana, waterMax: 10, waterB: opts.waterBDisplay ?? b.b.mana, waterGain: manaGain(b.turn),
...
export function mountTurnBattle(host: HTMLElement, getView: () => TurnBattleView, actions: TurnBattleActions = {}): { update: () => void; destroy: () => void } {
  ...
  // 宿主须有确定宽高（game211.tsx stage 已 100%×100% 占满 root）；无头(happy-dom)量到 0 → 回退 1（全尺寸·测只看 DOM）。
