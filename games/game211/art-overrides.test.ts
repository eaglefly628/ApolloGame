...
// 批28（owner 07-14「game211 全面台账化替换」）：贴图槽覆盖注册表 + 台账契约钉死。
...
describe('贴图槽覆盖注册表（art-textures）', () => {
  it('无覆盖=程序化回退（确定性·观感零变的根据）', () => {
    expect(textureOverrideUri('game211/tex/felt-brocade')).toBeNull();
    expect(feltBrocadeUri()).toBe(FELT_BROCADE);
    expect(feltBrocadeUri()).toBe(feltBrocadeUri()); // 同输入同输出
  });

  it('登记真图后消费点即换；空值不收；清空回落', () => {
    registerTextureOverrides({ 'game211/tex/felt-brocade': '/games/game211/art/gen/felt.png', 'game211/tex/x': '' });
    expect(feltBrocadeUri()).toBe('/games/game211/art/gen/felt.png');
...
describe('game211 美术台账契约（110 行·行行可替换·needs-art 行行有现况占位快照）', () => {
  const led = JSON.parse(readFileSync(join(process.cwd(), 'public/games/game211/art/art-ledger.json'), 'utf8')) as {
  ...
  it('110 行·skinKey 全带且唯一（一行一素材·fill 写回的别名依据）', () => {
    ...
    expect(keys.every((k) => typeof k === 'string' && k!.startsWith('game211/'))).toBe(true);
  ...
  it('套装图标 34 枚（批32「统一风格升级」）：统一风格锚写死 query 尾·生肖 12 全·编号 art-77~110', () => {
    const icons = led.rows.filter((r) => r.skinKey!.startsWith('game211/icon/'));
    expect(icons.length).toBe(34);
    expect(icons.every((r) => r.query.includes('unified icon set'))).toBe(true); // 成套出图不跑风格的锚
    expect(icons.filter((r) => r.skinKey!.startsWith('game211/icon/zodiac-')).length).toBe(12);
  ...
  it('UI 按钮皮三行（批29「按键也可换」）：hero/primary/ghost·主题级 buttonSkins 消费·编号顺延不动老账', () => {
    const ui = led.rows.filter((r) => r.skinKey!.startsWith('game211/ui/'));
    expect(ui.map((r) => [r.no, r.skinKey])).toEqual([
      ['art-61', 'game211/ui/btn-hero'], ['art-62', 'game211/ui/btn-primary'], ['art-63', 'game211/ui/btn-ghost'],
    ]);
  });
  ...
  it('needs-art 行行有现况占位快照（owner 07-15「54~63 没有预览占位符」）：placeholder.servedPath 存在且文件真在', () => {
    for (const r of led.rows.filter((x) => x.status === 'needs-art')) {
      expect(r.placeholder?.servedPath, r.no).toMatch(/^\/games\/game211\/art\/placeholder\/.+\.svg$/);
  ...
  it('批30 全屏面扩展：背景板 8 屏全（home/campaign/battle/lobby/collection/deck/craft/between）+ 故事 6 幕 + 卡池 banner 2', () => {
    ...
    for (const k of ['home', 'campaign', 'battle', 'lobby', 'collection', 'deck', 'craft', 'between']) {
      expect(keys.has(`game211/tex/${k}-backdrop`), k).toBe(true);
    }
    expect(led.rows.filter((r) => r.skinKey!.startsWith('game211/story/beat-')).length).toBe(6);
    expect(led.rows.filter((r) => r.skinKey!.startsWith('game211/shop/banner-')).length).toBe(2);
  });

  it('立绘行覆盖键与 portraits 一致（game211/hero/<花色字母+军衔>·消费端能对上）', () => {
    const heroKeys = led.rows.map((r) => r.skinKey!).filter((k) => k.startsWith('game211/hero/'));
    expect(heroKeys.length).toBe(52);
    for (const k of heroKeys) expect(k).toMatch(/^game211\/hero\/[shdc](A|K|Q|J|10|[2-9])$/);
  });
});
