...
- **Lead/主程**：§5 阶段路线（哪个阶段还哪笔债）+ `requests.md` REQ-F-030（本文唯一新增的引擎需求，候选两路待裁）。
...
"GameFlow": { "id": "round", "current": "prep", "states": [
  { "id": "prep",        // 备战：① 发基础收入(查表 stage/round) ② 触发利息 banded ③ 触发连胜金 banded
                          ...
                          //      ⑥ 【REQ-F-030】按阵容/关卡表展开 我方+敌方 棋子实例（满血满蓝，站到各自 HexPos）
...
| **MVP-1 ⬅ 下一步** | §3.2 run_flow + §3.3 round_flow 完整多回合循环 + §4.1 经济三件套 + §4.5 关卡表前 2 阶段 + 商店买人（§3.3 操作表前三行） | **REQ-F-030 待裁**（回合重置/实例重生，见 requests.md） |
...
| # | 差距 | 对应规范 | 备注 |
|---|---|---|---|
| P0 | **单局版 flow → 多回合 run/round 双层机** | §3.2/§3.3 | 阻塞点 = REQ-F-030（棋子每回合满状态重开）。先裁 030 再动工 |
