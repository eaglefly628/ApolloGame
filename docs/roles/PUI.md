...
- **✅ 你独占**（自由改·全绿即推）：
  ...
  - `games/game-i/**` —— **展示台**（活范例·逐能力样例台·主程原维护的展台移交 PUI）。
  ...
  - `src/ui/components/*.test.ts` + `games/game-i/*.test.ts` —— 本域测试。
...
- **🔒 域外**（只能提需求不许直改·走 `requests.md`）：
  ...
  - 其它游戏 `games/game-{d,e,f,g,q,x,z}/**` —— 各 PE/P3D；**game-z/d = P3D**。
...
4. 现状：`games/game-i`（活范例·反面教材见 `engine-llm-readiness-review §3.3` game-e 1163 行 React 屏）。
