...
- **掌机弱 GPU 黑屏**：session-handoff-2026-06-22-full.md §0（归档层已删·查 git 历史）——cartridge 烧进掌机弱 GPU webview 时，`transform:scale` 首帧烤成单合成图层→合成失败黑屏（无头测不出，Mac 正常）；修法=改 **CSS zoom**（CPU 布局缩放·不生成合成图层·消闪烁·fail-safe 只裁切不黑）。发行前真机烧版验证，别信无头绿。
