...
| 反馈 / juice（休闲爽感层） | `Particles`（组件）· `anim:fadeOut/popOut/floatUp` · `ProgressBar.shape:'ring'` · `fx:'holo'` · `Label.stroke` | **庆祝粒子** `Particles{kind:confetti/coins/stars/sparkle,count?,loop?}`=UI 层发射器(通关撒纸屑/金币雨/星光爆/环境微光·世界层对等件=`Vfx3D`·render-only·粒子位置确定式派生无裸 Math.random·给父 width/height)。**退场/飘字** anim 退场对称位:`fadeOut`/`popOut`(一次性·both 停末态·toast/弹窗/消除)、`floatUp`(循环升冒·+N 收益飘字·配 animDelay 错峰)。**环形进度** `ProgressBar.shape:'ring'`(+`size`·conic 弧+中心值·体力/日目标/冷却环)。**全息箔** `fx:'holo'`(彩虹随角度流动·收集/gacha 稀有卡·比 sheen 白斜扫炫·可与 sheen/glow 叠)。**描边字** `Label.stroke`(comic 深色粗轮廓·paint-order 保填色·卡通爆字·可叠 glow/art font)。缺 juice→提 requests 让主程加**一个 kind/字段**，绝不手写 CSS 粒子/动画。样例=game-i 「🧊 3D UI」子 tab `🎉 Juice` 段 |
