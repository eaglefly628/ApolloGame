...
| 3D 截图对拍 | `scripts/shoot-game.mjs`（P3D harness·**`PIXELQA=1` 开机器断言**：非黑占比/对比度直方图动态范围/两帧活动防冻结·纯函数 `scripts/lib/pixel-qa.mjs`·阈值草案对 game-z+game-i 标定·REQ-3D-像素断言） | 缺省=人审；`PIXELQA=1` → `PIXELQA: PASS\|FAIL` + 退出码（未进推送门·需全量并发连过 3 次再议） |
...
> **两条写法坑**（2026-08-08 各白查一轮）：① **连点别用 `await page.click()` 循环**——
> 每次点前都做可点性检查，而"点一下就消失"的键第一下之后就没了，剩下几下各等 30 秒超时、
> `.catch(()=>{})` 再静默吞掉；用 `page.evaluate(() => { for (...) el?.click(); })` 一个 JS 回合内同步连点
> （这才是真手速），断言改成**"只推进了一格"**。② **叠层遮挡会伪装成逻辑 bug**：按钮画出来了、
> `data-action` 也对、单测全绿，但被自适应高度的容器（横幅/字幕/自动换行说明——它们的下边界不写在
> 代码里）压住 → 探针点在别人身上、**世界零反应且零报错**。故**新加的可点元素首跑就留一张所在屏截图**。
> **同日又撞第二次**：同一枚键换到别处仍被同一条横幅压住——因为横幅高度**随伤害数字变化**，
> 掉 20 时不压、掉 40 时压。⇒ **真正的解不是挪位置，是挂进那个自适应容器当子节点**，
> 让布局去保证不重叠。「按当前数据量了一下不重叠」不是结论，**数据会变**。
