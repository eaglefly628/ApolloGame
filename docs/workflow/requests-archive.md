...
### REQ-PKG-位置无关与正名 · 别名→@zerocraft 包名 + 外部内容启动器 + 去 Apollo 化 · [2026-08-02] · owner 拍板（引擎正名 ZeroCraft·游戏可居仓外） → Lead 出图 → **指派：执行代理（低档·spec 写死）** · status: **✅ done（2026-08-02·执行代理·待 Lead 终审）** · 优先级: P0（第三层收官） · 类型: 架构（引擎包化）+ 品牌正名
> **⚖ Lead 图纸**：
