...
### REQ-PKG-位置无关与正名 · 别名→@zerocraft 包名 + 外部内容启动器 + 去 Apollo 化 · [2026-08-02] · owner 拍板（引擎正名 ZeroCraft·游戏可居仓外） → Lead 出图 → **指派：执行代理（低档·spec 写死）** · status: **✅ done（2026-08-02·执行代理·Lead 终审返工①已修正·待 Lead 复审）** · 优先级: P0（第三层收官） · 类型: 架构（引擎包化）+ 品牌正名
> **⚖ Lead 图纸**：
...
> **✅ done（执行代理·2026-08-02·待 Lead 终审）**：
> ① **包化**：`package.json` name→`@zerocraft/engine` + `exports`（十子路径 + `tsconfig.game-base.json` + `package.json` 自身）。`scripts/engine-aliases.mjs` 新增单一别名真相表（`ENGINE_SUBPATHS`）供 `vite.config.ts`/`vite.config.cartridge.ts`/`scripts/zerocraft.mjs` 共用——每处产**旧短别名（`@engine`等，src/ 内部未改仍要用）+ 新包名子路径**并存的 alias 表。`tsconfig.json` 同理新增十条 `@zerocraft/engine/<sub>/*` paths（旧十条不动）。games/** 全量改写：206 文件·490 处（engine 97·ui 119·runtime 53·skills 76·atom-skills 36·assembly 30·assets 22·services 20·renderer 19·net 18），迁移脚本写在 scratchpad 未入库（幂等·可重放同逻辑）。`tools/export-game.mjs` 的 `ALIASES` 与 `tools/export-targets/dokiworld.mjs` 的内联 vite 模板各加十条新前缀（旧十条留着——闭包里游戏自身文件用新前缀、被拉入的 src/** 引擎文件仍用旧前缀，两者必须同时可解）；`scripts/game-skill-audit.mjs` 的能力接入面探测正则补 `/atom-skills/` 分支（漏了会把纯用新前缀 atom-skills 的游戏误判成「零能力接入」黄旗，已用 `node scripts/game-skill-audit.mjs`全量核验无回归）。
> ② **外部内容启动器**：`scripts/zerocraft.mjs`（`run|test|build <game-dir>`）+ `tsconfig.game-base.json`。校验契约走真 Node 包解析（`createRequire` 从目标目录解 `@zerocraft/engine/package.json`，不是拼字符串猜路径）；三命令的 vite/vitest/tsc 本体与 react/react-dom/three/cannon-es 一律取**被依赖引擎 checkout 自己的 node_modules**（外部目录不必背整条工具链，只需 package.json 声明依赖 + tsconfig 继承 base）；`run`/`build` 用一次性生成的 `.zerocraft/` harness（index.html+main.tsx，命令结束/服务关闭即自清）包住游戏的 `mount(container, host?)` 入口（探测逻辑同 `tools/export-game.mjs` 的 `findEntry`，泛化到不知道游戏 id 的外部目录）。`tsconfig.game-base.json` 的 `paths` 同时列新包名子路径与旧十条短别名（原因同上——tsc 顺着新前缀把引擎 `src/**` 带进外部项目编译闭包后，那些文件互相还在用旧别名，同一份 paths 必须一并接住）；**baseUrl 由外部项目自己的 tsconfig 声明**（实测确认：TS 的 `paths` 相对解析锚定在「声明 baseUrl 的那个文件」，写死在 base 文件里反而会把 baseUrl 锚回引擎仓自己——引擎仓没有自引用的 `node_modules/@zerocraft/engine`，会全灭；`tsc --showConfig` 逐步验证定位）。**验收（仓外真跑，两次·迁移中途一次+全部改动完成后一次）**：`cp -r games/game-x` 到 `/tmp` 外部目录，补最小 `package.json`（唯一依赖 `"@zerocraft/engine": "file:/home/user/ApolloGame"`）+ `tsconfig.json`（`extends` base），`npm install` 产真 `node_modules/@zerocraft/engine` 符号链接（非网络安装·file: 协议本地链接）——`build`（`tsc --noEmit` 通过 + `vite build` 171 模块打包成功）/`test`（vitest 4 文件/56 测全绿）/`run`（起 dev server，`/main.tsx` 与游戏入口 `game-x.ts` 均经 `/@fs/` 正确解析回引擎真实 `src/**`，自证模式 fetch 首页 200）**三命令全绿**。
> ③ **去 Apollo 化**：env 变量新旧双读——Python 侧新增 `main_entry/sysutil.env(new_name, old_name=None, default=None)`（新名优先·旧名兜底，`ZEROCRAFT_*` 缺省按前缀自动推 `APOLLO_*` 旧名）单点收敛，改掉 `mock.py`/`config.py`/`server.py`/`cli.py` 里全部 `os.environ.get('APOLLO_*')` 直读；JS 侧同款 `process.env.ZEROCRAFT_X ?? process.env.APOLLO_X`（`vite.config.ts`/`scripts/{game-pipeline,acceptance-run,game-skill-audit}.mjs`）。覆盖 `ZEROCRAFT_MOCK_LLM`/`_MOCK_BAD_N`/`_MOCK_BAD_MANIFEST_N`/`LOG_VERBOSE`/`FEATURE_*`/`API_PORT`/`STATIC_DIR`/`PIPELINE_ROOT`/`ACCEPTANCE_ROOT`/`ACCEPTANCE_CLI`/`DEEP`/`AUDIT_BASELINE` 共 12 个变量；全部**设值调用点**（python smoke/e2e 脚本、`package.json` `test:deep`、`electron/platform-launch.cjs`）同步改设新名。新增点名回归测试（`scripts/game-pipeline.test.mjs`）：只设旧名 `APOLLO_PIPELINE_ROOT`（显式删掉新名）→ CLI 仍认得那个临时根，证明 fallback 真生效不是聋子的耳朵。`.apollo/`→`.zerocraft/`：`sysutil.dir_or_legacy(*parts)`（新目录已有优先用·否则旧目录有就读旧的·都没有落新路径待写）用于 6 处消费方（`design_drafts.py`/`groups.py`/`protocols.py`——**发现 `protocols.py` 有真读者 `handle_capgaps_list`，原计划「纯写无读者」判断有误，已改用 fallback**/`llm_log.py`/`workshop_store.py`）；`claude_code.py` 的 `claude-code-cwd`（空沙盒目录·无数据价值）直接切新路径不需要 fallback。写永远落 `.zerocraft/`。`apollo.py`→`zerocraft.py`：`git mv` 保历史；新 `apollo.py`=两行转发 shim（`import zerocraft` 触发模块级注册 + `__main__` 时调 `zerocraft.main()`）；`zerocraft.py` 模块初始化现在**同时**把聚合命名空间对象注册进 `sys.modules['zerocraft']` 与 `sys.modules['apollo']`（同一个对象）——`import apollo`/`import zerocraft`/`python3 apollo.py`/`python3 zerocraft.py` 四种用法行为逐字节一致，写穿透语义（如 `apollo.API_PORT = X` 改到 `server.API_PORT`）双向可见，已用临时脚本实测验证。**顺带查出一处真回归风险**：`electron/platform-launch.cjs`/`platform-main.cjs` 仍 spawn/校验字面量 `apollo.py`，但 `scripts/build-platform.mjs`（本单未碰）本就只把 `zerocraft.py` 拷进打包产物——**打包版 Electron 壳早已（非本单引入）对不上，本单顺手一并修好**（spawn 目标 + env 名同步改新名，`electron/platform-launch.test.mjs` 10/10 仍绿）。约 70 处代码/文档里字面量 `apollo.py` 提及机械改 `zerocraft.py`（scripts/docs/electron/.vscode/src 各处；`requests.md`/`requests-archive.md` 本身的历史记录文字不碰）。`.gitignore` 补齐 `.zerocraft/**` 镜像既有 `.apollo/**` 的 7 条忽略规则（发现的真缺口——迁移前 `.zerocraft/design-drafts/` 等完全没有 gitignore 覆盖，跑过 smoke 测试后一度出现未追踪空目录才揪出）。文档品牌词清扫（非穷举·见下方待裁清单①）：`docs/design/{asset-library,workshop-spec-2026-07-10,game-103/reference-solo-survivor-io}.md`、`docs/workflow/finish/PST-workshop-handoff.md`、`docs/llm-onboarding.md`、`docs/ref/opengame.md`（对比表列头）、`workshop/index.dc.html`（面板提示文案 + 路径提及）。**实测验证**（非只读代码猜对）：`python3 scripts/studio-design-draft-smoke.py` 30/30（真走一遍 `.zerocraft/design-drafts/` 读写）；`python3 scripts/studio-lowmodel-smoke.py` 43/44（1 红=预置 `cards` 模板系统间循环依赖，`git stash` 对比 origin 逐字节复现同一失败，非本单引入，未动）。
> ④ **保留清单**：一字未碰 `apollo-toon`/`apolloOnyx`/`apolloBrocade`/`apollo-sheen`/`data-apollo-*`；GitHub 仓名相关文案（`README.md`/`PUSH-TEST.md`/`docs/workflow/platform-mac-run.md` 的 `git clone` 说明）未动——仓库确实还叫 `ApolloGame`，改了反而是错的。**主动扩大同类保护范围（未经明确指名·执行时自行判断，列入待裁）**：`src/ui/components/{server,render}.ts` 的 `APOLLO_KEYFRAMES` 关键帧族（`apollo-fx-shake`/`apollo-fx-flash`/`apollo-pulse`/`apollo-float`/`apollo-pop`，与被保护的 `apollo-sheen` 同一份模板字符串同生共死）与 `src/studio/*.tsx` 各面板的 CSS class + `@keyframes`（`apollo-design-studio`/`apollo-wizard-panel`/`apollo-settings-panel`/`apollo-settings-row`/`apollo-shelf-newcard` 等，跨 ~10 个 playwright e2e 脚本用字符串选择器耦合）——判断这类改名的爆炸半径与图纸④原意同级（多文件同步改字符串·漏一处不报编译错只报 e2e 静默超时），故比照未动，未经 Lead 点头。
> ⑤ **验证**：`scoped-gate.mjs --run`（scope=full，改动 316 文件触达引擎/共享面）独立复跑两次：tsc 0 · vitest 快车道 425 文件/3749 测全绿 · build 0（仅既有 chunk 体积告警）· docs-ref-guard PASS（40 .md/345 路径）· context-budget-guard PASS · decouple-check 0 违规。慢车道 `ZEROCRAFT_DEEP=1`：446 文件/3954 测（444/3951 绿·2 文件 3 例红——`scripts/acceptance.test.mjs`game-103 缺 adapter+game102 规则不符、`scripts/audit-ratchet.test.mjs`game102 超红旗基线——`git stash` 对比 origin/claude/mainbranch 逐字节复现同一失败集，与 REQ-SPLIT 归档记录的存量问题一致，非本单引入未动）。15 游戏 `node scripts/game-pipeline.mjs board <g>` 全部正确识别 `(compiled)` 无崩溃；`acceptance-run --game game-a` 8/8 场景 PASS；dokiworld 真导出（game-c·224 源文件闭包）在独立 tmp 目录 `npm install && tsc --noEmit && vite build` 全绿，改动前后各跑一次结果一致。单一原子提交，迁移脚本（games/** 别名重写）写在会话 scratchpad 未入库。
> **Lead 终审返工①（2026-08-02·CONCERNS→已修正）**：Lead 按标准最小重演复验（`cp -r games/game-x` 到仓外 + **仅** `"@zerocraft/engine": "file:..."` 一个依赖 + `extends tsconfig.game-base.json` + `npm install`，**不装任何 devDeps**）：`test` 绿，`build` 红——`tsc` 对 `*.test.ts` 报 TS2307 找不到 `vitest` 类型。**根因**：我原验收环境的 `tsconfig.json` 是自己手写的，里面手动加了 `"exclude": [...,"**/*.test.ts",...]`，这份"理想版"配置掩盖了 `zerocraft.mjs build` 命令本身的真实缺口——`cmdBuild` 原样把**目标目录自己的** `tsconfig.json` 喂给 `tsc -p`，若目标 tsconfig（按官方契约只需 `extends` base，不强制手动排除测试文件）的 `include` 覆盖了 `*.test.ts`，构建期类型检查就会去查从未装过的 `vitest` 类型声明——机制层面真漏了这条，不是环境偶然差异。**修正**：`cmdBuild` 新增 `writeBuildTsconfig()`——读目标 `tsconfig.json` 自身的 `exclude`（没有则空数组）+ 追加 `**/*.test.ts`/`**/*.test.tsx`/`**/*.spec.ts`/`**/*.spec.tsx` 去重合并，写一份派生 `<game-dir>/.zerocraft-tsconfig.build.json`（`{"extends":"./tsconfig.json","exclude":[...]}`）喂给 tsc，用完即删。**踩过一个坑**：TS 对 `include`/`exclude` 里的相对 glob 按「声明它的那个配置文件所在目录」解析、不是按 `-p` 传入的入口文件——若把这份派生 tsconfig 写到别处（比如系统临时目录），`exclude` 压根锚不到游戏源码所在的 `game-dir`，形同虚设、悄悄不生效；必须落在与目标 `tsconfig.json` **同一目录**（`gameDir` 顶层）才能正确排除。测试文件的类型检查职责保留在 `zerocraft test`（vitest 自带类型环境，本就不需要额外声明）。**重新最小重演（全新 scratch 目录，逐条照 Lead 复验流程，直接量退出码不隔管道）**：`cp -r games/game-x` → 只写 `package.json`（单依赖 `"@zerocraft/engine": "file:/home/user/ApolloGame"`）+ `tsconfig.json`（仅 `extends`+`baseUrl:"."`+`include`，**不手动排除测试文件**）→ `npm install`（仅落 1 个包，`node_modules/` 下只有 `@zerocraft` 一个符号链接，零 devDeps）→ `zerocraft build`（`BUILD_EXIT=0`，tsc 通过 + vite build 171 模块）→ `zerocraft test`（`TEST_EXIT=0`，4 文件/56 测全绿）→ `ZEROCRAFT_RUN_CHECK=1 zerocraft run`（`RUN_EXIT=0`，自证 fetch 首页 200）→ 另起一次交互态 `run`（非自证模式）+ `curl` 首页与 `/main.tsx` 均 `HTTP 200`，`kill` 干净退出，确认 dev server 真的能被外部客户端访问到、不只是自证探针自己骗自己。改动范围仅 `scripts/zerocraft.mjs`（+`writeBuildTsconfig`），`git status --short` 确认无旁触。全量门禁复跑 tsc 0 / vitest 全绿 / build 0 / 双守卫 PASS / decouple-check 0（见下方⑤补验）。
>
> **待 Lead 裁决清单（拿不准·执行期未自裁·如实留痕）**：
> ① **④ 保留范围主动外扩**（见上·`APOLLO_KEYFRAMES` 关键帧族 + `src/studio/*.tsx` 面板 CSS class/`@keyframes`）——是否认可这类"同一份生成模板/同一批 e2e 选择器耦合"的识别口径当作④的隐含延伸，还是要求逐条明确列名才算保留、其余视为待改？
> ② **window 全局契约族一字未动**：`__APOLLO_INLINE_CART__`/`__APOLLO_INLINE_META__`/`__APOLLO_INLINE_ASSETS__`（单文件卡带打包契约·`src/cartridge-*.ts`/`vite.assets.ts`/`scripts/package-web*.mjs` 等 ~10 处）、`__APOLLO_STEAM__`/`__APOLLO_STEAM_MOCK__`/`__APOLLO_STEAM_CLOUD__`/`__APOLLO_FILE__`（electron preload 桥契约·`src/services/{platform,storage,save}/**`）——不是 env 变量、不是 `apollo.py`、不是 `.apollo/`，图纸①-④都没点名，跨文件精确匹配改名爆炸半径判断与④同级但未经点名，一个没动。
> ③ **localStorage key 字符串一字未动**：`apollo.playerProfile`（`src/services/profile/`）、`apollo-t-progress-v1`/`apollo-t-sfx-mute`（game-t）、`apollo-q-sfx-mute`（game-q）、`apollo:steam:mock`（platform mock 开关）——改名=已存档玩家的浏览器旧档一键失联（key 名对不上=读不到旧存档），未经点名不敢动。
> ④ **`.apollo-config.json`/`.apollo-styles.json` 两个顶层点文件一字未动**：图纸③字面只写“`.apollo/`→`.zerocraft/`”（目录），这两个是仓库根的独立点文件（BYO-key 配置 + 自建风格预设库·均 gitignored·owner 本机很可能已有真实内容），非目录本身，按字面不在范围内；风险也更高（本机现存 key 明文）——未动，需要的话建议另开小单专门裁（含是否要双读 fallback）。
> ⑤ **品牌/包装标识族一字未动**：`com.apollo.*` bundle id（`main_entry/packaging.py`/`scripts/dist.py`/`steam-publisher/`）、`ApolloBalatroDeck`/`ApolloPixelKingdoms` 产物名、`cartridge-station/` 的 `apollo-os`/`apollo-<game>-rk3562.*` 打包命名（`.gitignore` 里也有对应豁免行）——这些是已发布/可能已分发产物的标识连续性，图纸未点名，改名影响升级识别，未动。
> ⑥ **`docs/design/apollo-ui-contract.md`/`apollo-ui-porting-contract.md`/`apollo-ui-migration-guide.md`/`godot-vs-apollo.md` 文档族一字未动**：这是独立的、有自己一套跨文档引用（`docs/roles/PUI.md`/`docs/design/ui-playbook.md`/`docs/design/douyin-minigame-route-2026-07.md`/`requests-3d.md` 等都点名引用这几个文件名）的文档簇，重命名=改文件名+全库改链接，风险不小、图纸未点名，未动。历史层文档（`docs/apollo-engine-overview-for-planner.md`/`wiki/apollo-project-brief.md`，均已挂"仅考古"过期头）同理未动。
> ⑦ **顺路发现但不属本单的两处（据实报不改）**：`scripts/studio-m25-review-e2e.mjs:24` 硬编码了一个不属于本 session 的绝对 scratchpad 路径（`/tmp/.../1cb39fc5-.../scratchpad`，与本单无关的历史残留，巧合命中 grep）；mock 生成链的 `cards` 预设模板（`main_entry/blueprints.py` 相关）存在系统间循环依赖，`studio-lowmodel-smoke.py` 该项断言恒红（已用 git stash 确认 origin 上同样失败）——两者均与位置无关/正名无关，未碰，如实上报供另开单。

> **⚖ Lead 终审（2026-08-03）：✅ PASS（两轮返工后·末段由 Lead 验证落地）**——仓外最小重演（file: 依赖·零 devDeps·最小 tsconfig）三命令直量退出码全 0（commit `aef5fc6ef`）；仓内六项门禁全 0。返工史：①build 未排除测试类型检查（施工侧验收环境手写排除掩住缺口）；②game-base 编译选项独立手写过严致引擎源被错选项重查→改 extends 引擎根配置同源；返工②施工代理静默中止（10h 无动静），其 WIP 经 Lead 标准重演验证后代为落地。**教训记档：验收环境必须最小化（任何手写便利=掩缺口）；长任务代理要有心跳汇报。****七项残名清单裁决**：localStorage 存档键**永不改**（持久化契约·改=用户丢档）；桌面 bundle ID=待打包 epoch；其余五项并入 REQ-RETRO 批次。
...
### REQ-MERGEDIG-合并挖掘·二消清邻格阻碍层（merge-proximity-clear）· [2026-07-25] · GD-101 报（owner 核心乐趣·Gossip Harbor）→ Lead 裁 · status: **✅ done（PE-101 建·2026-07-25 Lead 复核 PASS·归档腾槽）** · 类型: 引擎机制能力缺口（主程域）
> **Lead 复核 PASS（2026-07-25·亲读源码 + 核测试/登记/红线）**：`t2-merge-proximity-clear`——merge-on-place 合成发 `MergeEvent{x,y}` → 对半径内 `Blocker{layers,reveal}` 各 −dec（Chebyshev 整数格邻近）·归零 DestroyRequest + reveal（spawn=SpawnRequest / resource=资源+amount 钳限）·读单例 `MergeProximity{cellSize,radius,dec}`（缺=零回归）。**确定性**（无裸随机/墙钟·按 query 序结算·几何整数比较不喂 Condition）·减层全在引擎 sim·零游戏层/宿主扫格（红线守）·7 测·registry 登记·component-manifest 过。game101 经 board-cover.json 数据消费（挖掘式区域解锁=Gossip Harbor 招牌「空间即奖励」）。API/相位/边界均无需调。原文全文见 git 史。

### REQ-SURVIVOR武器缺口-3 薄件（弹射 bounce / 诱饵 lure / pull 吸附）· [2026-07-24] · PE-game-103 报 → Lead 裁 · status: **✅ done（`a77118a6`·owner 撤 YAGNI 缓建改判建·2026-07-25 Lead 亲验 PASS·归档腾槽）** · 类型: 引擎 capability 薄缺口（Roguelite 武器·主程域）
> **交付（三条 Lead 定形·亲验确定性/撞环/无裸随机）**：① **bounce**=`Launch.bounce?:{times,targetTag}` 薄字段 + 新 `t2-bounce-relay`（launch 自删前落成持久 `Bounce` 组件·命中读 trigger-zone 的 Trigger + Hitbox.targetMask 过滤 → nearestByTag 重定向保 speed·times-1·反应系统 runsAfter trigger-zone/motion-apply/steering）。② **lure**=`Perception.lureTag?` 薄字段（aggro 内诱饵盖过默认目标·缺省零回归·零新能力）。③ **pull**=**走重组不下沉**·新 `t2-pull-anchor` 只批量改邻近「已挂 Steering」实体 Relation.target=锚点·位移复用 steering seek（runsAfter aggro + runsBefore motion-apply 破环）。诚实边界=拉不动无 Steering 实体（黑洞吸敌足够·要吸道具/子弹才下沉 pull-field）。33 测·gate 全绿·component-manifest 135→137·combat.md 回填三行。原文全文见 git 史。

### REQ-SURVIVOR被动轴-玩家属性 modifier 桥（stat-bind）+ Launch 无目标兜底方向 · [2026-07-24] · PE-game-103 报 → Lead 裁 · status: **✅ done（`96492eaa`·下沉 `src/skills/tier2/stat-bind.ts` `t2-stat-bind` + Launch.fallbackDir·2026-07-25 Lead 亲验 PASS·归档腾槽）** · 类型: 引擎能力缺口（stat/modifier 消费桥 + launch 边角·主程域）
> **交付（Lead 亲验·超 owner 原排期 R3·因 game-103 被动 4/8 待桥有真消费者·非 YAGNI）**：`StatBind{bindings:[{source:'ModifierTotals'|'Stats',key,component,field,op?:'set'|'mul'|'add'|'div',base?}]}` → 把聚合值投影到任意组件字段（moveSpeed→Controllable/Steering.speed·range→Shape/Hitbox.radius·attackSpeed→Timer.duration(div 逆映射)·maxHp→Resource.max）。解 `t2-modifier-stack`「ModifierTotals 全库零消费·算了没人用」。**幂等投影**：目标=`projectStatBind(op,base,v)` 纯函数每 tick 从 base 重算（防复利漂移·确定性）。落 **Commit 相位**（读本 tick 已算好的聚合、提交最终投影·排全部 Update 系统后·零 runsAfter 零环·延迟一帧同 steering 纪律）。
> **Lead 收紧（防 game-103 死环）**：stat-bind 纯投影器·`reads` 只含源(StatBind/ModifierTotals/Stats)·**不含写目标**——否则与同 Commit 相位 RMW Resource/Timer 的 `effect-apply`（game-103 蓝图已装）多出反向读边成 Circular（PE-103 死环同类）。加 effect-apply 同装回归测试（game-103 真能力集·maxHp→Resource/attackSpeed→Timer binding·not.toThrow + 投影落地 max=100/duration=30）。
> **+ Launch.fallbackDir**：`toward:'target'` 索敌落空且声明 fallbackDir → 沿它发射（不冻原地·`Math.sqrt` 归一）·缺省无=零回归。gate 全绿（tsc+3449 vitest+build+守卫·component-manifest 含 StatBind）。game-103 现可数据挂 StatBind 接通移速/攻速/范围/最大生命 类被动（原 4/8 惰性→可全接）。原文全文见 git 史。
...
1. **现状定性**：这段在 `games/game-g/lobby-screen.ts` 的**菜单胶水层**，非数据驱动战斗引擎。宣言最咬人处是**战斗**（确定性/公平/可回放）；菜单 chrome 性质不同。但 owner 直觉对——这是「逻辑跳转代码」。
...
### REQ-UI-fontPixel令牌 · [2026-06-27] · PI（game-i 展示台）→ 主程（引擎 UI 域）· status: **✅ done（主程·SHELL+ZeroCraft 基座补 fontPixel 令牌·`font-pixel-default.test.ts`）** · 优先级: P3 · 类型: 令牌补全（小·非结构）
...
> **M4 体检按钮 spec**：
> ③ 操作条加「🩺 体检」→ 新端点 `POST /api/library/<slug>/bench`：薄 node CLI（照 manifest-check.mjs 模式，vite-node 跑）把 manifest → parseManifest → 喂 `src/bench/zerocraft-bench.ts`（先读其真实入口签名）→ 返回 `{score, axes, pass}`（及格线 70）；前端浮层展示五轴分。
...
> - 回填 `docs/playbooks/combat.md`。**不碰 games/**（e/g 的迁移由各域 owner 另立 REQ）。
>
...
> **spec（Lead 图纸）**：① `scripts/game-skill-audit.mjs` 输出分层：**红=已破不变量**（裸 Math.random/innerHTML/自写解释器）·**黄=缺失防线**（零测试/零能力接入/未登记），末行判词 token `AUDIT: PASS|WARNINGS|FAIL` + 对应退出码（0/0/1）。② ZeroCraftBench 帧时轴补 **p99/max 判定**（均值绿尖峰红=CONCERNS·超标帧按帧号点名）+ 同场景 **prior 结果留档做 delta 回归行**（改善也记录）。③ 新脚本 `scripts/test-hygiene-check.mjs`：扫全部 `*.test.ts` 的真时间等待（墙钟 setTimeout/sleep/Date.now）、外部 IO（真 fetch/http）、裸 Math.random；**白名单放行有意用例**（fake timers/mock 合法）；判词 token+退出码。准则出处 `docs/playbooks/testing.md` 红线节。门禁全绿直推；涉 `src/bench`（主程域）按本 spec 施工不越范围。
>
...
> - **② ZeroCraftBench 帧时轴**（`src/bench/zerocraft-bench.ts` 加纯函数 `computeFrameStats/measureFrameTime/frameTimeDelta` + `run-bench.ts` 接线）：墙钟测量每 tick，报 mean/p99/max，**均值绿而 p99/max 尖峰超预算（默认 1000/60ms）→ CONCERNS 并按帧号点名**；prior 留档 `bench-results/frame-times.json`（**gitignore**·墙钟按机器波动不入库），次跑出 **Δmean/Δp99/Δmax 回归行**（↑退化/↓改善/≈持平·改善也记录）。**确定性 hash 逻辑与五轴打分完全未动**（帧时独立墙钟维度·不进 total/退出码）。判词样例：`PASS game-f — mean 0.41ms · p99 1.77ms · max 2.20ms`；`Δp99 ↓改善 2.75ms → 1.77ms (-35.8%)`。
...
> - **点名测试**：`src/bench/zerocraft-bench.frame.test.ts`（7 例·合成数组测 p99/max 判定·尖峰点名·delta 三向·空输入·真引擎 measure）；roster 修改由既有 12 例覆盖仍绿。三工具均自证运行输出见上。
...
> - **断链发现清单（本单主要价值）**：**0 处真断链**——264 路径引用全部命中真文件、agent/技能名近似检测 0 命中。首跑唯一红点 `src/{engine 非 assets,skills,games 逻辑}`（`docs/qa/specs/asset-manager.md:8`）经核**非断链**：是散文里花括号未闭合的口语化标注、非真路径 → 修的是**检测器**（未闭合花括号 fragment 略过），未动文档。另修正初版误判：`scripts/dist.py`·`scripts/*-smoke.py`·`scripts/studio-*` 等一度被当「CCGS 侧示例·ZeroCraft 无此文件」入白名单，实测这些 `.py` 脚本**真实存在**（早前 `ls *.mjs` 过滤漏看）→ 已移出白名单、由存在性检查自然放行。**留给 Lead 裁的项：无**。
...
> **真缺口（此单）**：**生成的库卡带（纯数据 manifest）打不成独立单文件/桌面包**——现 cartridge/electron 管线只认 `games/<id>` 工程游戏（`VITE_TARGET_GAME`）；生成的游戏是 `library/<slug>` 数据，运行时靠**在线 fetch** manifest（`?game=lib:<slug>`），离线单文件里没服务器可 fetch。
...
> ③ **薄适配契约**（`games/<g>/acceptance-adapter.ts`·PE 落·计划门 §4 记账 ~50 行）：`{createWorld(seed,config), applySignal(world,signal,args,by), readWorld(world)→机读态}`——纯接线零规则；runner 只认此契约。
...
> **偏差（据实报 Lead）**：(a) `readWorld` 定为**纯 passthrough**（`(w)=>w`）——机读态提取（扫 Resource/Flag/StringVar/comp）集中在 runner，非各 adapter 手写：直服本 harness 的 RCA（提取写错会掩断言失败）。(b) cart S4 也要 ≥3 场景+conformance，但 conformance 需 `games/<g>/acceptance-adapter.ts`——**通用 cart 适配机制不在本单**（adapter=各 PE/GD 随 S4 落）；有剧本无 adapter=门红（诚实态）。(c) runner 判 main 用 `VITEST` 环境变量（vite-node 剥脚本名·唯一 import 方是 vitest·gate 是 spawn 非 import）。
...
> ① `types.ts`：`PlatformCharacterDraft`（宽容读·全字段可缺）+ `ZeroCraftCharacterCard` 规范卡＝`{id, name, gender?, kind?, media:{avatarUrl?,imageUrl?,animationUrl?,imageName?,animationName?}, persona:{opening?,description?,cardDescription?,personality?,speakingStyle?,boundaries?,catchphrases[],backstory?,worldView?,eraBackground?,rules?,coreConflicts?,exampleDialogues?,conversationStyle?,replySettings?}, tags[], adultConfirmed, visibility?, backgroundPublic?, updatedAt?, passthrough}`。
...
> ⑤ 测试：满卡/空卡（截图 emptyCharacterDraft 同构 fixture）/媒体取优矩阵/成年闸开关/宽容读未知字段进 passthrough/确定性深等/v1 投影兼容。**不碰 games/**（三游戏消费=各 PE 随后接）。
> 完工标 ✅ 待 Lead 对抗性验收。
...
> ④ 测试照 acceptance 存在性门先例：计数纯函数/板提示/CLI 拒过路径/清单行断言。**不碰 games、src/ui**。
...
> **⚖ Lead 对抗性验收（2026-07-29）：✅ PASS**——亲手三态探针：缺产物=点名拒（对齐单/截图分项）·4/5 图=精确计数拒·齐活=放行进 conformance 真判词 ✓；慢车道 32/32 绿独立复跑 ✓；agent 自报的假信心自查（摘牙即红）与全量门禁 rebase 后重跑在案。偏差三条全照准：①既有 fixture 补自证产物（被新门先拦=正确层序）②shots 递归计数（按轮归档正当·手册已明）③八游戏 S4/S5 转红=预期诚实态（照 REQ-ACCEPT 先例·各 PE 补产物即绿）。
...
> ⑤ 红线：纯壳层·不进 sim/hash·不碰 games、src/ui、three-*；测试照 services 先例。
...
> - **红线自证**：`games`/`src/ui`/`three-*` **零触碰**（本提交 diff 里游戏面只有 `docs/design/<g>/requests.md` 的工单文字）；三件全是局外壳层，不进 world/snapshot/hash，不参与回放/lockstep。
...
> **⚖ Lead 对抗性验收（2026-07-29）：✅ PASS**——三件测试 32/32 独立复跑绿；改动面核验（20 文件·games|ui|three-* 零触碰·迁移只落单不动码）；偏差照准：③净码 88>50=codec 闭集各有 ≥2 家存量实证（迁移字节级同格式合理）；leaderboard 只落纯函数（单消费方不造抽象=正确克制）。**顺带上报的引擎核 bug 经 Lead 亲核属实**→ 开 REQ-LOOPSTOP 单独修（本单不背）。index.md 4199/4200 字符：维持现顶·下次要加字先腾字、真腾不动再报 Lead 裁抬顶。
...
> **⚖ Lead 终审（2026-08-01）：✅ PASS**——独立复跑全量门禁六项全 0；解耦守卫双向突变探针咬合（games 越界/src 反向均精确点名）；15 游戏 board 全识别 compiled；dokiworld 导出端到端通过。**偏差四条全照准**：①@runtime/@assembly 别名=同层补缺非新语义（导出路径实证必需）；②白名单 12 处祖父条款=显性记债（按文件精确列名·后续测试精简阶段偿还）；③main_entry python 顺手修=必要（/api/games 否则静默断）；④public 溯源串不动=正确。**终审另揪出**：全家验收剧本 2 红均为迁移前旧账（game-103 adapter 从未落地·game102 规则实现·已各自开单）；慢车道「定期跑」从未接线（owner 07-21 双车道决策的后半句）→ 巡检任务已改为跑 APOLLO_DEEP=1 完整网。**下一层（位置无关）图纸另单出**：别名→包名引用 + 外部内容启动器·验收标准=游戏目录移出仓库仍可构建可玩可测。
...
> **⚖ Lead 终审（2026-08-03）：✅ PASS**——独立复核（不采信施工自报）：HEAD=origin=`ca7b2fbc1`·工作树净；三守卫独立复跑退出码全 0（docs-ref-guard / decouple-check / context-budget-guard）+ 还原后推送态 tsc=0·build=0（`23a7b74f9` 亲测）；game-d+game-f 自身测试套独立复跑绿（退出码 0）；launcher 加载表亲核（d/f 在位·q/t/x 清零·货架 12 游戏）；三张新工单（B-014 / REQ-I-lab测试弱断言 / REQ-G102-HARDLINE）逐一在档核对。**据实偏差三条裁决**：①夹具解耦不随 game-f 还原撤销=**照准**（引擎测试借真实游戏当夹具是架构债·与游戏生死无关）；②白名单口径更正 12→5=**照准**（如实记账·game-f/lobby.tsx 随还原回填非新增）；③requests-archive 按月拆卷延后=**照准**（时间压力下不碰 1880 行历史审计=正确克制·后置不占槽）。品牌残名五项维持记债。深慢车道 3 红全部有主（2×acceptance 旧账已单·1×audit-ratchet=REQ-G102-HARDLINE 新单）。
> **⚠ 事故留痕（同单复盘·责任=Lead）**：施工中途 Lead 提交调研档时把施工代理已暂存的 ~400 删除卷入 `d140e4b9f` 提前推送（docs-ref-guard 红灯被管道量吞未拦），owner 本机拉到断态全游戏不可玩；恢复=施工代理续中间态两笔提交（`971551396` 引用清零 + `23a7b74f9` owner 同日改判 d/f 还原）。**两教训固化 CLAUDE.md 推送门禁**：共享工作树提交先查暂存区只提自己文件；守卫退出码永不经管道。施工代理另有一次 600s 看门狗停摆（唤醒续跑完成）——低档长任务拆小步防看门狗已随唤醒指令记档。
...
> **回执（施工 2026-08-03）**：①-⑤ 已完工——① 三陈旧目录（`public/games/{game,sample-platformer,smoke-test-game}`）批删（`git rm -r`·引用核查已证安全）；② `wiki/skills/index.md` 四处错名修正（`collision-separate`→`collision-resolve`；`ai-patrol,ai-chase` 注「无独立能力·按宣言由 aggro+steering+state 装配」；`resource-modify,damage-number` 注「无此能力名·registry 未建」；`check.resolve,love-interest,settlement` 注「乙游前瞻规划·registry 未建」·只改错名未扩写）；③ `main_entry/library_api.py:91` 附近过期 docstring（match3/dressup 示例）改为与 `blueprints.py PRESET_BLUEPRINTS` 现况一致（platformer/pong）；④ `scripts/decouple-check.mjs` 白名单 5 条重分类注释（SRC_GRANDFATHERED 3 条 studio 条目注「产品功能耦合·长期条目·非待偿测试债」；A_GRANDFATHERED 的 game-c/dokiworld-export.test 注「锚点守卫·须读真实导出插件·性质不同不偿还」、game-f/lobby.tsx 注「冻结封锁·随 game-f 冻结政策」·只改注释零行为改动·守卫自测仍绿）；⑤ 三张游戏级测速工单开出（`docs/design/game-103/requests.md REQ-103-测速`·`docs/design/game-c/requests.md REQ-C-测速`·`docs/design/game-g/requests.md REQ-G-测速`·均 P3·各指派对应 PE）。
> **裁决留痕（三条）**：能力矩阵实口径 = **capability 对象 100 项**（条目原文「192」系 export 计数口径·非 capability 对象数·就地更正）；矩阵半成品仅作情报存档、不施工（不删不合并·维持①「不删」原裁）。测试套裁决：慢测两单（game-103/game-c）+ game-g 自愿单已开出；弱断言 6/7 已有单覆盖，第 7 件（game-f 快照类）判**不开单**（game-f 冻结政策封锁）；死夹具排查=**0 件**；白名单 5 条重分类为「长期条目」/「性质不同不适用」，非待偿测试债。audit-ratchet 计时疑点**仅登记**（未施工·留待后续巡检核实）。
> **归档拆卷**：仍**等 owner 终字**——方案已侦察（按日期标签分桶·零引用破坏），本轮不执行。
> **⚖ Lead 终审（2026-08-03）：✅ PASS**——独立复核（不采信施工自报）：HEAD=origin=`d26f94b28`·工作树净；三守卫独立复跑退出码全 0（decouple-check / docs-ref-guard / context-budget-guard）；`public/games/` 三陈旧目录确认消失（现存 10 台账目录全现役）；wiki 错名清零（`collision-separate` 0 命中）；三张测速工单（REQ-103-测速 / REQ-C-测速 / REQ-G-测速）逐一在档。二期总账：老数据清理（3 目录 + 4 错名 + 1 过期 docstring）；测试套裁决（慢测两单 + game-g 自愿单开出·弱断言 6/7 已有单第 7 件判不开·死夹具 0·白名单 5 条定性「长期条目/性质不同」**收缩到此终止**）；能力库按 owner 裁**不删**（矩阵半成品留情报·capability 对象实口径 100 项）；audit-ratchet 计时疑点仅登记。悬置项唯一：归档拆卷等 owner 终字（方案在案：按日期标签分桶·桶内保序·零引用破坏——owner 若仍要「删」亦可执行，未字前不动）。REQ-DIALOGUE 启动条件（二期结案）自本终审起满足。
