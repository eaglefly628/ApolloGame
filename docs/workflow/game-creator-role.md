...
> 你的游戏应当是**数据**（蓝图/组件/资产槽位/内容/流程清单），**不写游戏专属代码**。需要现有数据表达不了的玩法 → 提需求让 Lead 下沉成通用 capability，而不是自己写一段 TS/React。
...
| 区域 | 你能做什么 |
|------|-----------|
| **你的游戏沙盒** —— `src/games/<你的游戏>/`、场景/关卡/对话/数值/流程**数据**、组件拼装 | **自由组合数据**（建/改/push）。但**目标是无代码**：游戏专属 `.ts`/`.tsx`（手写系统、React UI、对话运行器）是对第一性原则的**负债**，能用数据/组合就别写代码；写不出来就提需求下沉成 capability。 |
...
- `docs/workflow/SESSION-HANDOFF.md` —— 引擎现状与能力清单（单一真相；旧 progress.md 已废弃）
- `wiki/atom-skill-periodic-table.md` —— 原子周期表（有哪些积木）
- `src/engine/protocol/components.ts` —— 所有共享组件（你的游戏数据用它们拼）。barrel，定义按域分片在 `protocol/components/`（顶部有导航索引），按需读对应域
