...
#### 任务 F-17 · REQ-F-046/048① 接入：升星合成 + 超员自动卖（纯数据） — status: **done（PE-F 2026-06-11 回执，受限版全链：①升星=席位 marker 家族合成（bench/bench2/bench3_<将> 三档模板，每将两条 MergeRule；★ 角标子体随席 cascade）——「席位与上场槽同模板」的统一族要 REQ-F-049 部署门，落地后可整段简化；②星级入战斗：每将 star_<将> 资源（marker 计数 GroupCount(STAR2|每将位 含齐)→三条升降带）→ 部署窗按星分流 12 槽位（slot_<将>_s1/2/3，overrides 血×1.8^(星-1) + SelfRule.do 换 strike/ult_s<星> 弹=伤×1.5^(星-1)）；坑：部署窗必须门脉冲化（deploy_gate_done 同拍撤臂）——否则备战中途合成改 star 让升星带在同窗复燃=双子；③超员自动卖=cap_armed 入战拍→enforce_cap destroy-tagged TEAM_A keepResource:'level'（现 4 槽恒≤level=休眠保险丝，F-9 多实例测试为此抬 level 注入）；④bench_space 改派生：bench_cap−bench_occupied(GroupCount BENCH_OCC) 每拍 level 信号重算——合成 3→1 自动回 2 席，手工±对账整链删除（rune_c 改写 bench_cap）；⑤袋扩 9/将（3星可达，只追加不重排锁住前 24 张断言）；⑥星级卖价 2/8/26（=3×3−1/3×9−1），2/3星卖出不归还袋（3 张已熔毁，known wart 记 TUNE）。升星/拖拽/超员 3 新测 + 全套 24/24、1087 全绿）**
...
#### 任务 F-18 · REQ-F-045 接入摆子拖拽（纯数据+1 行壳层参数） — status: **partial（PE-F 2026-06-11：数据侧全就位——marker 全星级挂 Draggable{onlyFlag:'in_prep'}（惰性零开销）、in_prep 门旗 flow prep 进出维护、数据侧测试锁就绪度；**系统注册被 22 系统 SCC 挡下**：drag-place（读写 Transform）与 motion-apply（主角自由移动）互为 Transform RMW 对、runsBefore 六件套漏 'motion-apply'——探针二分定位+补丁假设 60 拍验绿，一行定序补丁已提 REQ-F-050；「部署链随新位置展开」三路纯数据重组证伪（grid-move 双键查询无 HexPos 即罚站/overrides 静态/常驻 Caster 在席也出兵），按注③预告提 REQ-F-049（Caster.requireHexPos + HexPos 继承）。两单落地后：解注 capabilities 一行 + marker 补 snap/cap 字段即全通）**
