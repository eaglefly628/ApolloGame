# 游戏工单池 · Requests-Games

> 2026-07-15 立（owner 拍板「需求池只留 10 条 owner 级硬需求」）——各游戏的**工作票**从主池分流至此：
> game-G 战斗/演出/平衡/UI 修、game-D 迁移与记债、game-Q/I 缺口。**不占主池 10 槽**；域主（程序A/B/PG/PE/design-G/P3D）自取自结。
> 纪律与主池同源：done 迁 `requests-archive.md`；引擎级缺口若需 Lead 裁决→占主池槽或走 capgap 通道。3D 线仍在 `requests-3d.md`。
