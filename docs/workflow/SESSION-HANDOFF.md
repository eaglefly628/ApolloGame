...
- **Game E · 小丑牌 roguelike(Balatro 切片, 投资路演靶点)**：纯数据装配的卡牌计分链，零游戏 system。一手计分（同 tick）：**poker-hand**(REQ-011) 判牌型 set 基础 chips/mult → **card-scoring**(REQ-014) 逐张累加 baseChips + 逐张小丑(每♦+m/每人头+c/retrigger) → **effect-apply** op/order(REQ-012) 有序加乘 hand-level 小丑 → **valueFrom**(REQ-013) `hand_score=chips×mult`(资源×资源) + Bull 每$1+2c(量纲动态值)。每张小丑=一条声明式数据(Effect / PerCardRule / PerCardRetrigger)。150 张官方小丑元数据 + 起手 14 张铺满 7 型。`game-e.tsx` 挂载 + 透视器接入。**retrigger×逐张小丑乘性耦合**（Hanging Chad 首♦→Greedy 触发7次）是聚合计数表达不了、必须逐张迭代的招牌证明。
