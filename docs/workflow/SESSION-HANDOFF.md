...
- **P0·冲刺主线（压过一切非冲刺工单）**：7·29 审核 demo——纲领=`docs/design/demo-sprint-2026-07-29.md`；生产总线=八阶段流程板（`docs/playbooks/game-production.md`）；**Workshop 统一工作台 spec 已定稿=`docs/design/workshop-spec-2026-07-10.md` → REQ-WORKSHOP（PST 施工·Lead 验收）**。残项归位：美术真 key=owner 采购（纲领 §六·LLM 通道改订阅 setup-token 不采购）；T3 吞吐=Opus 单；进度灯=PST 心跳队列；refImage=真 key 后验。
