...
### REQ-FLOWFIELD · 群体流场寻路能力 `t2-flow-field`（大规模 RTS 的寻路地基）· [2026-08-10] · **owner 拍板要做**（原话「我建议写流场寻路，我想做大规模 rts」「这个需求提给主程序来写，我们给一些最新技术的调研」）· **施工主体 = 主程 = 本 session（2026-08-24 抢锁·owner 当面指派「你就是主程」·本行即锁）** · 复查 = 另派独立 agent（复查人≠施工人红线·Lead 身份已因施工作废） · status: **in-progress（M1+软分离已复查毕·ORCA 三轮复查毕·13 条实伤全修·复查判「可交 Demo」→ 等 game211 反馈·M2/M3/M4 未做）** · 优先级: **P1（大规模 RTS 的前置地基·现有寻路实测撞墙）** · 类型: 引擎能力下沉（运动/寻路线）
...
第一轮 FAIL 打回的**不是算法**，是提交信息里三条头号声明经实证全为假：① 缓存是**状态通道**
（量化摘要做键、消费端吃原始浮点 ⇒ 差 0.0004 的两张场共用一份·lockstep 静默分叉）② 「与单位数无关」**零判据**
③ **新增了定序环**（`topological-sort` 遇环只告警不抛 ⇒ 全库一条不红）。第二轮 CONCERNS 再查出
④ 修 ① 时把键简化成裸 id ⇒ **6.7× 性能悬崖** ⑤ 把一条测试的功劳说大了。**逐条判词与复现步骤见上面两份报告。**
...
**软分离五次栽在同一个地方，教训写死在代码注释里**（都是实测逼出来的·不是设计出来的）：
① 力不许归一化（归一化后夹中间的和站边上的一样被推 ⇒ 整堆平移、间距纹丝不动 minPair 恒 0.0100）
② 不许除以实际邻居数（远邻居会稀释近邻的推力 ⇒ 受力忽大忽小 ⇒ 抖）③ 钳位只当天花板（同 ① 的病换个位置）
④ 越过到达线要减速（否则被挤出去的单位满速冲回、把刚散开的堆压实，队伍约 40 拍一个聚散周期）
⑤ 到点不许硬停（停了就不再互推 ⇒ 全叠在一个点 = RTS 里最显眼的假）。
...
**🛡 ORCA 硬避让**（三轮独立复查：**FAIL → CONCERNS → CONCERNS·判「可交 Demo」**·三批实伤共 13 条全修完）
`src/skills/tier2/orca.ts` = 逐行移植 RVO2 `Agent.cc`（Apache-2.0 三件套齐·偏离七条写在文件头）。
接口 `FlowAgent.orca?: { radius, timeHorizon?, maxNeighbors? }`，**不设 = 一个字节不变**（有点名用例咬着）。
...
**⚠ 对外口径已改（两轮复查各打掉我一句话，都不是算法错，是我把话说大了）**
- ~~「timeHorizon 拍内保证互不碰撞」~~ → **前提是线性规划有可行解**，迎面对撞时经常没有，落 LP3「最不违反」
  就是真压进去。同场景扫一族起始排布实测（中场段）：纯流场 0.047~0.100（直接对穿） · 软分离 0.061~0.128 · **ORCA 0.644~0.701**（半径和 0.70）。
  **ORCA 挣到的是「~90% 穿模 → 最坏 8%」，不是 0**，且最坏点在中场而非终点。
- ~~「不还礼的邻居仍有 0.603 的底」~~ → **没有下界**（2v2 0.505 · 3v3 0.187 · 慢对快 0.165）。混装要干净 = 双方都开。

**三轮判词、13+21+15 刀 sabotage、逐条复现步骤**：`docs/design/game211/orca-review{,2,3}-2026-08-25.md`；
**13 条实伤的病根与修法**：调研 §10.5 / §10.6 / §10.7。**别在池子里重抄。**

**三复查的判词 = 可以交 game211 做 Demo**（口径已是实话·承重判据从"一个点"变成"一族+分区+对照"·
15 刀里凡动到算法本体的全部恰红）。它另打出三条并已修完：**trace 折叠把最该喊的那一拍变成唯一不喊的**
（全员 fieldId 打错 → 一拍 0 条 trace·门漏了 continue 掉的两类）· **我新加的 JSDoc 把原文档挤成孤儿**
（TS 只认紧邻成员的最后一块·而我上一轮那条源码级用例恒绿）· **我说的「固有边界」不成立**——
`timeHorizon` 是活旋钮且**非单调**（中场 H8 0.644 / H16 0.692，但终点段 H12 只有 0.318、H20 只有 0.169），
缺省 8 的理由改成"两段都不塌的保守点"，档位交 Demo 定。

**这三轮沉淀成纪律的四条**（比单子本身值钱）：
· **「承重用例只钉一个初始条件」本身就是缺陷形状** —— 尤其当被测指标对初始条件混沌时（挪 0.25 就翻盘）。
· **记错的机理比不记更危险** —— 我给 `ORCA_RANGE_SLACK` 编的「过约束⇒无解」被实测证伪（LP3 落点反而 8→4）。
· **文档注释要验"挂没挂上"** —— 我新写的两块 JSDoc 一块都没挂到成员上，我请人去判的披露作者根本看不到；
  而且验的判据必须是**编译器取到的那份文档**，不是"源码里有这段字"（后者被"挤成孤儿"这一形态恒绿骗过）。
· **修 A 时新增的机制 B，B 自己也要配一刀** —— 三轮的新伤全是这个形态（LP3/提前退出/量级 → JSDoc 孤儿 →
  折叠的门/兜底的方向/暴力搜的样本量）。**已加进修复类提交的自检清单：新增的每一处分支·门·常量各配一刀。**
