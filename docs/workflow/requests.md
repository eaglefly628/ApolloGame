...
> **真演闭环（block-blast-mini）**：独立复查 agent 照单复查 S2-S5 + 如实打 S7 评分卡（色块初版=低分·丑明明白白写在板上）——见该游戏 pipeline.json reviews/scorecard 落账。**测试**：`scripts/game-pipeline.review.test.mjs` 8 例（复查语义/评分卡语义/三门合成/FAIL 压过机器绿/指纹过期/豁免关不受扰）。
