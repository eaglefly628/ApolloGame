...
### REQ-G-疲劳休整恢复 · [2026-07-04] · owner → design G(裁定+数值)+程序A(战斗核逻辑+AI)+程序B(恢复演出) · Game G · status: **open·待 design G 裁定+定模型** · 优先级: P1(核心战斗核·压核心稳) · 类型: 战斗核疲劳模型改（离散→连续）+ 平衡重标
...
> **① 对折无下限→战力打到 0·掷骰区间退化 `[1~0]`（逻辑bug）**：`:321` `mul = champMul * 0.5^wins`、`:343` `Math.max(0,…)` → 连胜对折 + 士气/溃散惩罚可把有效战力压到 **0**（log T14 下路：`连胜2·战力已对折 = 0 → 掷战力骰 [1~0] = 3`）。`[1~0]` 是非法掷骰区间。**待修**：对折后有效战力**钳下限 ≥1**（`Math.max(1, …)`·至少能掷 [1~1]）；或按 owner 定"对折不低于底牌力的 X%"。GD 建议下限 1（掷骰系统前提=区间合法）。
...
> **附·占位deck 验收点**（并入 `REQ-G-Boss写死明牌天罡`）：接 16写死牌组时须核——**主将=A♠列奥尼达**（现占位 deck 把主将挂到 3♠ 且 养成+16→战力20 墙）；敌杂兵养成、玩家养成须回到 boss-config 公平口径（现 敌-2/我+1/主将+16 全是占位乱值）。
