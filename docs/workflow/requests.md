...
### REQ-DESIGNLINE-设计稿产线·二期 · S3 后自动生成「全控件清单版」UI 设计需求单 + 收稿对账（owner 2026-08-07 令重开） → **施工主体=Lead 代理（已派）** · status: **done（施工完·待 Lead 对抗性验收）** · 优先级: P1 · 类型: 生产线工具（scripts 推导器+workshop 接线·PST 域）
> **①②③⑥ 全部落地**：`scripts/ui-brief.mjs --game <slug> [--taste "…"] [--json]`（纯函数+CLI）——**全动作清单**三源合并去重（该游戏验收剧本 signal 名【词表对齐律同源】→ 蓝图/manifest 扫描【`Clickable.action`/`KeyBinding.signal`·或 'compiled' 形态正则扫 `games/<slug>/**/*.ts` 字面量】→ R2 实测 `probe/ui-inventory.json` 钩子【本仓暂无生产者持久化·恒空是「若在档」条件不成立非缺陷】）+ **屏清单**（GDD「屏幕」节表格/列表推导·推不出留占位）+ **风格锚**（同 workshop `refreshDesignAnchor` 先例查 art-ledger）+ **品味槽**（`--taste`）+ **固定输出契约段**（④·单文件/zip 两收·`data-action` 照抄·屏尺寸·关键状态·收稿箱交付）+ **⑤交付方式**（工坊拖拽 / `design-import` 命令行两条路）。workshop 步进器「🎨 设计稿产线」区加「📐 生成 UI 设计需求单」钮（S3 骨架关绿后可点·未绿灰字提示）+ 品味输入框，结果进既有一键复制框；`POST /api/design/ui-brief` 薄封装（同 `_pipeline_cli` 先例）。`design_ingest.py` ingest 时核「brief② vs 稿内 data-action」，缺项记 `briefCheck:{missing,ok}`（不拒收·台账+响应+收稿箱行 ⚠ 亮警示）。
> **④ZIP 收稿 + ⑤命令行导入 全部落地**（owner 2026-08-08 追加·`--watch` 轮询同日 owner 明裁**砍掉不做**）：`handle_design_ingest_zip`——zip-slip 纵深断言（越界=整包拒收）+ 50MB 上限（原始包体+解压后总量双卡防炸弹）+ 扩展名白名单（html/css/png/jpg/jpeg/svg/webp/json·外白名单单条跳过非致命）+ html 仍拒外链 `<script src=`+入口 html 判定（index.html 优先/唯一亦可/多个判不出则拒收）→ 落 `docs/design/<slug>/ui-refs/<稿名>/`（保留包内相对路径·全部校验先行·任何一条不过零字节落盘）+ 台账 `kind:'pack'`+`entryHtml`+`files[{path,sha256,size}]`。`GET /api/design/preview` 泛化为可读 pack 内任意相对路径（否则预览裂图）。`python3 zerocraft.py design-import <zip> --game <slug>`（`main_entry/cli.py::cmd_design_import`）零 HTTP 直调同一套逻辑。
> **测试**：`scripts/ui-brief.test.mjs`（21 例：屏清单/三源合并去重/风格锚/契约段模板/端到端落盘）+ `scripts/design-ingest-zip.test.mjs`→spawn `scripts/design-ingest-zip-check.py`（zip-slip/超限/白名单跳过/台账完整性 22 断言）——vitest 全纳入 `scoped-gate --run`。**假信心自查**：手改 `mergeActions` 短路去重判断（保留 push 但去掉 `if (!bySources.has(...))` 早退）→ 目标测试即时转红（合并数组从 3 变 5）；撤修复验绿——记录在案，非自陈。
> **真跑**：`node scripts/ui-brief.mjs --game game-a` → `docs/design/game-a/ui-briefs/brief-2026-08-08.md`（0 屏【GDD 无「屏幕」节·占位】·25 动作【剧本 6（旧引擎信号 play-round/next-round/… ）+ 蓝图 19（真 UI `hud.ts` 的 `menu.start`/`hand.toggle`/… ）·**两源零重叠**——现场复现了 RENDERCHECK R2「剧本词表与真 UI 两套从未同源」的病灶，非臆测】）；`node scripts/ui-brief.mjs --game game108 --taste "走暗黑帝国感·金属质感强"` → `docs/design/game108/ui-briefs/brief-2026-08-08.md`（6 屏·6 动作【GDD §14 词表声明 10 个·验收剧本现只覆盖 6 个——`throw.void`/`shard.pick`/`duel.next`/`charge.scissors` 尚无剧本覆盖，如实呈现，非工具 bug】）；`design-import` 真造 zip（含 `.DS_Store` 白名单外文件+图片相对路径）真导入 game108 一遍，对账正确列出 6 个缺项。**过程中现场逮到并修的两处真 bug**（撞见即修·非预先设计）：① `_brief_action_list` 正则未限定表格行，把 ④ 契约段里字面量 `` `data-action` `` 反引号误当成一个动作名（真跑 `design-import` 时台账多出虚假 missing 项，复现后限定「只认表格行」修复）；② zip 拒收路径（zip-slip 一支）在校验前就 `mkdir` 了 `ui-refs/`，导致拒收后仍留一个空目录——挪到全部校验通过之后才建目录。
> **回填**：`docs/workflow/finish/PST-workshop-handoff.md` 部件地图/接口契约/验证方法/已知债四处补齐。**门禁**：`node scripts/scoped-gate.mjs --run`（scope=full，因触碰 `scripts/`）见提交记录，退出码直接量。**待 Lead 复查**：抢锁/施工与复查铁律要求复查人独立复跑（不采信本单自陈）+ 撤修验红。主轨（自养设计会话）仍等 owner 亲比，未动。
