...
### REQ-FLOWFIELD · 群体流场寻路能力 `t2-flow-field`（大规模 RTS 的寻路地基）· [2026-08-10] · **owner 拍板要做**（原话「我建议写流场寻路，我想做大规模 rts」「这个需求提给主程序来写，我们给一些最新技术的调研」）· **施工主体 = 主程 = 本 session（2026-08-24 抢锁·owner 当面指派「你就是主程」·本行即锁）** · 复查 = 另派独立 agent（复查人≠施工人红线·Lead 身份已因施工作废） · status: **in-progress（M1+软分离已复查毕·ORCA 两轮复查 FAIL→CONCERNS·两批实伤均已修完·待三复查·M2/M3/M4 未做）** · 优先级: **P1（大规模 RTS 的前置地基·现有寻路实测撞墙）** · 类型: 引擎能力下沉（运动/寻路线）
...
**🛡 ORCA 硬避让**（`2e9ea915` 交 → 复查 **FAIL** → `2c0a9524` 修完 → 再复查 **CONCERNS** → 本轮四条新伤修完 · **待三复查**）
`src/skills/tier2/orca.ts` = 逐行移植 RVO2 `Agent.cc`（Apache-2.0 三件套齐：源文件 SPDX + `licenses/Apache-2.0.txt` +
`THIRD-PARTY-NOTICES.md` 修改说明）。接口 `FlowAgent.orca?: { radius, timeHorizon?, maxNeighbors? }`，
**不设 = 一个字节不变**（有点名用例咬着）。偏离七条写在文件头。
**三档开销**（同机 bench）：纯流场 0.39/2.51 · 软分离 1.01/4.08 · ORCA **8.64/35.77** ms/tick（1000/4000 单位）
...
- ~~「timeHorizon 拍内保证互不碰撞」~~ → **前提是线性规划有可行解**，迎面对撞时经常没有，落 LP3「最不违反」
  就是真压进去。同场景扫一族起始排布实测：纯流场 0.047~0.100 · 软分离 0.061~0.224 · **ORCA 0.631~0.701**
  （半径和 0.70）。**ORCA 挣到的是「~90% 穿模 → 最坏 10%」，不是 0**，且最坏点在中场而非终点。
- ~~「不还礼的邻居仍有 0.603 的底」~~ → **没有下界**（2v2 0.505 · 3v3 0.187 · 慢对快 0.165）。混装要干净 = 双方都开。

**两轮复查的判词、13+21 刀 sabotage、逐条复现步骤**：`docs/design/game211/orca-review-2026-08-25.md`
与 `…-review2-2026-08-25.md`；**六条 + 四条实伤的病根与修法**：调研 §10.5 / §10.6。**别在池子里重抄。**
