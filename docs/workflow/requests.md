...
### REQ-F-044 · [2026-06-10] · PE-F（批C 主角拾取接入，§4.7 预案兑现）· 框架级 · status: **done（Hitbox.consumeOnHit；接入派 inbox F-16）** · 优先级: 高（主角拾取入账=Phase 2.5 收口件；"一次性触碰结算"是拾取/独头弹/踩雷通用形状） · 类型: 真缺口（hitbox 无"结算即耗尽"；trigger-zone 双 zone 互斥使"双向两清"不可表达）
...
>
> **Lead 裁决（2026-06-10）**：**接受 (A)，回驳 (B)（如你所料：双 zone 配对动所有既有 zone 语义，面太大）。** 三条重组证伪全对（语义错/泵变慢/baroque）。落地：`Hitbox.consumeOnHit?: boolean`——**真结算**（过了阵营/状态门）才计入 settled 集（集合语义与遍历序无关），拍末对自身发 DestroyRequest；实测语义比提案更干净：**同拍即清**（hitbox 写请求→destroy-apply 同拍消费，writer→consumer 自然定序，不留尾拍）。cascade 连挂件。hitbox writes 加 'DestroyRequest'（单向汇入 cascade/destroy-apply，零回边）。3 测（碰一下入账一次+同拍消失+站桩不再涨 / 缺省持续结算回归 / 与 047 组合）。
...
### REQ-F-047 · [2026-06-10] · PE-F（§一.4 羁绊 + 战斗型符文共同前置）· 框架级 · status: **done（Hitbox.scaleByResource；REQ-023 簇最窄落点成立）** · 优先级: 高（Phase 3 主体） · 类型: 真缺口（伤害读不了活属性——buff 无处施加）
...
>
> **Lead 裁决（2026-06-10）**：**接受 (A)，回驳 (B)**——per-attacker Stats 比需求宽（羁绊/符文是阵营级系数；真到"逐单位攻击成长"再开 REQ-023 簇下一格）。落地：`Hitbox.scaleByResource?: string`，结算时 amount × 全局系数 Resource.current（缺省/找不到=×1 零迁移）；**fracOfMax 部分不乘**（保"按目标 max"语义独立）。同模板两阵营各自系数=槽位 overrides 改本字段指向各自资源 id（纯数据，F-033 管道现成）。hitbox 本就读 Resource——**零声明变化零新边**。1 测（×3 系数 + 缺资源回退）。羁绊全链就此重组闭环：group-count → EventWhen 阈值 → Effect 写系数 → scaleByResource 生效。
...
### REQ-F-054 · [2026-06-11] · 用户实测 bug（「商店越刷人越少最后没了」）· 框架级 · status: **done（同日，代理主程落地）** · 优先级: 高（经济主循环可用性） · 类型: 引擎缺陷（card-pile 刷新把手牌烧掉——有限袋只出不进）

> **根因**：`refreshOnSignal` 实现为 `pile.hand = []`（弃到虚空）。有限袋（36 张）每次刷新净 −handSize，自动刷新每回合再 −handSize → 数回合+几次手动刷新即枯竭，手牌补不满 → 卡面越来越少直至空。
> **裁决**：金铲铲语义=没被买走的牌**归还公共池**。改 refresh 为「旧手牌回袋底再补」——卡池守恒（只有买走才离池、卖出归还），连刷无限轮换永不枯竭，三星可达性语义同时保真。唯一使用方 game-f，零迁移面；引擎测改为守恒断言（含连刷两轮轮换）。

---

### REQ-F-055 · [2026-06-11] · 用户钦定（「下排英雄平台：买入落座、来回拖着互换」）· 框架级 · status: **done（同日，代理主程落地）** · 优先级: 高（备战席=核心交互面） · 类型: 真缺口（「一排槽位的占位分配/互换」无词汇）

> **证伪重组**：① 买入 marker 出生点=持位 Caster 的静态 Transform——同将叠同点、不同将各占一点，没有「第一个空槽」概念（占位分配需要扫描在席集合，条件/Effect 词汇无此能力）；② 席内互换=两实体席位状态对调——拖拽只写被拖者 Transform，无状态可换；③ 9 个槽框纯视觉摆得出来，但落座/让座/弹回全是状态机。
> **裁决**：下沉窄能力 `t2-tray`（`Tray{originX,originY,gap,capacity,requiredTag}` 配置 + `TraySeat{index}` 运行时 POD）——自动落座（最小空槽，id 序确定）/ 拖到他槽=互换或挪动 / 拖上板（得 HexPos）=让座 / 无效落点=弹回原槽（地上不留单位）。**备战席、手牌排、背包栏同形**，通用性达标。定序：runsAfter drag-place（拖完本拍收口）+ runsBefore grid-move/motion-apply（Transform RMW 对，同 REQ-F-050 纪律）。评审自纠一处：互换后被换方同拍会被误判"又被拖了"再换回——先快照偏离名单再统一执行（4 测含此回归）。
> 豁口注记：容量满后新成员顺延排出托盘右侧（入口由容量资源把门，托盘只管摆不当裁判）；落座取最小空槽而非最近槽（TUNE 候选）。
### REQ-F-061 · [2026-06-13] · 主策划（Game F 卡牌系统 D0 核对拉动；白衣/攻心/渡江/先登进阶）· 框架级 · status: **open（提主程评估）** · 优先级: 中（差异化「斩杀/抢人头」流派的必需算子；不阻塞 D1 单机原型） · 类型: 真缺口（hitbox 缺「按目标当前血量比例」的条件门 + 处决）

> **要表达的游戏行为**：斩杀（目标 hp<X% → 致死/暴伤）、残血加伤、狂暴。卡牌「白衣」（对 hp<30% ×N 伤害）、「攻心」（目标已损血越多伤害越高）、「渡江」星球牌（斩杀线档位）、「先登进阶」（对满血加伤）全压在此。
> **缺什么（已核真实代码 `src/skills/tier2/hitbox.ts`）**：命中过滤只有 ① `targetMask`（Tag 阵营）② `requireMask`（Status 位）；伤害只有 `amount` + `fracOfMax`（伤害=目标 max 的比例，**不是**按当前血量的条件）。**没有任何「读目标当前 hp 比例做条件门/处决」的词汇**。`fracOfMax` 是伤害量纲，不是条件。
> **为什么不能重组绕过**：现有条件门只认 Status 位，血量是连续 Resource，烘不成 Status；`condition`/`event-when` 能读资源但那是**触发**层，管不到 hitbox 命中那一刻的目标血量。→ 真缺口。
> **为什么值得下沉（通用性）**：「按目标血量比例的条件伤害/处决」是**普适 ARPG 词汇**（斩杀/狂暴/残血斩），非一卡专用。
> **建议形态（窄·只读·不碰 §7 红线）**：给 `Hitbox` 加可选只读门 `requireHpFracBelow?`/`requireHpFracAbove?`（读 target 当前 `resource.current/max`），不满足则本目标跳过结算；处决=配 `executeBelow?` 命中即把目标该资源打到 0。**只读 hp 比例做 gate，绝不引入伤害分型/重定向**（守草船借箭回驳边界，范围才窄、确定性不变）。
> **复用对账**：倍率（×N）走 REQ-012 `Effect.op:'mul'`（done）；动态值走 REQ-013 `valueFrom`（done）。本 REQ 只补「条件门」这一块。

---

### REQ-F-062 · [2026-06-13] · 主策划（Game F 卡牌系统 D0 核对拉动；绕后奇袭）· 框架级 · status: **open（提主程评估）** · 优先级: 低-中（刺客「绕后」/狙击/嘲讽等流派；不阻塞 D1） · 类型: 真缺口（aggro 索敌策略不可选，只能最近）

> **要表达的游戏行为**：刺客直插后排（锁「最远/最高费」敌）、嘲讽（被迫锁某单位）、狙击（锁最高威胁/最低血）。卡牌「绕后奇袭」需之。
> **缺什么（已核真实代码 `src/skills/tier3/aggro.ts`）**：写死 `nearestByTag`（只锁最近，id 升序 tie-break）。`Perception` 只有 `targetTag`+`sightRadius`，**无策略字段**。
> **为什么不能重组绕过**：索敌选取规则在 aggro 内部硬编码，数据层无开关。
> **为什么值得下沉（通用性）**：「最近/最远/最高某属性/最低血」是普适索敌词汇（刺客/狙击/嘲讽/治疗锁友），一个 enum 参数化即可。
> **建议形态（窄）**：`Perception.policy?: 'nearest'|'farthest'|'highestStat'|'lowestHp'`（缺省 nearest，向后兼容）+ `spatial-query` 加对应变体（沿用 id tie-break 保确定性）。**不改语义、不动定序。**

---

### REQ-F-057 · [2026-06-10] · 策划 PF（「孙刘抗曹」合作立项 C0 前置，用户拍板；原编 054→056 两度撞号让位）· 框架级 · status: **降级 open（不再是多人前置；条件性保留）** · 优先级: 低（仅"共享战斗"路线才需要） · 类型: 验证探针（战斗跨端确定性）

> 🔻 **Lead 复评（2026-06-13，用户拍板转型）**：核心战斗 = **各人单独 PvE**，玩家间只传递**战局镜像 / 连携 / 卡牌**，**无共享战斗实例**。lockstep 那套"两端逐拍 hash 一致"已被**状态快照同步**取代（`src/net/state-sync.ts` 打包层 @8060cb9 + 会话层 @310e11e：各跑各世界、只搬运状态，天然容抖动、无硬 desync）。
> → **本探针不再是多人前置**，降级为条件性保留：**仅当未来真引入「盟友单位进同一场战斗实例厮杀」才需要**。「战斗浮点整数化」那笔大账因此**可暂不立**（转型把最硬的骨头绕过去了）。


> **要证的命题**：连续战斗 SIM（overlap 碰撞/aggro 距离/坐标投影中的浮点）在两个独立实例间**逐拍 hash 一致**。离散域（卡牌/经济/流程）已被 REQ-016/017 双端 lockstep 证过；HexPos 整数化（F-024/037）已把位置真相挪出浮点——剩这一根骨头。
> **建议形态（复用现成对拍设施）**：扩 `src/net/lockstep` 的既有 2 实例对拍：跑同一份 game-f 战斗蓝图（含 grid-move/滑行/hitbox/DoT/冰冻/大招全链）N=3000 拍，逐拍比 `world.hash()`；绿=联机地基宣告就绪；红=输出首个发散拍+组件 diff 定位。
> **失败退路（代价已知，不慌）**：战斗浮点整数化/定点化（成熟技术）；探针本身就是在给这笔账定价。
> **范围注**：纯测试/工具，不动引擎语义；浏览器同源 V8 的 IEEE-754 语义让风险偏低（REQ-F-034 sqrt 论证同源），但**要证不要赌**。详见 `docs/game-design/game-f-coop-sunliu.md` §四.3/§五 C0。
...
### [2026-06-10] · 用户（直发 Lead） · 引擎原生 · status: done · 资源库 v1.4：货架全量视觉语义层（沙盒内逐格扫描 4687 张）
- 需求：用沙盒里的模型本体把货架全部扫一遍、自动标注（不走外部 API）。
- 落地（§5.8）：19+1 并行分身受控词表逐格打标 → tags-vision.json（4687/4687 全覆盖，merge 校验+废词过滤）；途中修复 Adam7 隔行解码缺陷（曾致噪点），30 张受害图人工复审补标；三层标签（视觉语义/像素事实/人工精标）并入索引。
- token 优化（用户要求）：大拼图分组+增量写盘+禁自验+短汇报，单批消耗 11万→7.8万。
