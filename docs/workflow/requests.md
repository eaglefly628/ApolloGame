...
### REQ-ASSET-导入抠图 · 创作台导入时「背景移除→真 alpha」选项（选型 rembg） · [2026-07-16] · owner 拍板「搞起·用 rembg」→ **指派：PA（抠图能力 + 端点·✅ done）+ PST（创作台导入向导选项 UI）** · status: **PA ✅ done（PA 2026-07-16）；PST v1 ✅（勾选+模式选+导入前逐图 matte·2026-07-17）·余 before/after 预览 + tolerance/despill 控件（enhancement）** · 优先级: P1 · 类型: 资产线能力 + 创作台导入 UI（authoring-time·确定性/AI 二档·不碰 sim/hash）
> **PST v1 完工回执（2026-07-17·门禁 tsc0/vitest3242/build）**：`src/studio/AssetImportWizard.tsx` step4 加「🎯 背景移除 → 真 alpha（抠图）」勾选 + 模式选（flood 漫填 / rembg AI）；提交时导入前逐图过 `POST /api/assets/matte`、用抠好的真 alpha 图入库、provenance 记 matte 步；**任一图失败即整批中止**（绝不静默入没抠的原图）；sheet 模式不适用（不显勾选）。抽出纯函数 `matteImportFiles` 导出 + 单测 `src/studio/asset-matte-import.test.ts`（4 例：逐图替换/provenance 合并/失败中止/进度回调）。**余（enhancement·非阻塞）**：PA 回执要的 before/after 预览 + tolerance/despill 控件——待 owner 要再补（现走端点默认 tolerance；产物直接入库·未走 M2.5 pending·因导入向导本身不经 pending 门·若要人审门另议）。
...
### REQ-EVENTLOG-下沉共享事件日志原子 · game-b + game-c 各造一份 → 引擎缺 log/journal 件 · [2026-07-21] · PE-B 报（数据驱动 review 档 B·owner 追问「能用积木就用」纠出）→ Lead 裁决 · status: **✅ atom done（2026-07-23·Lead·`src/skills/tier1/event-log.ts` + 9 测·tsc0/全量绿）；迁移 open 落 B-013(PE-B)/C-115(PE-C)** · 优先级: P2 · 类型: 引擎原子下沉（DRY·跨游戏复用）
> **✅ atom 完工（Lead 2026-07-23）**：`src/skills/tier1/event-log.ts`——`EventLog<K, Extra>` 泛型类 + `createEventLog()` 工厂·`push`(自增 seq)/`recent(k=14)`/`all()`/`size()`/`clear()`/`dump(fmt?)`·kind 泛型闭集 + text + Extra(round/actor/tile…) 由消费方填·纯数据零随机不进 hash·9 测（seq 自增/recent 截断/dump 格式/泛型 Extra 透传/clear 归零）·自 `@skills/tier1/index.js` 导出。**迁移待接**：B-013(PE-B: game-b `GameLog`→薄封装)/C-115(PE-C: game-c `GameEvent`→同)。
> **⚖ Lead 裁决（2026-07-23·核过两款源码）**：✅ **下沉**——rule-of-two 属实（game-b `core/game-log.ts` `GameLog` 类 + game-c `game-log.ts` 各手写一份带 seq 的类型化事件流·core 骨架逐字近同：`push` 自增 seq / `recent(k)` / `all()` / `size()` / `clear()` / `dump()`），第三款卡牌必再造，宪法 §2「真缺口·可复用」成立；现无「流水日志」原子（`tier3/timeline`=演出时序调度·非日志）。
> **下沉形态**：**headless 泛型数据结构**（非 defineCapability ECS·同 `dice.ts` 纯 helper 先例）——`src/skills/tier1/event-log.ts`：`createEventLog<K extends string, Extra = {}>()`（或轻类）·API=`push(e: { round?: string; actor?: string; kind: K; text: string } & Extra)` 自增 seq / `recent(k=14)` / `all(): readonly LogEvent[]` / `size()` / `clear()` / `dump(): string`。**kind 泛型闭集 + text + 额外字段（如 game-b `tile?`）由消费游戏各填**·能力只管容器与 seq 骨架·**不碰文案/kind 语义**（正文恒机读口径）。确定性·零随机·零 IO·**不进 sim hash**（旁路观测·同现状）。
> **施工分工**：① 引擎 atom + 单测（push 自增/recent 截断/dump 格式/泛型 Extra 透传/clear 归零）=Lead 域·**派 Opus 施工中**；② 两游戏迁移消费（game-b `LogKind/LogEvent/GameLog`→薄封装泛型实例·保留自家 kind 闭集与 `tile?`；game-c 同）=**游戏域·落 B-013（PE-B）/ C-115（PE-C）·atom 落地后接**——Lead 不改游戏内部。
...
### REQ-SURVIVOR编排-幸存者三处编排能力归属（三选一 draft / 进化替换 / 波次刷怪）· [2026-07-23] · GD-103 报（owner 拍板「下沉新能力·交 Lead」）→ Lead 裁决 · status: **✅ done（2026-07-23）：2 下沉已建+测(draft-offer/spawn-director·Lead) + 2 回驳(重组·E2/E4)·game-103 可开工** · 优先级: P2 · 类型: 引擎 capability 下沉裁决（Roguelite 通用编排·先重组再下沉）
...
> - **E1 三选一 draft → ✅ 下沉 `draft-offer`**（真缺口·通用）。dice-roll 只加权抽·无「按持有武器/被动 + 槽位满否**过滤候选池** → offer N → 选中回填」。**Spec(→Opus)**：headless 纯数据·确定性种子化；`rollOffer(pool, state, {n, seed})`——pool=候选闭集(each: id/weight/eligibility 谓词=数据键)·state=已持有+槽位满标·过滤不合格(槽满/已满级/前置未满)→加权抽 N 个**不重复**→返回 offer 列；`applyPick(offerId, state)`回填。kind/资格规则=消费游戏填数据·能力只管容器。测试：过滤-按持有/N-不重复/同种子可复现/槽满排除/空池边角。放 `src/skills/tier2/draft-offer.ts`。
> - **E2 进化触发替换 → 🔁 回驳·重组**。条件门=`t2-event-when`（武器 lv5 且对应被动在场→edge Signal）✅；替换=Signal→`spawn`(进化模板)+退场旧武器（实体生命周期）。`t3-merge-rule` 是「N 个**同**模板→1」**计数触发**·不匹配「条件触发·单实体换模板」——别硬套 need:1。GD 照此摆数据（WEAPONS_EVO + event-when 条件树 + spawn/退场）。**若实测无干净「信号→退场旧实体」路径→回本单开薄 `evolve` 缺口**（暂判重组可解）。
> - **E3 波次刷怪 → ✅ 下沉 `spawn-director`**（薄缺口·通用）。`spawn` 单发·无 rate（每秒 N）/cap（同屏上限）/波表调度。**Spec(→Opus)**：headless 确定性；`director` 数据=波表(entries: {atTime, template, ratePerSec, cap, burst?})·state=经过时间+每模板存活数；`tickDirector(director, {now, aliveCounts, seed})`→返回**本 tick 该发的 spawn 列**（尊重 rate 累积 + cap 上限 + 波表时段）·消费现有 `spawn` 原子真生成。测试：限速累积/cap 拦截/波段切换/同种子可复现。放 `src/skills/tier2/spawn-director.ts`。
> - **E4 质变 flag → 🔁 回驳·重组·capgap**。homing=`t2-steering`+`t3-aggro`✅·fan=`Launch.amount>1`✅·pull=反向 steering（逐个走 capgap 快速通道·不占本单）。
> - **过审结论**：game-103 可开工 §1-3 数据面 + E2/E4 重组接线；**E1/E3 两下沉件落地前 game-103 相关 system 暂等**（Lead 派 Opus 施工·完成回填 registry + 本单结案）。
