...
### REQ-FLOWFIELD · 群体流场寻路能力 `t2-flow-field`（大规模 RTS 的寻路地基）· [2026-08-10] · **owner 拍板要做**（原话「我建议写流场寻路，我想做大规模 rts」「这个需求提给主程序来写，我们给一些最新技术的调研」）· **施工主体 = 主程 = 本 session（2026-08-24 抢锁·owner 当面指派「你就是主程」·本行即锁）** · 复查 = 另派独立 agent（复查人≠施工人红线·Lead 身份已因施工作废） · status: **in-progress（M1+软分离已复查毕·ORCA 首轮复查 FAIL 已全修·待再复查·M2/M3/M4 未做）** · 优先级: **P1（大规模 RTS 的前置地基·现有寻路实测撞墙）** · 类型: 引擎能力下沉（运动/寻路线）
...
**🛡 ORCA 硬避让（`2e9ea915` 交 → 独立复查判 **FAIL** → 六条实伤修完 · 待再复查）**
移植本体没打穿（LP1/2/3 与半平面构造逐段对得上原码·环形搜索与暴力最近 k 在 13 万次随机试验里逐位一致），
**六条全在算法之外，且没有一条是读代码读出来的**：① 「不设 orca = 一个字节不变」是假的——桶键换成几何键时
顺手改了**软分离**语义（只开 separation 的多场世界轨迹 hash 就变）→ 两套索引各建各的；② 完全同位 ⇒ `w/|w|`
得 NaN ⇒ **NaN 约束在 `det>0` 里恒假被静默丢弃** ⇒ 两个单位永不分开 → 退化分支按下标定左右；③ 混装单位类型
时强承诺静默失效（纯流场单位不进桶=隐形·实测对穿 0.10）→ 桶收全部单位 + 不还礼的 `u` 不打对折；④ 组件注释
把 `timeHorizon` 缺省写成 2（实为 8）；⑤ LP3 **零断言覆盖**且用例名与实测相反（整段删掉 53 测全绿）→ 判据改
「违反量 ≤ LP2 落点」；⑥ 环形提前退出的正确性零覆盖（撤掉距离校验 3.2% 查询拿错邻居而全绿）+ 三档开销零判据
（撤掉提前退出 5.75× 悬崖而 exit 0）→ 补暴力对照 + 量**访问格数**（23.8 → 撤掉 283.8）。
**九刀撤修验红刀刀命中锚点·空刀对照绿。** 判词与复现步骤全文：`docs/design/game211/orca-review-2026-08-25.md`，
修法与那条**被实测否掉的修法**（加宽邻域反而把对穿从 0.70004 压到 0.5423）见调研 §10.5。
`src/skills/tier2/orca.ts` = **逐行移植 RVO2 `Agent.cc`**（Apache-2.0·三件套齐：源文件 SPDX + `licenses/Apache-2.0.txt`
正文 + `THIRD-PARTY-NOTICES.md` 修改说明），接口 `FlowAgent.orca?: { radius, timeHorizon?, maxNeighbors? }`，
**不设 = 一个字节不变**（这句现在有点名用例咬着）。偏离七条写在文件头。
**接线坑**（实测撞出来的·非设计）：邻居桶按 `fieldId` 分 ⇒ 敌我互相看不见（0.10）→ 按几何键；固定 3×3 窗口装不下
前瞻邻域 → 按半径张开；全窗口扫 36.8ms → **环形搜索 + 第 k 近提前退出** 7.0ms；到点硬停 ⇒ 互惠破缺（0.33）→ 到点也跑 ORCA。
**三档开销**（同机·bench 三段）：纯流场 0.39/2.51 · 软分离 1.01/4.08 · ORCA **8.64/35.77** ms/tick（1000/4000 单位）。
