...
### REQ-CYCLEHAZ-既有定序成环隐患 · event-when+keybind+timeline+resource-apply 四能力共世界拓扑成环 · [2026-08-04] · REQ-MATRIXDUEL 施工探针实测带出（未触碰·如实上报） → 待 Lead 排期核查 · status: open · 优先级: P2（潜伏雷·现役游戏未踩到才没炸） · 类型: 引擎核定序卫生
> 实测：四件既有能力装进同一世界时 `topologicalSort` 抛 Circular dependency——现役游戏恰好没有四件同装所以未暴露；任何新游戏踩中组合即装载炸。核查方向：复现最小组合→定位环上哪条 runsBefore/After 申报过宽→改申报或拆相位（照 matrix-duel 两系统拆法判例）。修复=引擎核定序面·xhigh 不降档。
