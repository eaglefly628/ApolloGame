...
### REQ-G-Player-AI · [2026-06-21] · design G → 甲（引擎域·AI） · Game G · status: open · 优先级: **P0（与 Boss-AI 并列·解锁可信 sim）** · 类型: 真缺口（**owner 明确豁免数据驱动·单独开发**）· 规格: `design/player-ai-spec.md`
...
> **派甲（分步·先地基）**：
> 1. **改掷层解释器**（vision §2.2）：`applyRollMods(roll, mods)`（`mul/add` on 掷）+ 掷前决策窗口 + AI 子决策（§5）+ **§2.3 三道防爆炸护栏**（乘不叠/每场限张 N=2/掷后夹 `ROLL_CAP=60`）。**resolveClash 已留 TODO 插入点。**
> 2. **通用定向**：卡带 `target`（self-unit/enemy-hero/any-unit/lane）→ 引擎选目标 + 应用算子。
> 3. **与地煞合流建议**：改掷层做成**玩家专属牌 + Boss 地煞共用**的通用解释器（同 `{target,op,value}` 词汇）→ 一次实装两边都吃（与 `REQ-G-地煞原生战力重构` 合并考虑）。
> **依赖 design G/owner**：§七开放问题（对谁用/绑定英雄否/占不占loadout/操作窗口繁琐度）待 owner 拍板 → design G 定死卡结构 + 出首批英雄牌数据。**先实装改掷层地基·数据后填。**
