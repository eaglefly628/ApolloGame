...
### REQ-F-033 · [2026-06-10] · PE-F（接入 F-7/REQ-F-032 时暴露）· 框架级 · status: **done（'@local:' 显式标记重映射，采纳 PE-F 倾向的方案 B）** · 优先级: **高（F-7 回合重置接入的前置 → 顶替 F-032 成为 MVP-1 阻塞点）** · 类型: 真缺口（prefab 多实体模板的**内部实体引用**不重映射）
