...
### REQ-G-战斗逻辑批次 · [2026-06-21] · owner→甲（playtest 连发·战斗模型/AI/平衡·乙代登记） · status: **open（甲 active=#5；#1 暂缓待 owner 数据；#4 转交策划；#2/#3/#6 转交他人 — owner 2026-06-21 多轮分工）** · 类型: 战斗逻辑（非表现·甲域）
> owner 2026-06-21 深度 playtest 连发的一批**战斗逻辑/AI/平衡**需求——均属甲（turn-combat / 战斗驱动 / 平衡），乙代登记。乙只在甲落地钩子后接「表现」（全屏通知/fx）。
> **owner 2026-06-21 分工调整（多轮）**：#2 地煞可打 / #3 敌 AI 用地煞 / #6 敌用地煞全屏通知 → **转交其他 session**；#4 牌力概率反算 → **转交策划**；#1 敌方牌库镜像 → **暂缓**（owner 数据将出·出后甲直接接数据更新建库）；**甲当前只做 #5 敌回合逐步演出钩子（owner「你看一下怎么做」）**。
...
7. 🐞 **[BUG·甲·乙搜定根因 owner 2026-06-21]** **死战不退(lastStand)主将退格 → slot 碰撞 → 后方兵被画面吃掉**（playtest 报「我胜了但敌人没消失·它后面那格的人消失了·黑桃3 没消失」）：
   ...
   - 建议修：退格时**级联**把身后兵也依次后挤（保 slot 唯一·间距 1）/ 或退到**最近空格** / 或与身后兵**换位**——保证一格一兵。乙只诊断，落地交甲（determinism/hash 敏感·甲域）。
...
### PG-乙→甲 · [2026-06-21] · Game G · status: **open（转交甲·owner 拍板"留给甲砸"·owner 2026-06-21：「全部做完以后再做一次」=排到最后）** · 类型: 战斗段死代码清理
