...
### REQ-G-战场UI批次 · [2026-06-21] · owner→game-g 乙（甲代登记·战场屏 owner 授权乙动）· status: **open** · 类型: 表现层一批（playtest 连发）
...
> 2. **每张牌开销=源泉滴数**：牌上把 cost 画成水滴（1/2/3 滴·0 不画）。✅ **甲已落经济**：放牌按 rank 收 0/1/2/3（owner 拍板）·`PokerCard.cost` 已上卡 + `buildTurnBattleView` 已读 `c.cost`，现有右上 `★N` 角标已显示**真费用** → **乙只需把 `★N` 角标改画成 N 颗水滴**（costPill 在 `turn-battle-screen.ts` handCard·gang 牌用 CAST_COST 同理）。
> 3. **牌库剩余**：显示我方牌库还剩多少张可抽（抽牌台/牌堆角标·读 `tb.a.pokerDeck.length`/`tengangDeck.length`）。
...
> 8. **掷骰子动态表现**（owner 2026-06-21）：掷命对决特写里加**命运骰滚动动画**——揭晓时一颗骰子翻滚 → 落定在掷点上 → 与胜率门槛比对定生死。**数据已就绪零依赖**：`ClashEvent` 已含 `roll`(0~1) / `winrate`(0~1) / `aWins`，乙直接读：骰子落点 = `roll*100`，门槛 = `winrate*100`，`roll<winrate`→正面(我胜)否则反面。建议表现：骰子滚动 0.6~1s → 停在掷点数字 → 高亮"≤胜率=活/＞=亡"，配 clashReveal 音（已有）。纯表现·不改判定。
>
> 甲并行做对应**战斗逻辑**（弃牌返源泉+不互斥 / 战胜牌回库+返还 / 放置不可重叠 / 回合流程改同步推进），落地后给乙数据/钩子；乙只管战场屏表现。
