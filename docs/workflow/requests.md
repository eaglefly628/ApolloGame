...
### REQ-PKG-数据游戏打成独立可运行包·引擎「从内联 manifest 启动」钩子 · [2026-07-12] · owner（workshop 发布屏「打包下载=直接可运行文件」）→ PST 建流程撞到真缺口 · status: **open（真缺口·PST 已接流程+端点+内置样例真打包·此钩子未落地=生成的卡带只能出工程 zip）** · 优先级: P1（owner 主诉：要能打包"我生成的游戏"·非样例） · 类型: 能力下沉候选（发布线×引擎·game-publisher/Lead 域）
