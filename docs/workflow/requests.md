...
> 8. **掷命=扔 10 颗十面骰·3D 物理表现** ✅ **甲已做（owner 改派·乙在拆文件）**：新 `dice-roll.ts`（独立浮层·CSS-3D 翻滚骰 + 点数跳进度条 + 门槛线 + 投掷/继续）+ 驱动 `playPerf` 挂载——**没碰 `turn-battle-screen.ts`**（浮层盖在特写上·继续=perfResume），读 `clashDiceRoll` 既定结果。**乙不用再做这条。** 原需求：点【投掷】→ 10 颗 d10 3D 滚 → 点数跳进度条 → 冲破门槛(胜率低门槛高)。
