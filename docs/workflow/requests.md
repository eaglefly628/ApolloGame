...
> **M1 ✅ done（PUI·2026-08-04·待 Lead 对抗性验收）**：catalog 闭集新增三控件 `dialog`/`choiceList`/`portrait`（沿 panelTexture 先例·types+render+catalog describe+validate 自动认+点名测试+ui.md 回填+house 主题取色可皮）。**投影读世界**=另立结构投影器 `resolveDialogue(tree, DialogueSource)`（bindings.ts·DI 接口·ui 不 import @skills）——因标量 UIDataSource 表达不了「变长选项+逐项 optionAvailable」这类结构投影；写世界=`dialogue.advance`/`dialogue.choose`+arg 下标（t3-dialogue 已认 arg 串·零游戏 handler）。**+ 华丽货架接线（owner「对话 UI 多样化美术」）**：三控件透传既有闭集令牌——choiceList 选项渲成真 Button（house `buttonSkins` 糖果厚唇皮 + optionKind/optionShape/hoverSheen）·dialog `skin`/`shape`/`edge`·portrait `shape`/`edge`/`glow`（非新写美术）；顺清 REQ-STYLESET border-image 盲区（皮钮自动带 `data-audit-skip-contrast`）。**+ Lead 整改 ✅（真跑）**：展台从 literal 静态摆拍改为**真起 dialogueCapability 世界**（`games/game-i/dialogue-world.ts`·dialogue+resource 双能力·宿主 tick）——点台词框真发 DialogueAdvance、点选项真发 DialogueChoose+下标 → 世界推进 → `resolveDialogue` 每 tick 重投影（真跑三控件·好感 8→14 真涨·握手项暖场后真解锁·三态截图在案）。**退役令达成**：`ui/vn` React 五件已删（无消费方·防双轨）。守卫：`dialogue.test.ts`(8)+`dialogue-world.test.ts`(6·真跑循环)+`dialogue-demo.test.ts`(3)+catalog-coverage 41 型·audit 0 阻断·scoped-gate 全绿。**Lead 评判**：M1 真闭集缺口（游戏代码建选项列=「代码重建 UI 树」反模式）·PUI 独立复核同判接受。
...
> **⚖ Lead 终审 R3（2026-08-05）：✅ PASS**（`0e4937e11`·随本提交一并推送）——独立复跑 25 测绿·亲跑 game-e compare 退出码 0·基准照/台账在档。亮点照准：稳帧判定（连续两帧同哈希）+ 抖动如实标 flaky 绝不静默放松阈值；真漂移演示在案（改点色→ratio 0.59% 判红+diff 可视图→还原转绿）；共享件抽 `scripts/lib/render-harness.mjs` 供 R1/R3 复用（防漂移正解）。首批基准 bless 署 auto-bootstrap 如实标机器引导·真人门后续换签。**其押推纪律=模范**：被超槽守卫拦下即停手上报不硬推不擅清池。
