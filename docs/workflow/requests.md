...
> **边界**：①属主程/引擎（`src/skills/tier2/steering.ts` 或新 2D collision 件）；②属 renderer/P3D（`src/renderer/**`）。game-103 只经数据/组件消费。PE 侧临时降同屏 cap（spawn-director）缓解、非根治。撞墙实证=`docs/design/game-103/requests.md BUG-02/05`。
