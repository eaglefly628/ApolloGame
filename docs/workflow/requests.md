...
### REQ-DEMO-T1-美术替换工作流·产线段 · placeholder 先行+替换列表推导+批量生成+对位替换 · [2026-07-07·07-08 owner 定形两段式] · Lead 图纸=`docs/design/art-replacement-workflow.md`（**唯一权威·本条只摘要**） → **指派：PST（施工）·PA 会审契约（风格包库/台账 schema/缓存键）** · status: **产线段 ①-⑥⑧ ✅ done（PST 2026-07-08·门禁全绿·全链冒烟 20/20）；⑦进度灯 UI 随心跳余项、台账浏览墙=T2** · 优先级: **P0** · 期限: 7/13 · 类型: 产品化·美术管线
...
### REQ-DEMO-T2-换皮流水线+台账浏览/单槽替换 · 同玩法×新风格锚=新卡带；按编号点名优化 · [2026-07-07] · Lead 图纸 → **指派：PST** · status: **✅ done（PST 2026-07-08·门禁全绿·冒烟 33/33·待 Lead 真浏览器验收）** · 优先级: P0 · 期限: 7/18 · 类型: 产品化·量产乘法器+优化闭环
...
### REQ-DEMO-R1-平台归一·主干扩建与真调打通 · 择一方法：主干=台账墙线 · [2026-07-09·owner「重新整理·各归各线·先清理再重写或择一」] · Lead 归一裁决 → **指派：PST（主干骨架）** · status: **open（冲刺 P0·最高优先——mock 死穴不除 demo 出不了真图）** · 期限: 7/13 · 类型: 整改·产品化
> **⓪ 归一裁决（Lead 2026-07-09·终审）**：两套美术 UI 并存（cockpit=game-q 硬编码·每行生成 vs ArtLedgerPanel=slug 参数化·风格包/换皮/三式）——**择台账墙线为唯一主干**。理由：参数化、接了风格包与 dialectPrompt、有 parseManifest 门与 33 断言 smoke；cockpit prompt=裸实体名且服务端永远 mock，重写成本低的一侧是它。cockpit **独有件**（编译期游戏数据源、活场景预览、生成→人审→填回链）由其作者整理并入主干后 cockpit 退役（见 R3）。**canonical 行 schema=deriveRequirements 结构**（no/kind/slot/query/placeholder/spec/context/status/gen/provenance·工作流档 §三），`deriveLedger` 改产同构行（placeholder.source 区分 freelib/procedural），status 枚举合并 `needs-art|placeholder→queued→generated→filled|replaced→approved`——平台只读 canonical 一种，谁也不用适配两套。
> **① 主干扩成通用「美术资产平台」（PST）**：ArtLedgerPanel 升级为双数据源——library 卡带（manifest 线·现有）+ 编译期游戏（本地 index+台账线·吸收 R3 移交件）；入口统一资源控制台（🔬/🎨 收敛为一）；逐行生成/预览/填回只存一套实现。
> **② 真调打通（demo 死穴）**：(a) `/api/assets/generate` 删无条件 `--mock`（apollo.py:1443）——mock 只在显式 env 或前端显式勾选，绝不静默顶替；(b) ArtLedgerPanel `doRegen/doReskin` 删写死 `mock:true`（:85/:87）；(c) SettingsPanel 加「生成 key」区（DASHSCOPE/TRIPO/MESHY → .apollo-config.json·打码显示）；(d) apollo.py `_spawn` 把 config key 注入生成子进程 env（key 绝不落日志·探针纪律照旧）。
> **③ 风格与粒度**：平台两种数据源都接风格包+`dialectPrompt` 自动拼提示词；**一键全量**与单行两种粒度；style-packs 从 `scripts/style-packs.mjs` 代码常量迁**数据文件**（扩包不改代码）；refImage 参考图配置实现或记 blocker。
> **④ 人审姿态统一（按 owner 07-07 归置闭环）**：单行生成=预览→「入库/弃置」确认（即人审）；批量/换皮=快速通道直落+unreviewed 标+台账可见；进**共享货架**仍必须 M2.5 pending。upload 端点加内容嗅探（magic bytes）。
> **⑤ 卫生（主干侧）**：`scripts/art-replace.mjs` 两个真 NUL 字节改 `'\u0000'` 转义（git 现把它当二进制·diff/blame 全瞎）；`deriveLedger` 出 canonical 行；needs-fill 全链自动化测试（现零测试·端点并入主干后补）。
> **⑤bis 编号 append-only（game-q 程序员上报·owner 点名「考虑 ID 错位」·实缺陷）**：现 `deriveRequirements` 按排序整批分配 no——游戏加/删实体后重跑**序号整体挪位**，与工作流档 §三/§六「重跑不漂移·只追加不重排旧号」相悖（写回不受影响·它认 slot；但人念的编号会串）。修法=派号时 diff 现有台账：**已有 slot 保留原 no；新 slot 取 max+1 顺延；删除的 slot 行标 status:'retired' 保号不复用**（墓碑·编号永不回收）。补点名测试：加实体重跑→旧号全不动新号顺延；删实体重跑→墓碑保号。仍是**单文件合一**——不加第二个文件。
> **⑥ 验收**：门禁全绿；Lead 真浏览器旅程=资源控制台→选游戏（编译期+卡带各一）→选风格包→一键全量（mock 显式态）→单行生成（有 key 则真调）→填回台账。完工标 ✅。

### REQ-DEMO-R2-game-q 皮肤槽接线与需求真相合一 · 写回最后一环 · [2026-07-09] · Lead 审计裁决（owner「开两个单子落地」）→ **指派：game-q 侧程序员** · status: **open（冲刺 P0——没有皮肤槽·填回的 ID 上不了画面·闭环断头）** · 期限: 7/13 · 类型: 整改·游戏侧接线
> **① 皮肤槽接线（owner 点题「不然怎么写回替换美术 ID」）**：blueprint 现**零** Sprite/art 槽位（grep 零命中）——`docs/design/game-q/asset-requirements.md` 自己写的接线法从未实施，台账 filled 的资产 id 上不了画面。改：对照台账 31 行给可换皮实体（塔/敌/大本营/pad/背景…）加 Sprite 皮肤槽；无资产时优雅回退现程序化观感（不许变裸）；填回后重载游戏资产真上画面。
> **② 需求真相合一**：`docs/design/game-q/art-list.md` 手拼提示词用私有 ID（q-spr-NN）与台账 art-NN **零映射**、规格已漂移（md 64×128 vs 台账 56×120）——精心拼的提示词管线够不着。改：**先**把提示词回填台账行 prompt 字段（机器真相·手拼的活别丢），**然后**两份 md（art-list/asset-requirements）删除或降级只读视图（头注「真相=art-ledger.json」）——删是合法终态，回填完成前不许删。
> **③ 删过期双真相**：`public/games/game-q/art/game-q-art-ledger.json`（3D 版 23 行·提交当刻即与已回退 2D 的游戏矛盾）删除（配合 R1 删回退读写路径）。
> **④ 测试与验收**：mock 全链走查（台账行→needs-fill 填回→重载→占位图上画面·自动断言）；audit 无新红旗；门禁全绿。完工标 ✅ 待 Lead 验收。
> **⑤ schema 口径（归一裁决）**：你们的 `deriveRequirements` 行结构就是 canonical（R1 ⓪）——继续照它产 `art-ledger.json`，别改结构；提示词回填就加行内 prompt 字段（canonical 已含）。**写回步是你的活**（blueprint 皮肤槽=①），cockpit 作者已按裁决停在填台账 ID，不会碰你的 blueprint。

### REQ-DEMO-R3-cockpit 清理移交·独有件并入主干 · [2026-07-09] · Lead 归一裁决（owner「先清理掉该清理的东西」）→ **指派：cockpit/透视器程序员** · status: open（冲刺 P0·与 R1 协同） · 期限: 7/13 · 类型: 整改·移交
> 背景：归一裁决（R1 ⓪）主干=台账墙线，cockpit 退役。你提的三问答复：①schema=deriveRequirements 结构为 canonical，不用适配两套；②并存裁决=本单+R1，协调 note 采纳（写进本单交接段）；③**写回步停在填台账 ID**——blueprint 钉回是 game-q 程序员的活（R2 ①），勿越域。
> **① 清理**：删 `public/games/game-q/art/game-q-art-ledger.json`（过期 3D 版·与已回退 2D 的游戏矛盾）+ 删 apollo.py/cockpit 对它的回退读写路径；「3D 盒庭/ThreeRenderer」过期文案清理。
> **② 独有件整理移交（并入主干·与 PST 协同）**：三件打包成可复用模块+一页交接 note——(a) 编译期游戏数据源适配（读本地 index+台账·无 manifest 线）；(b) 活场景预览组件（选行→游戏画面定位）；(c) 生成→人审→填回链（`/api/assets/generate`+`/api/assets/review`+`/api/art/needs-fill`·M2.5 衔接是三件里唯一合规的人审姿态，保留语义并入 R1 ④ 统一版）。
> **③ 退役**：主干双数据源跑通后（R1 ①验收过），删 GameQArtCockpit 入口与组件（git 留档）；退役前 cockpit 冻结不加新功能。
> **④ 验收**：门禁全绿；交接 note 一页在 `docs/workflow/finish/` 落档；Lead 核主干上「编译期游戏线」功能等价（生成→确认→填回全链在主干可走通）。完工标 ✅。
