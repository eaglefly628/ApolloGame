...
### REQ-PIPEHASH-03 · **自我失效环第三次**：`self-check/` 与 `review/` 也是门证产物，却在内容指纹里 · [2026-08-09] · game108 S7 实测 · status: open · 优先级: P2 · 类型: 工具修（管线）· **归属 🔴 主程**（`game-pipeline.mjs` = 跨游戏共享面·拿不准按 🔴 走）
> **实测复现**（脚本探针·非推断）：只往 `docs/design/game108/self-check/S7-scorecard-selfassess.md` 追加一行注释——
> ```
> 原始         22c48e2e7e45dc42
> 改自证单之后 86239a99a9838671  ← 变了
> 改回         22c48e2e7e45dc42  （还原 ✓）
> ```
> 后果：**写一份「证据文档」就把 S3/S4/S5 的机器门全判成过期**，看板当场三个 ⚠。
> 本轮就是这么发生的——我最后一笔只加了 S7 自评单，一行游戏代码没动，三门齐灭。
>
> **这是同一个形状的第三次**（所以它该被当成规律而不是个案）：
> ① `probe/` 门禁自产证据入指纹（我修·2026-08-08）
> ② `golden/` 标准照基准入指纹（主程修·`eb36e7ef`「门证产物不入内容指纹·自我失效环」）
> ③ **本条**：`self-check/`（自证对齐单）与 `review/`（复查导航单）——同样是**门产出的证据**，不是游戏内容。
>
> **判据一句话**：指纹该问「**游戏变了没有**」，而 `probe/`·`golden/`·`self-check/`·`review/` 回答的是
> 「**门跑过没有**」。把答案塞进问题里，门就永远在自我失效。
> **要什么**：`gameHash` 的目录排除从 `mock|probe|golden` 扩到含 `self-check|review`（`scripts/game-pipeline.mjs:90` 一行）。
> ⚠ **`gdd.md` / `capability-plan.md` / `acceptance/` 必须继续入指纹**——它们是**规格**，改了就该重验。
> 只排「门自己产的证据」，不排「门要验的东西」。
