...
### REQ-UI-emoji图渲 · 文本里的 emoji 自动渲成库里美术图（Twemoji·render-only） · [2026-07-16] · owner「emoji 直接用库里美术 emoji 替换·别逐个手转槽」→ PA 出映射底座 → **指派：PUI（UI 库渲染线域）** · status: **✅ done（PUI 2026-07-16·渲染层落地·待 Lead 真浏览器验收 + PA 会审映射接线）** · 优先级: P1（game-g 456 处 + 全线通用·省掉逐处 icon 槽手转） · 类型: 引擎 UI 库能力（render-only）
