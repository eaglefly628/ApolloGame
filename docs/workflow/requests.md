...
> 8. **掷命=扔 10 颗十面骰·3D 物理表现**（owner 2026-06-21 升级·要"仪式感"）：把对决特写做成——玩家**点【投掷】** → **10 颗 d10 物理 3D 滚落**（JS 渲染·真实物理感）→ 落定后**点数一粒一粒跳进一条进度条**累加 → 看总点**是否冲破"需要的门槛线"**：**胜率低→门槛高(要扔很高才赢·如90)·胜率高→门槛低(轻松超过·如6)**。
>    - ✅ **甲已提供一致性数据接口**（在 `turn-combat.ts`）：`clashDiceRoll(roll, winrate, aWins) → { dice:number[10](每颗0-9), sum, threshold, win }`。**乙直接调它拿数渲染即可**：`dice`=10 粒落点（逐粒跳进度条）、`sum`=总点、`threshold`=门槛线、`win`=是否冲破。
>    - ⚠️ **关键(已在 helper 内保证)**：`sum>threshold` 恒等于**实际胜负**（含平局裁定·**绝不重新 RNG**）——3D 骰只是把既定结果演出来，**永不与真实胜负矛盾**(确定性/回放安全)。乙**别自己再 roll**，落点用 `dice`。
>    - 数据源：`ClashEvent.roll`(0~1)/`winrate`(0~1)/`aWins` → 喂 `clashDiceRoll`。配已有 `clashReveal` 音 + 冲破/未破各一种判定音。
