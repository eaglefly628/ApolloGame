...
> 8. **掷命骰** · 甲做 · status: **❌ 旧方案(10颗d10浮层)owner 否决 → 已回退** · 新方案待 owner 确认结算公式
>    - **否决理由**（owner 2026-06-21）：① 全屏浮层**盖住了原战力明细特写**（玩家要看 战力 怎么算的）；② 骰子是**反推安排**的（`sum` 对齐既定 aWins）→「明显不是随机、是算出来的·太假」。已删 `dice-roll.ts` + `clashDiceRoll`，`playPerf` 回退到原特写。
>    - **新方案**（owner 口述·甲实现·这是**战斗逻辑+特写**改动）：保留**原战力明细特写**；特写里**两个投掷钮**（我方一个·在我侧下方 / AI 一个·在敌侧下方）→ 各掷**一颗真随机 d6(1~6)**（种子化·非反推）→ 该骰点数**加到本方胜率上** → 动画演示是否**冲破"需要的点数"**。⚠️ 这是 **real random**·改了掷命结算（不再单一 `roll<winrate`）·**结算公式待 owner 拍板**（两骰如何合成胜负·见甲提问）。
...
### REQ-G-战斗结构 · [2026-06-21] · design G → 甲（引擎域） · Game G · status: **open（★ owner 已拍板「人头留场续攻」· 待甲实现）** · 优先级: **P0（阻塞全部数值平衡）** · 类型: 真缺口（结构性·非数值）
