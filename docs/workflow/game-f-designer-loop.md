...
> | 字段 | 值 |
> |---|---|
> | **持棒方 BATON** | 🟢 **程序F**(提交版优化:战利品自动拾取+概率掉落;新核心案已存档候提交后启动) |
> | **状态** | owner 要先提交一版(当前自走棋基础上做优化)·策划F 派提交版 loot 优化·新核心案归档待启动 |
> | **上一手(策划F)完成** | owner 定节奏:**先提交一版(当前自走棋只做手感优化)**,新核心战斗(双人合作)**提交后再启动**。策划F:① 落档 **`docs/game-design/game-f-coop-combat-spec.md`**(双人合作·共享战线·3常驻+1救火·续航轮换·互动即得分·每波KPI竞合·镜像同步·先单人+AI验玩法;**标 ⏸ 提交后启动、暂不实现**);② 出**提交版优化派单**(下)。 |
> | **轮到谁 · 需做什么** | **程序F · 提交版手感优化**(当前自走棋基础,owner 拉动):**① 战利品自动拾取**——删"主公满地捡 orb"摩擦,敌死掉落**自动飞进物品栏/物品计数**(已核:combat.ts:360-361/465-466 eorb `Hitbox{items,-1,BAG}` 靠拾取;改成击杀即入袋,或 orb 自动归集);**② 概率掉落**——不再每个敌人必掉,改少数/按概率掉(⚠️若在 sim 内决定需确定性:优先**按单位类型确定性掉**[精英/名将掉、杂兵不掉]或种子RNG,**勿用 Math.random 入 sim**;掉落染色/depth 已有)。**纯游戏侧、零引擎、全绿才推。** 干完翻 🔵策划F 验收。 |
> | **唤醒/检查条件** | 下个 `src/games/game-f/` commit;或 owner 指定更多提交版优化项 |
> | **最后更新** | 2026-06-17 · by 策划F · 循环#50(新核心案归档+派提交版loot优化) |
