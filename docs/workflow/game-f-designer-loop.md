...
## 🪧 当前轮次状态 · BATON(owner 2026-06-17 立:双方共维护,**动手前先读、干完必更新**)

> **协议(铁律)**:① 任一方干完活,必须回这里**翻棒**给对方 + 写「我完成了什么 / 轮到谁 / 对方需做什么 / 唤醒条件」。② **禁止空悬挂**(不许干完就挂个永久 watcher 等超时——换一次性有界计时器,到点回来读棒)。③ 对方持棒时,本方可干自己不冲突的活,但棒归属要清。
>
> | 字段 | 值 |
> |---|---|
> | **持棒方 BATON** | 🟢 **程序F**(提交版打磨清单 P1–P4;新核心案已存档候提交后启动) |
> | **状态** | 程序F 推进中:✅P1(验收 PASS) ✅P2(验收 PASS) ✅P3(验收 PASS) · ⬜P4 续做 |
> | **上一手(程序F)完成** | ✅ P1(自动拾取+概率掉落,验收 PASS) ✅ P2(新手教程页,验收 PASS) ✅ **P3 UI 清理**：大厅顶栏写死假货币(🪙金1.2k/🎟券30/💎钻88)→ 换**真值** ✨尘 + 🃏收藏(战功本就真)；抽卡/附魔/分解后顶栏 refreshWf/refreshDust/refreshColl 同步真值。真机 顶栏=「战功0·尘0·收藏0」无假货币、零报错、1449 绿。**P4 续做(本棒仍程序F)；HUD 滚动槽/攻岛条/tooltip 一致性 + 市场页 💎 stub 归 P4 真机随 owner 试。** |
> | **轮到谁 · 需做什么** | **程序F · 提交版打磨清单**(当前自走棋,按优先级,各切片配测,**纯游戏侧零引擎全绿才推**):<br>**P1 战利品自动拾取**——删"主公捡 orb"摩擦→敌死**击杀即入袋/自动归集**(已核 combat.ts:360-361/465-466 eorb `Hitbox{items,-1,BAG}`)。<br>**P1 概率掉落**——不再每敌必掉:**按单位类型确定性掉**(精英/名将掉、杂兵不掉)或种子RNG;**⚠勿 Math.random 入 sim**。<br>**P2 接入新手教程页**——owner 已 approve:`game-f-tutorial.html` 接进启动「新手教程」按钮(纯前端弹层;**不含分步引导**=大、留提交后)。<br>**P3 UI/视觉清理**——大厅写死假货币("🪙金1.2k/🎟券30/💎钻88")清掉或接真值;HUD 可读性(滚动槽/贡献·攻岛条/品级 tooltip 一致)。<br>**P4 战斗手感微调**(真机相关,数值待 owner 试,先给合理默认):锦囊冷却/充能反馈、W1–W6 难度曲线、击杀反馈。<br>**P3.5(顺手·提交版体面)**:**市场/商城 tab 是 phase3 stub 却显假 💎 价**(lobby.tsx:210-212)→ 提交版**给个「敬请期待/即将开放」遮罩或隐藏该 tab**,别露半成品假数据。<br>干完翻 🔵策划F 验收。 |
> | **唤醒/检查条件** | 下个 `src/games/game-f/` commit;或 owner 追加提交版项 |
> | **最后更新** | 2026-06-18 · by 策划F · 循环#53(验收 P3 PASS + 加 P3.5 市场 stub 遮罩;棒续程序F P4) |
...
## 循环 #53 · 2026-06-18 · Designer F —— ✅ 验收提交版 P3(大厅假货币清理) PASS + 加 P3.5(市场 stub 遮罩)

验收 `2b43bec`:大厅顶栏写死假货币(🪙1.2k/🎟30/💎88)→ 接真值 ✨尘/🃏收藏(战功本就真),抽卡/附魔后同步刷新。**game-f tsc clean + 131 绿 + build 通**。PASS。<br>📌 加 **P3.5**:市场/商城 tab 仍是 phase3 stub 显假 💎 价(lobby.tsx:210-212)→ 提交版给「即将开放」遮罩/隐藏,别露假数据半成品。棒仍 🟢程序F 续 P4 + P3.5。

---

## 循环 #52 · 2026-06-18 · Designer F —— ✅ 验收提交版 P2(接入新手教程页) PASS

验收 `20b5d1c`:lobby「📖 新手教程」按钮 → 弹层 `<iframe srcdoc>` 内联 `game-f-tutorial.html`(Vite `?raw` 导入 + global.d.ts 声明,srcdoc 样式隔离;✕/点遮罩关;无分步引导=留提交后)。**game-f tsc clean + 131 绿 + build 通(?raw 入包)**。PASS。棒仍 🟢程序F 续 P3(UI 清理)/P4(手感)。

---

## 循环 #51 · 2026-06-18 · Designer F —— ✅ 验收提交版 P1(自动拾取+概率掉落) PASS

验收 `58631bd`:① 自动拾取=item_bag 改全盘静态收集区(隐形覆盖 ARENA)→敌死任意格 orb 自动归集,删主公捡 orb 摩擦;② 概率掉落=mob_death 拆名将掉/杂兵 bare,按 `unit.seg` **确定性**选(零 RNG、不入 sim,确认 combat/blueprint 无 Math.random)。**game-f tsc clean + 131 绿 + frame golden/快照更新**。PASS。棒仍 🟢程序F 续 P2(接教程页)/P3(UI 清理)/P4(手感)。

---

## 循环 #50 · 2026-06-17 · Designer F —— 新核心战斗案归档(双人合作,提交后启动) + 派提交版 loot 优化

> owner 定节奏:**先提交一版**(当前自走棋基础上做手感优化),新核心(双人合作)**提交后再启动**。

### A. 新核心战斗案落档(⏸ 提交后启动,暂不实现)
经多轮 owner 讨论收敛,落 **`docs/game-design/game-f-coop-combat-spec.md`**:换核=**双人合作·共享小战线·每人3常驻+1救火技·续航轮换深板凳·互动即得分(集火/连携/救场/供能)·每波KPI(衡量打得巧非战力)较劲·累计争岛主·镜像同步·先单人+AI验玩法再上联机·三国/小丑牌/装备/养成/太阁内容全留用重映射**。病根=现自走棋是确定性滚雪球(被动+无竞争+无悬念),owner"金铲铲不爱玩"。**owner 未说开始前不实现。**

### B. 派提交版手感优化(当前自走棋,Program F)
- **① 战利品自动拾取**:删主公捡 orb 摩擦 → 击杀即入袋/自动归集。
- **② 概率掉落**:不再必掉,按类型确定性掉(精英/名将掉)或种子RNG,**勿 Math.random 入 sim**。
- 纯游戏侧零引擎全绿才推。

### C. 待 owner
提交版还想优化啥?(教程页接入按钮 / 别的手感)——策划F 已问。
...
### 验收(本轮无 Program F 新装备提交 —— rate-limited,HEAD game-f 仍是 spec)
- 暂无可验收的实现;不空等。Designer F 主动消除其落地阻塞(见下)。

### 派 Program F(在 §七 数据清单基础上实现,免再设计)
- **决策锁**:命名传说 = **50**(§三 36 + §七.5 补 14);程序化变体凑满 600+。owner 50-vs-80 之议 → 锁 50(够记忆点,余靠程序化;不再问)。
- **spec 新增 §七 可誊写清单**:7.1 BASES(46 件含确定 base stats)/7.2 品级前缀+倍率/7.3 AFFIXES(12 含确定 delta)/7.4 `expandItems()` 确定性展开算法(纯函数,hash 不破)/7.5 UNIQUES 50 件。
- **落地序不变**:①数据库(用 expandItems 达 600+)→②tooltip→③拖拽装备→④拆解。**全绿才推。** 抢到窗口即按 §七 誊写,零设计决策。
...
- 万箭齐发(点地范围真伤 amount30 TEAM_B)/妙手回春(点地负伤=回血 TEAM_A,石田治疗同算子)。
- 👍 **空城计巧解**:我担心"我方减伤无读者",Program F **用 `dmg_scale_b`(敌伤系数,mob hitbox 已读)写负值**=敌伤-20% → 零新机制重组;还把 buff 泛化加 `buffTarget` 字段。范本级"先重组"。
- **零引擎、103 测绿**。主动锦囊现 **6 张**(鼓舞/火烧/定身/万箭/回春/空城计)。

### 下一步:疑兵增援(最后一张 catalog)+ 卡牌可见性线收口
- **疑兵增援**(self/pointer):`caster spawn ash_yari×2`(TEAM_A 友军,= 太阁召援镜像我方版)。最后一张,照填。
- 收口后:**owner「看不到牌/少操作感」反馈 = 完全解决**(P0 可见 + 小丑牌被动可见 + 7 锦囊主动点放)。
- 回 polish 队列:AI 难度档 / 掷点手感 / 锦囊平衡数值(待 owner 真机调)。
...
- **片5**:建 `data/game-f.manifest.json`(blueprint 生成器残余展平为字面实体);核账:脉冲=0(已)、非测试行数大降、对照纯数据基线。
