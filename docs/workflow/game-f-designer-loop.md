...
> | 字段 | 值 |
> |---|---|
> | **持棒方 BATON** | 🟣 **owner 共同设计中**(★ owner 钦定 C1 大掉头:核心战斗重做=三路概率卡牌对决) |
> | **状态** | 重大方向变更·策划 F 已出核心战斗策划案 v0·候 owner 锁最后决策+策划F 做引擎评估 → 再派 |
> | **上一手(策划F)完成** | ✅ 与 owner 完成核心玩法设计讨论 → **`docs/game-design/game-f-core-combat-spec.md` v0 落案**:点数牌(公平/续航/三路行进)+小丑牌(收集/功能/不行进)；buff聚合→clamped logistic 概率→**种子骰**→战线接力推进→破大本营**3血(皇室战争)**；输→本局弃牌、赢→沉牌底+回合冷却+续航(数字1/人头2/小丑3)；胜率可读+单挑特写=支柱；仿真台规格。**装备/附魔/羁绊 将从喂 atk/hp 重映射到喂 点数/buff。** |
> | **轮到谁 · 需做什么** | **⏸ 候 owner + 策划F**:① owner 拍最后结构问 **小丑牌怎么用**(被动槽/主动出牌/两者,我荐两者) + 微调点(续航/缝宽/回合上限);② 策划F 做**引擎评估**(spec §九:绝大多数重组=grid-move行军/card-pile/Timer;**唯两项候下沉**=in-sim 确定性种子RNG + clash-resolve 解算原子,逐项判能否重组,真缺才提主程 REQ)。**这是大重构,锁案+评估前不派 Program F 动手**(避免做了再推翻)。Program F 当前仍可静默/无悔 prep。 |
> | **唤醒/检查条件** | owner 拍小丑牌用法/微调;或策划F 完成引擎评估出实现规格;或主程/Program F 动 game-f |
> | **最后更新** | 2026-06-17 · by 策划F · 循环#49(C1 核心战斗重做 spec v0) |

> 翻棒写法:程序F 干完 → 把「持棒方」改 🔵**策划F**、状态「程序已完成待策划验收」、填「完成什么」;策划F 验收完 → 改回 🟢**程序F**;**双方均无无悔活时 → 🟡 待 owner**;**owner 在共同设计大方向时 → 🟣 owner 共同设计中**(暂不派实现,锁案后再翻 🟢)。

---

## 循环 #49 · 2026-06-17 · Designer F —— ★ owner 钦定 C1 大掉头:核心战斗重做(三路概率卡牌对决) + spec v0

### owner 设计讨论结论(多轮收敛)
game-f **核心战斗掉头**:退役金铲铲 hitbox 自走棋 → **三路布阵 + buff 聚合→概率→种子骰→战线推进攻大本营**。
- **两类牌**:点数牌(唯一上场战斗,公平同分布,续航数字1/人头2)+ 小丑牌(不上场,Balatro 功能/收集牌,养成灵魂,可延续航到3)。
- **对决**:`P_eff=clamp(点数+Σbuff)` → `Winrate=clamp(logistic(ΔP/k),p_min,p_max)` → 种子骰定正负。buff 源=点数/前后相邻/士气/附魔能量/路buff/主动干涉(各 bounded)。
- **行军**:三路 slot 逐格前进,接敌对决,赢=占格前进+续航−1(尽则沉牌底+回合冷却),输=本局弃牌(收集不减)。
- **胜负**:皇室战争式大本营 **3 血** + 回合上限 + 可平局(不做大血条)。
- **支柱**:胜率可读(对决前显天平+buff明细) + 单挑特写。
- **重映射**:现有装备/附魔/羁绊 从喂 atk/hp → 喂 点数/buff。

### 产出 + 下一步
- 已落 **`docs/game-design/game-f-core-combat-spec.md` v0**(含 buff 全表/数学模型/行军对决流程/回收冷却经济/皇室战争胜负/仿真台规格/重组分析)。
- **未派实现**(大重构,先锁后做):待 ① owner 拍小丑牌用法(被动/主动/两者)+微调;② 策划F 引擎评估(候下沉=种子RNG+clash-resolve,余重组)。
- BATON 置 🟣 owner 共同设计中。
