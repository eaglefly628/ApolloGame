...
> | **状态** | 程序F 推进中:✅P1 done **+ 策划F 验收 PASS**(game-f tsc clean+131绿+sim零RNG) · ⬜P2/P3/P4 续做 |
> | **上一手(程序F)完成** | ✅ **P1 战利品自动拾取**：item_bag 改全盘静态收集体(覆盖 ARENA、Visibility 隐形)→ 敌死任意格装备 orb 自动归集，删"主公捡 orb"摩擦。✅ **P1 概率掉落**：mob_death 拆 `mob_death`(名将/Boss 掉装备)/`mob_death_bare`(杂兵足轻仅掉金不掉装备)，mobTemplate 按 `unit.seg` 确定性选(零 RNG、不入 sim)。+测(自动拾取棋盘中央归集 / seg 分流)、frame golden + 快照更新、1448 绿、真机无杂框零报错。**P2/P3/P4 续做(本棒仍程序F)。** |
...
> | **最后更新** | 2026-06-18 · by 策划F · 循环#51(验收 P1 PASS,棒仍在程序F续 P2–P4) |
