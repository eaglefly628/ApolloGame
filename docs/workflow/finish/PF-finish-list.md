...
25. ✅ **UI 锦霞换肤批（2026-06-12 凌晨，用户钦定 design_handoff_game_f @9e08971 女性向风格）**：①对局内全量染锦霞（纯数据：palette 表逐 token 映射全部 Color.tint——势力内容色蜀红#d8504e/吴绿/魏蓝、HP/MP 条、横幅/HUD/按钮/商店面板/席位框/垃圾桶/符文/战果面板；三字体槽 FONT_DISPLAY 行楷毛笔/FONT_BODY 宋体/FONT_NUM 像素数字进 Text.fontFamily 数据；棋盘格 SVG 重染暖珍珠描金/雾青柔蓝并改 40×46 贴合 TILE）；②壳层 game-f.tsx 重写：锦霞/玄铁双 ThemeTokens CSS 变量（README 表逐值照抄）+ 页标签「对局｜商城」+ 货币 chips + 换肤切换 + kit notification 提示条 + Google Fonts 注入；③**商城五分页 hifi**（抽卡：凤羽转角纹饰横幅/UP 玉印/保底进度/单抽十连[送1次角标]/三常驻池；皮肤 3 卡限时折扣角标；通行证 8 段双轨道当前级高亮；钻石 6 档首充热卖角标；礼包 3 卡划线价）——占位数据、交互态按 §交互态规范（hover 抬升/press 下沉/reduced-motion 降级）。全套 1102 + build 绿。
