...
- **cycle#24 (VIS-1 ⭐⭐ 离线看帧 · enabler · design/16)** ✅：owner 转向"把画面做出来看一看"，design G 派**视觉优先**(`16`)。VIS-1=离线看帧（enabler，先做）。加 `src/games/game-g/render-frame.ts`：跑一局到关键拍→**投影成 SVG 落 `doc/screenshots/`**（02-开战抛牌/03-掷命揭晓金石/04-终局Boss 三帧）。复刻 ThreeRenderer 抛飞弧+相撞靠拢+翻面+金石对比（**共用 `feel.ts` 曲线**，时序与真渲染器一致）。tsc+build 全绿（render-frame 是独立脚本、不入浏览器 bundle）；总测不变（1311 绿）。**已把 3 帧发给 owner 看**。
  - **⚠️ 报 design G（与 `16`§二的 PNG/WebGL 差异，CORE RULE）**：spec 写"ThreeRenderer 渲离屏 canvas→导 PNG"，但 **node 无 WebGL/GL 上下文**（headless-gl 未装）跑不了真 ThreeRenderer；**game-f 同理也用 SVG 投影**。故我用 **SVG 投影**（确定性/可版本控制/浏览器直接看/可 diff），复刻同一套表现曲线 = 同样"看得见、能评"。⚠️ 是 3D 场景的**正交近似**（非透视像素级一致），评审场景结构/演出足够，不替代真机 WebGL。**这一帧立刻坐实了「有牌没战场」**（cards 在空场、无三路轨/老家/哨塔）——正是 VIS-2 要建的。布阵/大厅是 DOM 屏（非 3D 场景），可后续 SVG mock 或真机看。**下一步 VIS-2**：三路战场 3D 场景（路/老家牌王座♔/哨塔/列阵/相机框路）。
