...
- **cycle#33 (⛔⛔ WIRE-MARCH · live-combat 真正接进可玩游戏)** ✅：owner 跑游戏纠正「战斗还是一大堆刷过去」+ design G 查实根因（`live-combat.ts` 是孤儿、`showMatch` 仍跑老 `buildGameGArmyMatch` ~2.5s 瞬翻插值）。本轮把它**真接线**（`ff3980f`）：
  ...
  - **W3 出帧**：`battle-screen.frame.test` 重写真 live sim → **4 golden**(tick6 全 108 面朝下行军 / tick25 三路最前两张相邻翻牌成波 / 破家「已破」/ 锦霞皮) + **行为断言**(最前兵 pos01 随 tick 单调 0.12→0.30→0.50、行军 revealed=0、接敌 revealed=6)。结算改读 live(survA=在场+续航退场、净控路、3 血老家)。tsc+vitest(**1417**)+build 全绿。
  - **⚠️ 诚实留 design G(2 条·不阻塞)**：① 镜像对局(0vs0)A 偏胜 ~62/38(`marchSide` 同拍先 A 后 B 的位置小不对称；实战恒非镜像已被掩、Boss 局 b 也赢，留 3D-SIM 校准、不动既测 sim)。② 阵亡只即时消失、暂无「斩」死亡闪帧(driver ghost 待做、纯表现)。**翻棒 design G 跑起来肉眼终验**（测绿≠体验对）。
...
- **cycle#28 (VIS-4 逐路揭晓 · 2:1 翻盘悬念)** ✅：接 design G 第2轮派的 VIS-4。`feel.ts` 加纯函数 `laneRevealProgress(prog,lane)`（上路 0 先翻/下路 2 后翻、prog=1 各路皆落定、每路单调）+ `easeOutCubic`（翻面缓出）。**render-frame 与 ThreeRenderer 共用**：每张牌的 抛飞弧/翻面/金石揭晓 都吃**该路的** lp → 上→中→下**依次落定**（非三路同闪），制造"上路赢了、中下还能否 2:1 翻盘"的连续悬念。ThreeRenderer 的翻面角**重导自 Tween+easeOutCubic(lp)**（不读 sim 角、纯表现按路错开、不进 hash）。**新出 `03-stagger.svg` 帧**：上路落定亮金石、下路仍在弧上飞 = 逐路揭晓可视、已发 owner。+4 测(laneRevealProgress 上先下后/落定一致/单调/easeOutCubic)；总 **1323 绿** tsc+build 0。出帧 4 张(开战/逐路揭晓/揭晓/Boss)。**评审环又一轮**：VIS-1→2/2b→港→VIS-4 全在 scene.ts/feel.ts 单一真相、帧=游戏。**余**：斩首聚焦 hitstop/Boss 入场仪式（VIS-4 余）、VIS-3 HUD、VIS-5 美术资产。
- **cycle#27 (VIS-2 港进 ThreeRenderer · 真 3D 战场)** ✅：design G 放行后，把 approved 布局**港进真渲染器**——`ThreeRenderer` 现**消费 `scene.ts`**（与 SVG 评审帧同一份）：① 卡按 `cardScreenPos` 摆**三路**(A 左/B 右、front 接敌中线、抛飞弧上跳，缩 CARD_SCALE 档)；② 静态 `buildScenery()`：古风地面 + 三路分区带 + **左右老家牌王座 ♔**(我金/敌青·♔ 画布贴图) + **哨塔**(每路 A/B)；③ 相机固定框 scene 包围盒；④ 保留 applyReveal 金辉光/石压暗。**因共用 scene.ts → 浏览器真画面 = 你和 design G 批过的那几帧**(不漂移)。⚠️ WebGL 在 node 不可 headless 验，**布局由 scene.ts(SVG 帧已证)保真、3D mesh 码由 build 绿保真、实际观感待 owner 真机看**。tsc+build 全绿(game-g chunk 533kB)；总 **1321 测不变**(渲染器浏览器侧、不入测)。**下一步**：VIS-4 逐路揭晓（feel.ts 纯函数+render-frame 可验+ThreeRenderer）。
- **cycle#26 (VIS-2 港前置 · 抽 scene.ts 单一真相布局)** ✅：design G 第2轮**通过 VIS-2/2b、放行进 ThreeRenderer**（`16`§九）。港进真 3D 渲染器是大块且 **WebGL headless 不可验**、owner 关切 rate limit → **分两步走、步步可验**。本步=**抽 `scene.ts` 单一真相布局**（`cardScreenPos`/`LANE_Y`/`HOME_AX/BX`/`TOWERS`/`laneScores` 等，纯函数·零 Three/DOM），render-frame 改为消费它——**保证"评审帧 = 真游戏"不漂移**（render-frame 与即将改的 ThreeRenderer 共用同一份）。重构后**3 帧字节级 identical**（证明抽取忠实）+ `scene.test.ts` 4 测（A 左 B 右对称/后排远中线/lane 决定 y/arc 上跳/laneScores/TOWERS）。总 **1321 绿** tsc+build 0。**下一步**：ThreeRenderer 消费 scene.ts（卡按三路摆 + 老家牌王座/哨塔/分区 mesh + 金辉光/石压暗）→ 真游戏长成 approved 帧的样子；再 VIS-4 逐路揭晓。
- **cycle#25 (VIS-2/2b · 三路战场 + 命运一掷加戏 · 接 design G 第1轮评审)** ✅：design G 读 VIS-1 帧后评（`16`§八）：①有牌没战场 ②命运一掷 flat。本轮直接回应（render-frame.ts SVG = 评审介质，先在帧里成形）：
  - **VIS-2 三路战场**：渲染器侧**纯表现重映射**——按 `lane/side/idx`(从 pairKey+side 解)把卡摆成 **MOBA 三路**(上/中/下横轨分区) + **左右老家牌王座 ♔**(我军金/敌军青) + **哨塔**(每路 A/B 各一) + **卡按路列阵**(A 左 B 右、front 接敌中线、后排退向老家) + **三路比分**(活牌数 A:B、领先变色 → 弃一保二一眼读) + 接敌中线。补"有牌没战场"。gameplay 位(laneSlot)不变、不进 hash。
  - **VIS-2b 命运一掷加戏**：活牌落定 **gold 辉光**(radialGradient,rev 驱动) / 死牌 **碎裂纹+去色压暗** / **古风战场底**(linearGradient 地面,非纯 void)。从"太平"→"金石生死戏"。
  - tsc+build 全绿；3 帧已发 owner + 落 doc/screenshots/。**⚠️ 现仅在 render-frame(SVG 评审帧)成形**；待 design G 认可构图后**同步进 ThreeRenderer**(真 3D 游戏画面)——评审环：先帧后端、改一处看一处。**下一步**：等评审 → 港到 ThreeRenderer + VIS-4 逐路揭晓/溃散级联。
- **cycle#24 (VIS-1 ⭐⭐ 离线看帧 · enabler · design/16)** ✅：owner 转向"把画面做出来看一看"，design G 派**视觉优先**(`16`)。VIS-1=离线看帧（enabler，先做）。加 `src/games/game-g/render-frame.ts`：跑一局到关键拍→**投影成 SVG 落 `doc/screenshots/`**（02-开战抛牌/03-掷命揭晓金石/04-终局Boss 三帧）。复刻 ThreeRenderer 抛飞弧+相撞靠拢+翻面+金石对比（**共用 `feel.ts` 曲线**，时序与真渲染器一致）。tsc+build 全绿（render-frame 是独立脚本、不入浏览器 bundle）；总测不变（1311 绿）。**已把 3 帧发给 owner 看**。
  - **⚠️ 报 design G（与 `16`§二的 PNG/WebGL 差异，CORE RULE）**：spec 写"ThreeRenderer 渲离屏 canvas→导 PNG"，但 **node 无 WebGL/GL 上下文**（headless-gl 未装）跑不了真 ThreeRenderer；**game-d/game-f 同理都用 SVG 投影**。故我用 **SVG 投影**（确定性/可版本控制/浏览器直接看/可 diff），复刻同一套表现曲线 = 同样"看得见、能评"。⚠️ 是 3D 场景的**正交近似**（非透视像素级一致），评审场景结构/演出足够，不替代真机 WebGL。**这一帧立刻坐实了「有牌没战场」**（cards 在空场、无三路轨/老家/哨塔）——正是 VIS-2 要建的。布阵/大厅是 DOM 屏（非 3D 场景），可后续 SVG mock 或真机看。**下一步 VIS-2**：三路战场 3D 场景（路/老家牌王座♔/哨塔/列阵/相机框路）。
- **cycle#23 (T-G6 收官 · foil 闪艺收集 + 心跳改 15min)** 🟡：**owner 反馈循环断=rate limit**（4min/重周期烧配额太快 + 心跳 Monitor 随会话挂起而死）→ **心跳改 15 分钟/次**(省配额)，周期更轻。接 design reply#17 余项 **foil 皮肤**（附魔回驳后改造的纯表现收集）：`GAME_G_FOILS` 4 张闪艺(鎏金/碧霄/赤焰/玄曜)纯数据 + Save `foils[]` 局外持久 + 大厅改造坊 foil 收藏区(花材料解锁、N/M 收集进度)。**⛔ 纯表现·不进 hash·零 gameplay/平衡影响**(design 红线)、零引擎触点。+1 测(池/唯一 id/cost/名述)；总 **1311** 绿 tsc+build 0。**T-G6 培养层=收官**(10 小丑+4 星球+流派网+激活质变+foil 收集)。**余**：次环克制校准(待数值意图)、U5 L1 演出(⚠️ 真机验)、AI 真按流派差异化(需给 AI 自己的 archetype/杠杆,较大,待 design)。
- **cycle#22 (U6 · AI 按克制反制布阵 + 下沉可测)** 🟡：接主策划 reply#17 列的余项"AI 按克制反制布阵"。把原**散在 game-g.tsx mount 里、未测**的 `aiFormation` 逻辑**下沉**成纯函数 `pickAiFormation(stage,materials,lastOfficers,committed)`(blueprint·可测)。新增 **committed 反制**：玩家**集齐招牌流派**(`activeArchetype!=null`)→ AI **全程**(不限高关)猛攻你最弱一路——"亮出招牌的强玩家面对更尖 AI"。+4 测(低关均衡/中关变化/高关攻弱/committed 低关也反制 vs 对照)；总 **1295** 绿 tsc+build 0。
  - **⚠️ 报主策划（CORE RULE·诚实）**：克制网是 **archetype↔archetype**，而 AI 的杠杆是 **formation**——formation **无法精确"克制某流派"**(best-of-3 数学下，攻对手最弱路才是稳解，逐流派映 formation 既弱又糊)。故我取**"committed→AI 更尖(攻最弱路)"**这个**清晰可测**的反制，而非"每流派→特定阵型"的糊映射。**若你要 AI 真按流派差异化**，需给 **AI 自己的 archetype/干预杠杆**(让 AI 也能斩首/铺场…)——那是更大的一块(给 AI 一套对称的 build)，请定要不要做。现版=committed 难度反制，清晰、零糊。**余**：foil 皮肤(纯表现·收集，render 不可 headless 验)、次环克制数值校准(待你意图)。
- **cycle#21 (硬化 · 胜负正确性测 + idle 待迭代)** ✅：补 cycle#20 揭出的覆盖盲点——既有测多是**确定性(hash)/存活计数/favor 变换**，但**没测"谁赢"**(best-of-3 路比较方向)。加胜负正确性测：压倒强军(偏置+20+不屈地板88) vs 裸弱敌 → `winner=a`；反向 → `winner=b`。验 lane→best-of-3→winner 链方向对。总 **1291** 绿 tsc+build 0。**审计确认游戏码无同类 bug**：`game-g.tsx` onFrame 读存活用对的实体 id `res_a0`(只测里写错过、已修)。**idle — 待主策划下一轮迭代**：規格'd 玩法做完(T-G5 全✅ / T-G6 小丑10+星球4+流派网+激活质变)；余项均待外部输入——**附魔**(我评判倾向回驳/改造、待裁)、**U5 L1 演出**(⚠️ headless 看不到观感、需真机/主策划盯)、**AI 按克制反制布阵**(需定 archetype↔counter-formation 映射意图)、**次环克制校准**(需数值意图)、**T-G1 大厅 GameShell 迁移**(大架构、低优先)。我每心跳继续硬化/小打磨、绝不留红/未推送；有派单即接。規格'd 玩法基本做完(T-G5 全 ✅ / T-G6 小丑 10 + 星球 4 + 流派网 + 激活质变)，按 SOP 步 7 做硬化而非硬塞。**抓到并修真 bug**：之前 4 个"存活单调"测(diehard/死士/旗手/影武者用)读存活资源用了**资源 id `a_l0`** 而非**实体 id `res_a0`** → `getComponent` 返 undefined→0 → `>=0` **空过**(没真测存活)！新加的 `>` 测(养成回报)揪出。**全改 `res_aN`** → 4 测现真验存活单调 + 新 2 测(养成回报：满配星球+集齐将领流+干预 vs 同 Boss 存活>裸军；最大配置：星球+铺场流激活+联动+护盾/增援+终局 Boss 同 seed 逐拍 hash 一致)。总 **1290** 绿 tsc+build 0。**这就是硬化的价值——空过的测=没有的测**。
  - **附魔 CORE-RULE 评判（报主策划，建议先别做）**：`12`§一 附魔=「+favor% / +触发次数(foil/holo)」。①**+favor%**：乘法 favor 与现成**大厅商店(+favor 平)+星球(军/型 favor)**功能高度重叠，乘法 vs 加法仅薄差异、且无"per-card 持久"载体(Game G 军阵从布阵 build、deck 只是均值偏置，无具体卡身份挂附魔)；②**+触发次数**：**不合 outcome-first**——本作无运行时"触发"可重复(胜负 build 时一次定)，"重触发"无落点。→ **倾向回驳/改造**：要么砍(养成已够厚：10 小丑+4 星球+流派激活 3 轴)，要么把附魔**重定义成与现成不重叠的东西**(如纯表现 foil 皮肤=不进 gameplay；或"某军衔档 ×favor"的乘法档，与星球·军加法区分)。**请主策划定**：附魔做不做/做成什么；不做则 T-G6 养成层=**收官**。
  - **approaching idle**：規格'd 玩法已厚。余 = 附魔(待裁)、U5 L1 演出(⚠️ headless 看不到实际观感、只能盲调)、AI 按克制反制布阵(U6)、T-G1 大厅 GameShell 迁移(大架构收口、低优先)。**等主策划下一轮迭代指方向**；其间我做硬化/小打磨，绝不留红。
