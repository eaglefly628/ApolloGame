...
## 2. 剧本清单（9·覆盖 owner ①②③ + 开局 + 典当 + 节奏/逻辑·acceptance-run 全绿）
...
| `07-street-progression-rhythm` | **节奏**·一手四街推进（gdd §2 心跳单元） | **精确**：主角被动跟/过·逐街断 street+公共牌(0→3→4→5)+下注圈重开(注归0)·河后摊牌三条独收 1100 |
| `08-uncontested-fold-to-win` | **逻辑**·全弃收池（gdd §2 全弃直接收池） | **精确**：除座2 外全弃→doUncontested 无摊牌收池·winner-type「全弃收池·无摊牌」·座2 得 950·守恒 |
| `09-turn-rotation-pacing` | **节奏**·逐拍轮转（gdd §5 分步演出） | **精确**：一 tick 一行动·actor 顺时针 3→4→5→主角·每步 last-action 钉死·注线随动 |

> 断言进化（GD-C 历轮加固）：owner ①②③ 从「结构不变量（`gte 1`·有赢家池已派）」升级为**精确值钉死**——三条胜两对的**比牌**、底池的**逐层切分金额**、深栈 kicker 决胜（Q>10）、以及**守恒**（派出总额==底池）全部机读断死；再补**节奏/逻辑**面（本轮）：四街推进的发牌节拍(07)+下注圈重开、全弃收池的无摊牌结算分支(08·区别于比牌)、逐拍轮转的「轮到谁」节奏(09)。确定性引擎（seed+占位 AI）令精确值可复现；`gte 1` 测不出「算错赢家/漏派池/切错层/发牌节拍乱/收池走错分支」，精确值能。代价：值钉死到 (seed + M1 占位 AI)，M2 换真行为树后 GD 按同法重派（REQ-ACCEPT 分工·PE 不改剧本）。
