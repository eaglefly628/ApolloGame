...
**所以本约定不是"防静默错数"，是"防开局即崩"**——配错不再算错分，而是**当场崩给你看**。
约定本身一字不改：各侧唯一 id 仍是唯一正解，硬抛只是安全网、不是替代品。
落盘门抓不到这件事（它只看 `DuelMatrix` 数据，看不见世界里挂了几份同 id 资源），故只能靠本约定在前、运行期硬抛在后。
详见 `requests.md` 复查侧验收「发现一」。
