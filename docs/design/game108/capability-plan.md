...
| ↳ `payoff` 支持蓄力缩放 | 伤害 = base + 蓄力 × step【R-108-13】 | 🔺 **唯一缺口·待 owner 判 A/B**（工单 `REQ-108-ENG-01`·见 §4） |
...
### ⚖ 缺口裁决协议（**owner 2026-08-04 立·本项目适用**）
...
### 🔺 唯一缺口：`payoff` 蓄力缩放（**待 owner 按上述协议判 A/B**·工单 `REQ-108-ENG-01`）
...
| 伤害按蓄力缩放【R-108-13】 | **L2 待 owner 判 A/B** | 判 A（扩写 payoff）→ 降 L1；判 B（游戏层自算）→ 升 L3 并记债 |
...
- **Lead 初裁**：🔶 **有条件通过**——条件两项：
  1. **`payoff` 蓄力缩放【R-108-13】等 owner 按缺口裁决协议判 A/B**，判完并落地才进 S3 玩法骨架
     （本体 `t2-matrix-duel` 已落地，卡口已从「整条能力」缩到「一个字段」）；
...
- **待 owner 一并定**：缺口裁决协议（§4 头）**是否提升为全库规则写进 CLAUDE.md**——它与核心规则 2/3 的
  「Lead 裁」冲突，目前只对 game108 生效；不提升则其他游戏仍按 Lead 裁，两套口径并存。
