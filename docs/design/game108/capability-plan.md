...
| ↳ `payoff` 支持蓄力缩放 | 伤害 = base + **出手方**蓄力 × step【R-108-13】 | ✅ **返工已交付·复查侧验收 PASS**（`perSide:true` + 相对名·端到端条款对拍在案） |
| ↳ `clearOnSettle` / `lastThrowVar` 结算副作用 | 出过即清零【R-108-14】+ 记本回合的手【R-108-02/30】 | ⏳ **需扩写**（`REQ-108-ENG-03`·owner 判 A1·**§7 普查证伪了原「6 条静态规则」重组方案**） |
| `t3-flow`（`GameFlow`） | **三时区四拍状态机**（T1蓄力→T2出招→T3对决→T4结算）+ 对局胜负收束 + 爬塔层进【R-108-01/15/50】 | ✅ 现有 |
| ~~`e1-timer`（原子）~~ | ~~每时区 3/3/2/1 秒倒计时 + 超时顺延【R-108-02】~~ **v3 起没用上**：四拍时序全由 `t3-flow` 的 `FlowTransition.after` 走（`flow.elapsed` 进状态归零，天然对齐相位）；罚血周期也用 flow 的两态互跳当钟——Timer 全程自走，进罚血那一刻对不上相位，头一次罚血会随机延迟 0–1 秒 | — |
...
**本 plan 累计三个引擎缺口，全部由 owner 判 A 系（ENG-01 按侧缩放返工 / ENG-02 出招输入接缝 / ENG-03 结算副作用）：**
...
- ~~**出过即清零【R-108-14】**：6 条静态 `t2-event-when` → `t2-effect-apply` 置零~~
  🔴 **已作废（§7 普查实查证伪·2026-08-06）**：出招信号两侧共用一个名（靠 `Signal.source` 认侧），
  而 `Effect.targetId` 是**全局寻址**——一条规则分不清该清哪一侧的槽；且「记本回合的手」要写字符串，
  `Effect.kind` 十项里没有。→ 改走 `REQ-108-ENG-03`（owner 判 A1·`clearOnSettle`/`lastThrowVar`）。
  **教训**：S2 时我按「大概能拼出来」写下了这条重组方案，没有落到「具体哪条 Effect、targetId 填什么」——
  **重组方案也要查到字段级**，否则就是纸面自信。
...
- **S6 现状（2026-08-08·owner review 后重做）**：台账 **15 行** 0 孤儿 · 锚 ✓ · 写回 3 · 复核 0。
  · **3 行 → 15 行的原因**（owner：「我只看到 3 张图片，我们这个游戏不可能只有 3 张图片」）：
    原先只有那 3 样**可替换**，其余视觉全是代码里现画的（`plate()` 调 46 次 + `handArt`/`armArt`/…），
    没有皮肤槽 ⇒ 按红线不许进台账。**「只有 3 行」说明的不是图少，是能换的少。**
  · 修法：新建**皮肤槽登记表** `games/game108/art-slots.ts`（单一真相），屏与台账推导脚本**同源读它**；
    屏那层加 `pickSkin(view.skins, key, 程序化回退)`；宿主用基座件 `loadGameArtOverrides('game108')` 灌 skinMap。
  · 打开的面：亮拳大手 ×6（**屏上最大的一件·情绪核**）· 手臂 ×2 · 判定表石板 · 烟雾图标 · 舞台背景 · 手型图标 ×3。
  · **刻意不进台账的**：血条填充 / 倒计时环 / 加载条进度 —— **值驱动**，形状每帧随数值变，
    一张静态 raster 顶不上去。要换皮得走「底槽 + 填充」两件套或 9-slice，那是另一张单，不拿它们凑行数。
  · 对手立绘由外部卡片角色传入，是**调用方的资产**，记这里会变成两处真相。
  · 两条点名测试守着：① 登记表 ↔ 台账逐行对得上 ② **喂假皮进去，屏上必须出现它**（防"记了账没人读"）。
  · **每一行都看得见现状**（owner 2026-08-08 二次 review：「占位符，但我看不到本来是什么样子」）：
    `scripts/game108-art-previews.mjs` 把**游戏里当前的程序化画法**导成每行的 `.svg` 预览
    （直接调 `handArt`/`armArt`/`plate`/`scene`，**不另画一份**——另画就是第二处真相），
    并给每行配一句**现状实话**（`art-slots.ts` 的 `look`）。行状态从 needs-art 升到 **placeholder**。
    ⚠ **placeholder ≠ mock**：前者是游戏里真在用的那张、可上画面；后者永不上画面、不可 approve。
    点名测试钉死：不是真图就必须有预览文件（**盘上真的在**）+ 一句说明 + 不许标 mock。
  · 这一轮真渲染又逮到一个只有看图才看得见的：`scene/stage` 的槽**接在了 wrapper 底色上**，
    而玩家看见的背景是 `stageBg()` 画的那张 ⇒ 换了图也盖不住。更糟的是我的守卫测试
    **给它开了后门**（`painted.add(SCENE_SLOT)` 豁免）——尺子照着被测物做，豁免掉的恰好就是错的那一格。
    后门已拆，撤修即红。
  · **仍卡在 `DASHSCOPE_API_KEY` 未配**：探针 `configured:false → mock 占位（绝不静默顶替）`；
    mock 不上画面、不写回、不可 approve，故 12 行新面只能停在 needs-art。这是**外部条件**不是欠工。
...
| 三时区四拍 + 倒计时 + **罚血读秒**【R-108-01/02/04】v3 | L1 | `t3-flow`（`after` 时序门 + 两态互跳当周期钟）|
...
### 🔒 实现约定（**v3·返工落地后的最终写法**·2026-08-06）

**S3 建蓄力槽照这个写，两处缺一不可：**

| 哪儿 | 怎么写 |
|---|---|
| **槽的 Resource id** | `p1.charge.rock` / `p1.charge.paper` / `p1.charge.scissors` / `p2.charge.rock` / …（各侧唯一） |
| **`DUEL_MATRIX.payoff` 里** | 填**相对名** + 开关：`{ base: 10, scaleByResource: 'charge.rock', step: 10, perSide: true }` |

运行期由 `t2-matrix-duel` 拼成 `<出手方实体 id>.<相对名>` → p1 出手取 `p1.charge.rock`、p2 出手取 `p2.charge.rock`。

> ⚠ **只改一半 = 老毛病原样复发**：光把槽改成唯一 id、表里仍填绝对 id（`p1.charge.rock`），
> **p2 出手照样按 p1 的槽结算**——这正是 `REQ-108-ENG-01` 被打回的那个缺陷，已实测。
> `perSide` 忘了写同理（缺省 = 绝对 id）。

<details><summary>历史：v1「各侧唯一 id 即可」为什么是错的（留档防再犯）</summary>

v1 约定只写了「各侧唯一 id」，暗示这样就能按侧取值——**错的，已实测证伪**：
`DuelMatrix.payoff` 双方共用一张、`scaleByResource` 只能填一个字符串，所以无论同 id 还是唯一 id，
两侧出同一手都取到同一条槽。真·local 寻址（读 attacker 自己身上那份）也永远落空，因为引擎
一实体一组件、侧实体那个 `Resource` 槽已被 `hpResource` 占死。
⇒ 光靠命名约定治不了，必须引擎侧支持 id 组装（= `REQ-108-ENG-01` 返工，已交付）。

</details>

**配错时引擎会怎样**（两道防线·都不是「按侧」的替代品，按侧靠上表那两处写对）：

| 配错形态 | 引擎行为 |
|---|---|
| 两侧共用同一个 id（`charge_rock` × 2 份） | **运行期点名硬抛**（`e06fb61c`·报份数 + 全部涉事实体 + 改法建议）——不静默算错侧 |
| `perSide` 忘写 / 表里填绝对 id | 只有 1 份命中 → **硬抛不触发**，缩放静默取那一条固定槽 ⇒ **只能靠上表约定 + S4 对拍防** |
| 相对名拼不出槽（打错字） | 退化成 `base`（绝不 NaN 污染 hp 与快照 hash）·**不会**去撞同名的绝对 id 槽（有专门守卫） |

落盘门这三件都抓不到——它只看 `DuelMatrix` 数据，既看不见世界里挂了几份同 id 资源，
也不知道运行期谁是 attacker。故防线 = **上表约定在前 + S4 对拍在后**。
详见 `requests.md` 复查侧验收「发现一」+「改判」+「返工交付」。

**S4 验收剧本必须带一条对拍**：双方同手蓄力不同时，各自伤害只跟**自己**那条槽走。
（引擎侧同形状的守卫已在 `matrix-duel.test.ts`「两侧对拍」，S4 这条验的是**本作数据表填对没有**。）



- **随机源**：世界单例 `RandomSeed`；游戏层**零裸 `Math.random`**（`game-skill-audit` 红旗）。
- **seed 来源**：常规局开局一次性取并存档；**每日挑战 = 日期码派生**（同日同塔·可比可回放）。
- **消费点**：AI 两次抽选【R-108-30】· 破绽概率门【R-108-31】· 二选一抽取【R-108-41/42】。（~~超时顺延【R-108-02】~~ v3 作废）
...
## 7. S3 骨架关 · 数据通路系统普查（2026-08-06·**一次查完，不再逐条挤牙膏**）

前两个缺口是「撞一个报一个」，效率低。开工写蓝图前改为**对竖切涉及的每条条款逐条查数据通路**，
一次性把缺口摆齐。逐条结论：

| 条款 | 需要的世界写入 | 通路 | 判定 |
|---|---|---|---|
| 【R-108-10】蓄力 +1 | 信号 → 某侧槽 +1 | 玩家 `charge.<手>` / AI 用**内部信号名**（词表只管玩家动作）→ `t2-effect-apply{modify-resource}` | ✅ 通 |
| 【R-108-10】上限锁 | 槽封顶 3 | `Resource.max` 天然钳位；UI 侧按 bind 值禁用键 | ✅ 通 |
| 【R-108-11】出招 | 信号 → `DuelIntent` | `intentSignals`（两侧共用信号名·靠 `Signal.source` 认侧） | ✅ 通（`REQ-108-ENG-02`） |
| 【R-108-12/13】判定与伤害 | — | `t2-matrix-duel` + `perSide` 缩放 | ✅ 通（端到端对拍在案） |
| 【R-108-15】胜负 | hp≤0 收局 | `t3-flow` 读 `Resource` | ✅ 通 |
| 【R-108-01】三时区四拍 | 按 tick 推进相位 | `t3-flow` 的 `FlowTransition.after`（tick 门） | ✅ 通 |
| **【R-108-14】出过即清零** | **按侧**把「刚出的那只手」的槽置 0 | `clearOnSettle` | ⏳ `REQ-108-ENG-03`（owner 判 A1） |
| ~~**【R-108-02】超时顺延「保持上一回合的选择」**~~ | **v3 作废**（owner 2026-08-07）：T2 超时改走【R-108-04】罚血读秒，罚到玩家出手为止。`lastThrowVar` 仍在用（AI 抄上一手）| `lastThrowVar` | ✅ 落地（六条 `carry:*` 规则已删） |
| **【R-108-30】AI「抄玩家上一手」** | 读上回合玩家出了哪只手 | `lastThrowVar` | ⏳ `REQ-108-ENG-03`（同源） |

### 两个缺口是**同一个根因**

`t2-effect-apply` 的 `Effect` 只能写**数值 / 布尔 / 状态名**，且 `targetId` 是**全局 id 寻址**：

- **缺口 A**：出招信号两侧**共用一个名字**（靠 `Signal.source` 认侧），而 `Effect.targetId` 是全局的
  → 一条 Effect 分不清该清 `p1.charge.rock` 还是 `p2.charge.rock`。
  （`@signal-source` 哨兵**只对物理 kind**（set-sensor/set-visible/destroy）生效，`modify-resource` 用不了——已实查。）
- **缺口 B**：「上回合出了哪只手」是**字符串**。`StringVar` 只能由 `StringSet` 事件改，
  而**全库只有 `t3-poker-hand` 和 `x3-string-variable` 自己产 `StringSet`**——
  `Effect.kind` 十项里没有「写字符串」（已实查）。

⇒ **信号能改数字，改不了「按侧的数字」，也改不了字符串。**

### 摆路（Lead 举证·不下裁决·依 CLAUDE.md 核心规则 3）

| | **A1 收进解释器**（Lead 推荐） | **A2 扩通用信号写入面** | **B 游戏独有逻辑** |
|---|---|---|---|
| 做法 | `t2-matrix-duel` 结算时自带两件副作用：① `clearOnSettle?: 相对名` → 把双方**各自出的那只手**对应的 `<侧>.<相对名>` 置 0；② `lastThrowVar?: 相对名` → 把双方本回合的手写进 `<侧>.<相对名>` 的 `StringVar` | 给 `Effect` 扩两处：`modify-resource` 支持 `@signal-source` + 相对名寻址；新增 `set-string` kind | host 在结算后自己清零、自己记 lastThrow |
| 为什么可行 | **matrix-duel 是唯一同时知道「谁出了什么」的地方**——这两件事本来就是它的结算副作用 | 通用，任何「按信号源写自己那份」都受益 | — |
| 风险 | 低（additive·不填=零回归·边界仅结算末尾） | **中高**：`effect-apply` 是全库最核心的写入面，改它影响所有游戏 | 违 `events-logic.md` 红线；且 AI 与玩家各写一套 |
| 通用性 | 中（服务所有同时决策对抗） | 高 | 零 |

**✅ owner 2026-08-06 判 A1**（工单 `REQ-108-ENG-03`·spec 已写死·施工主体=主程）。Lead 推荐理由：范围最小、语义最贴（结算副作用归结算者）、且**不动全库最核心的 effect-apply**。
A2 更通用但风险面大得多，不该被一个游戏的需求推着改。

**A1 的边界（若判 A1）**：只在 `matrix-duel` 结算末尾加两个可选字段的处理 + 落盘门（相对名非空、不得是 `hpResource`）
+ 测试（清零按侧 / lastThrow 按侧 / 平局也清 / 不填零回归）；**不碰**判定、补丁 fold、定序、缩放、接缝。

> **S3 现状**：`theme.ts`（数值 + id 约定 + 动作词表，全部对条款实名）已落地且 tsc 绿；
> **蓝图装配等这条裁决**——缺口 A/B 不解决，【R-108-14】和【R-108-02/30】就只能靠游戏层自由代码，
> 那正是本项目从头到尾在避免的东西。
