...
| ↳ `payoff` 支持蓄力缩放 | 伤害 = base + 蓄力 × step【R-108-13】 | ⏳ **唯一缺口·owner 已判 A**（扩写引擎·工单 `REQ-108-ENG-01`·待施工） |
...
| 伤害按蓄力缩放【R-108-13】 | **L1**（owner 判 A） | 扩写 `t2-matrix-duel.payoff`；落地即生效，游戏层零代码 |
...
- **Lead 初裁**：🔶 **有条件通过**——条件两项：
  1. **`payoff` 蓄力缩放【R-108-13】——owner 2026-08-06 已判 A（补引擎缺口）**，扩写落地才进 S3 玩法骨架
     （本体 `t2-matrix-duel` 已落地，卡口只剩这一个字段）；
