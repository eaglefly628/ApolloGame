...
| ↳ `payoff` 支持蓄力缩放 | 伤害 = base + 蓄力 × step【R-108-13】 | ✅ **已落地**（`e3a568fb`·owner 判 A·复查侧验收 PASS·38/38 绿） |
...
| 伤害按蓄力缩放【R-108-13】 | **L1** ✅ | `t2-matrix-duel.payoff` 缩放式已落地（`e3a568fb`）；游戏层零代码 |
...
### 🔒 实现约定（复查侧 2026-08-06 实测发现·S3 必须照此建）
...
出手方那份 `Resource` 已被 `hp` 占用 → 蓄力槽必然另居实体 → **local 永远落空 → 一律全局回落 → 取第一个同 id 的**。
两侧同 id 时会**静默算错侧的伤害**（已实测：p2 用自身蓄力 0 的手取胜却打出 15 伤，取的是 p1 的 3）。
无报错、无告警，只是数字错——最难查的一类 bug。详见 `requests.md` 复查侧验收「发现一」。
...
- **Lead 初裁**：🔶 **有条件通过**——条件两项：
  1. ~~`payoff` 蓄力缩放【R-108-13】~~ **✅ 已解除**（`e3a568fb` 落地 + 复查侧验收 PASS）——**S3 已无引擎卡口**；
