...
### REQ-108-ENG-01-收益缩放（**owner 2026-08-06 判 A：补引擎缺口**·引擎面·降级存放） · `DuelPayoff.damage` 支持按资源线性缩放 · [2026-08-06] · game108 GDD v2 超休闲重构带出（owner 同日定「公开蓄力槽」机制） → **指派：Opus**（spec 写死·边界极窄） · status: **in-progress·施工主体 = owner 2026-08-06 另派的 Lead session**（S1/S2 已 owner 签·本条 = game108 S3 唯一卡口·未落地不进玩法骨架） · 优先级: P1 · 类型: 既有能力扩写（`t2-matrix-duel`）
