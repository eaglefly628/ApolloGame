...
> 规矩同主池：done 同提交删除条目（查 git 历史）；引擎级缺口不写这里、走主池（满槽由 Lead 裁）。
...
### B-003 · 引擎缺口盘点与提单 · [2026-07-17] · GD-B → LEAD · status: **✅ done（2026-07-17·当日结·全文查 git 历史）** · P1
...
> **⚖ PA 会审回执（2026-07-22·问责=PA 包/手册缺陷·认账）**：PE-B 发现**实锤**——PA 用纯 Node PNG 解码亲验：货架 `mahjong/tex/man-1.png` = **77.5% 透明**（字形浮在透明底），作 opaque albedo 必渲黑；`front.png`=1.1%（象牙底）。**三处处置**：①**源头修（PA 域·优于收编工具）**——与其各游戏各造合成工具，PA 直接把货架 38 张透明字形（37 牌面 + `blank`）**预压不透明象牙底**（新建通用工具 `scripts/asset-flatten.mjs`＝`asset-matte` 的反操作·纯 Node 确定性·压 `front.png` 底 + `#faf4e4` 兜底圆角·`--reindex` 回填 `provenance.flattened`），实测 77.5%→0%·幂等·`reconcile --shared` PASS·before/after 目视成立。**自此任何游戏 vendor `tex/*` 即得可用 albedo·无需再合成**（含 game-b 后续新 vendor）。②**收编定论**：不收编 `game-b-compose-tiles.mjs`（无头 Chromium 重·且源头修后无消费方）——通用能力落 `asset-flatten.mjs`（纯 Node 零依赖）。**game-b 现有本地已合成拷贝 PA 不动**（PE-B 域·已可用·无冲突）；若 PE-B 愿，后续可退掉 `game-b-compose-tiles.mjs` 改直 vendor 新货架（可选·不强求）。③**手册/文档补全**：placeholder 文档「box 贴面」段补透明底陷阱 + 预压说明；`assets.md` 做 X→用什么 加 `asset-flatten` 行（通则：透明底精灵当 3D albedo 先压底）。
...
### B-012 · 美术走索引消费（工坊替换才生效）· [2026-07-22] · owner 报（工坊替换 art-02 游戏没看到）→ Lead 诊断 → **PE-B** · status: open · P1 · 类型: 接消费槽（REQ-ART-可消费槽铁律 option a·game-b 侧）
> **症状**：owner 在工坊替换 game-b `art-02`（二萬 牌面）后，游戏里没变。
> **根因（Lead 2026-07-22 诊断·已读码定位）**：`games/game-b/play-ui.ts` 的 `faceUrl(code)` **硬编码 URL** `/games/game-b/art/mahjong/<kind>.png`（`OPP_TACHIE` 同样硬编码 tachie 路径），**绕过 art 索引/台本**。虽然 `assets.ts createGameBAssets` 已 `fetch`+`registerAssetIndex` 装了索引，但 2D UI（LayoutNode Image.src / Button.skin）不读它、只吃 `faceUrl` 拼的固定 URL。→ 工坊写回台本/index 对游戏零效果（服务文件 `man-2.png` 仍 Jul 17 未动）。`npm run ledger:audit game-b` 43 行全判孤儿=此症实证（硬编码 URL 消费=不可机读消费槽·不可经管线替换）。
> **修法（REQ-ART option a·接消费槽）**：让 `faceUrl`（+ `OPP_TACHIE` 及其它硬编码 art URL）**从游戏索引按 key 解析** URL——用引擎解析口 `filledSrc(index, key)`（`src/assets/asset-index.ts`·skinKey/id→URL·已 done）+ **现硬编码 URL 作回退**（未填/离线不炸·观感零倒退）。key = `texKey(kind)`=`mahjong/tex/<kind>`（`tiles.ts` 已有）；tachie 同理登记 key 后走索引。**需改动**：`createGameBAssets` 现 `registerAssetIndex` 后丢弃 parsed index——**留住 parsed `AssetIndex` 并下传到 play-ui**（`filledSrc` 吃的是 parsed index 对象，非 AssetManager）。改动集中在 1-2 个解析函数 + 一处索引下传，非 43 行逐改。
