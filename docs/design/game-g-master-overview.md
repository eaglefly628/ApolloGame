...
### 3.0 战场长什么样（一图看懂）
![标注·回合制战场：①三路各9格 ②召唤源泉 ③四选一动作 ④结束回合推进 ⑤Boss地煞明牌](overview-shots/20-battle-annotated.png)
...
### 3.1 赛前（出征 → 看牌）
- 选关**出征** → 播**关卡开场剧情**（该名将的成名战役背景 + Boss 开场台词·可跳过）。
- **明牌看 Boss 的 3 张地煞**（例：列奥尼达「温泉关死守 = 大本营 4 血 + 隘口 +15% 胜率」「斯巴达方阵 = 相邻兵互加战力」「死战不退 = 主将濒死不亡」）。**明牌可破** → 据此心里有数 / counter-pick 天罡。

### 3.2 开局（init）
- 三路 ×9 空格；双方大本营满血；**你起手摸若干张**（手牌混编：扑克兵 + 天罡法术）；**你先手**，回合开始 **+1 召唤源泉**。

### 3.3 你的回合（每回合 +1 源泉 · 四选一互斥）
![局部·四选一动作（抽牌/放牌/打天罡/弃牌）+ 手牌](overview-shots/21-actions.png)
![局部·召唤源泉条](overview-shots/22-source.png)
...
![我的牌组（构筑屏）](overview-shots/02-decks.png)
![局部·牌组槽位（扑克 + 天罡）](overview-shots/30-deck-crop.png)

- **出战牌组 = 自选 16 张扑克英雄 + 5 张天罡**（从 52 收藏池挑·养/附魔最好的人带上去）。
- **一键自动构筑**：帮你配一副均衡的（费用曲线铺开·不全大点·尽量凑连携）。
- **16 槽很挤**，要平衡：**费用曲线**（够便宜牌开局铺场）+ **连携**（同点/同花）+ **养成**（镶地支/附魔的核心牌）。
...
- **设计已定 / 甲乙待实装**：16+5 牌组构筑 + 一键自动构筑 + 放牌 4 档费用 + 基础连携（同点/同花）+ 关 6-52 地煞数据形（141 张全产出）+ 新手引导 coachmark 接入（接入清单见 `DEV-CHECKLIST-*.md`）。
