...
- **PE-T run2 S2 计划门确认（2026-07-17）**：重走形态评审，结论不变=编译期薄壳（cart=library/ 不入库无法接力·builtin manifest 仍无 LayoutNode UI/多关表达——援引 §4 例外行与 owner「可以有 TS 但要克制」拍板）；**较 run1 更薄**：宿主 DOM 骨架/缩放/卸载全走引擎 `mountHost` helper、指针/拖拽合成全走 `PointerInputSource`（REQ-AUDIT-守门 C 下沉件·audit 零红旗零基线条目）；输入面新增消费已验收 `t2-match3-drag-swap`（§2 拖拽行转 ✅）；皮肤槽三行接入就位（§4.5·台账脚本随 S6 建）。领工声明/边界=game-t requests T-101。
