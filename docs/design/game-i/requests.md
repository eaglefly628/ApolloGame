...
>
> **★ 补充（P3D 2026-07-16·owner「刚写的那些覆盖内容也放进展览台」）**：上批之后 P3D 又下沉了一组新原语，同样请各摆一个可见 sample（都是纯数据 component·挂带 `Transform3D` 的实体即可·已在 game-z 三台实装可参照 `games/game-z/diorama.ts` platformTwo/platformThree）：
