...
拆解：① 调研 Gossip Harbor 核心玩法（合并/体力/订单/剧情/装修）；② 以本项目数据驱动口径复刻**核心 gameplay**（不做整包商业化）；③ 产出**给 claude designer 的逐屏设计规格**（`ui-brief.md`），供其出 `.dc.html` 设计稿。详见 `brief.md`。
...
| [`ui-brief.md`](./ui-brief.md) | **给 claude designer 出 `.dc.html` 设计稿的逐屏规格**（LayoutNode 控件映射 + 信号） | 🟢 v1 |
| [`requests.md`](./requests.md) | 游戏级工单（不占引擎池槽；能力缺口→引擎 requests.md 提 LEAD） | 🟢 |
| `mockups/`（待 designer 交付） | claude designer 产出的 `.dc.html` + `support.js`（1:1 复刻基准） | ⬜ 待接收 |
...
- 设计稿：`ui-brief.md`（GD 逐屏规格）→ 待 claude designer 交付 `mockups/*.dc.html`（**1:1 复刻基准·界面必须 LayoutNode 重做，禁直接挪用交付 HTML**）
...
- **M3 视觉 1:1**：接 claude designer 的 `.dc.html` 设计稿（结构 1:1→视觉 1:1）
...
- 实现由 PE-101 领工（capability-plan 过审后）；UI 设计稿由 claude designer 出 `.dc.html`，GD-101 录入本目录 `mockups/`。
...
视觉属后续阶段：M3 以 docs/design/game101/mockups/*.dc.html 为 1:1 复刻基准，
但界面必须用 LayoutNode 控件闭集重做，禁止直接挪用该 HTML/CSS/JS。
