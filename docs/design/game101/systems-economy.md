...
## 4.5 订单续单循环（meta 三层·owner 2026-07-25「顾客满足后就没事了 = 循环在哪」追加）— 全套接受
...
- **游戏数据**：`customers.json`（顾客花名册·脸/名/订单 pool）+ 槽位轮换规则。

**构建顺序（PE 建议）**：① 引擎缺口先报 Lead 下沉（阻塞项）→ 期间 PE 出 ②③ 的游戏数据 + §此设计 → ① 落地后一次接线三层。②（进度）不依赖①可先独立做出正反馈。**红线**：三层都禁游戏层写换单/轮换/解锁 solver——①走引擎、②③走 flag+event-when 数据路。
