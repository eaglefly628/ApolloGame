...
- **Lead 裁决：✅ 过审（v1·2026-07-23·核过 registry + craft-recipe/merge-rule/drag-place 源码）——零预下沉·PE-101 可开工 M1（REQ-101-04）**
  ...
  - **§2.5 G2 订单交付 order-fulfill → 先试组合·撞墙即下沉**（不预下沉）。**`craft-recipe` 确认不适配**（核源码：costs/gains 只吞/产**资源计数**·不吞棋盘**实体实例**）。试组合=`drop-zone`(drag-place)+`event-when`(投放物模板==订单 needItem)+`k2-destroy`(销毁该实例)+`effect-apply`(发奖)。**风险**：event-when 能否引用「被投放的那个具体实例」并干净销毁待 M1 实证；**表达不了→下沉薄 `order-fulfill`**（投放匹配→消耗该实例+发奖·通用于合并/交付游戏）。
    - **✅ 实证结果（2026-07-24·M1 落地）**：**撞墙**——组合无法干净引用「被投放的那个具体实例」并原子「匹配未满槽→销毁该实例→集齐发奖」（event-when 条件读 flag/resource·不读「被拖实例的模板 vs 订单某槽」；effect-apply destroy 只吃固定 entity id）。**按 Lead 预祝福下沉薄 `t2-order-fulfill`**（`src/skills/tier2/order-fulfill.ts`·组件 `Order{needItems,filled,reward}`+`DeliverDrop{item,order}`·8 测过）。**镜像 merge-on-place 模式**：宿主拖拽合成 DeliverDrop 意图·引擎裁决（match→DestroyRequest+置槽+集齐发奖钳限+可重置）。**多槽（最多 3 slot·owner 基准需求）天然内建**（needItems 数组）。game101 只经 orders.json 数据消费·游戏层零交付逻辑。
