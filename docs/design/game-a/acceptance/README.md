...
| `06-resist-tribute-exempt` | 0 | ⑥ **抗贡 G3**：应贡方持双大王 → 免贡（tribute_count=0）·头游先出（§2.3 G3） |
| `07-double-down-tribute` | 8 | ⑦ **双下 G2**：对方一二 → 双进贡·大者(大王)归头游·进大贡者先出（§2.3 G2） |
| `08-run-won-we-pass-ace` | 0 | ⑧ run **终局**：我方过 A → 通关（run-won·A2·与 ④ 成镜像） |
...
- **`tribute1_*` 投影缺口**：⑦ 双下的「次者(小王)归二游 partner」半句需 tribute1_from/to/card 机读投影才能钉死；
  现薄适配只投影 tribute0_*。已在 `docs/design/game-a/requests.md` 报 PE 加 tribute1_* 投影（纯读·零规则）；
  落地后 ⑦ 加断言 `tribute1_to=partner`、`tribute1_card=15`。当前 ⑦ 已钉「双进贡·大者归头游·大贡者先出」。
