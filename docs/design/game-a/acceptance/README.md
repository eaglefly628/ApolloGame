...
| 文件 | seed | 查什么（gdd 依据） |
|---|---|---|
| `01-round-settle-tribute` | 47 | ① 一整盘打到**头游判定与结算** + **进贡/还贡口径**（§2.4 结算·R3 升级·§2.3 G1 单下：进最大牌/还≤10/进贡者先出） |
| `02-illegal-plays-rejected` | 1 | ② **非法出牌被拒·状态不变**：牌型不合法 / 压不过上家 / 乱序 / 无牌（§2.2 牌型闭集+压制序·R6 行牌序） |
| `03-level-advance-restart` | 3 | ③ **重开/下一局·级牌推进正确**：对方连续双上 2→5→8（R3 双上+3·盘循环重开纪律） |
...
| `06-resist-tribute-exempt` | 7 | ⑥ **抗贡 G3**：应贡方持双大王 → 免贡（tribute_count=0）·头游先出（§2.3 G3） |
...
> ⚠ **AI 出牌线耦合（PE·A-013）**：①③④⑤⑥⑦⑧ 用 `play-round`/`play-run` 全自动打，落到哪个
> 名次/进贡/过 A 分支**取决于 AI 出牌线** + 种子 PRNG 消耗步数——AI 策略一改，这些 seed 会漂离目标分支、断言值过期。
> - **一次重选（2026-07-18·「不拆炸+保留大牌」）**：01→16·06→4·07→2·08→1。
> - **二次重选（2026-07-20·「提示/AI 不拆牌型 preferNoSplit」）**：01→**47**（首盘 hero 先出·跨队一三单下·进♠A）、
>   03→**3**（对方连双上 2→5→8·断言不变）、06→**7**（双大王免贡·头游 west 先出）、08→**1** 不变仅放宽 `level_theirs`≤14
>   （对方亦达 A·未过；「未过 A」由 run_won+winner=team0 判定）。还贡 ≤10 改钉 `tribute0_return_rank`（点数·suit 无关·修 code 隐患）。
> 均**保机制重钉·非弱化**（分支真被走到）。**GD-A 长期建议**：把分支断言（抗贡/双下/过 A）迁到 walkthrough 单测
> （`forceRanking` 直接摆名次·与 AI 解耦），acceptance 只留②式「显式出牌驱动」的稳健剧本，免每次 AI 改都追种子（两次追种子已印证「脆」非偶发）。
