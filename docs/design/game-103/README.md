...
> slot 说明：owner 在 session 内称「game103」，文档暂落此目录；`src/games/` 字母位由 owner/Lead 分配后再定，文档可平移。

## 状态（8 阶段生产板）

- **S1 立项讨论**：✅ v1 → `brief.md`（含 GD 架构评判：接受立项·有前置条件）
- **S2 capability-plan**：✅ 送审稿 → `capability-plan.md`（⏳ 待 Lead 审；焦点=三选一/进化/波次编排能力归属 E1–E4；**过审前零游戏层代码**）
...
| 文档 | 内容 |
|---|---|
| [`brief.md`](./brief.md) | 立项讨论稿 + GD 架构评判 + 待拍板项 + 风险 |
| [`capability-plan.md`](./capability-plan.md) | 能力总览（门禁）：每系统对照 registry 实名消费能力 + 编排例外待裁 |
