...
### BUG-01 · 无相对位移背景·看不清在动 · [P0·PE] · open
- **现象**：移动时没参照物，玩家像没动。
...
### BUG-04 · 升级时敌人仍在 tick·未时停 · [P0·PE] · open
...
## 分工小结（给 PE 的即刻队列）
- **PE 今日可修**：BUG-04（时停·P0）→ BUG-01（世界地砖·P0）→ BUG-03（回旋镖时序·P1）。
- **等引擎（Lead）**：BUG-02/05（群体分离 + 2D 批绘）——GD 已升级 `REQ-SURVIVOR群体`；PE 侧先降同屏 cap 缓解。
