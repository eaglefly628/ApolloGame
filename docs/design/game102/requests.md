...
> **差异化·3D 旋转核心（owner 2026-07-24·不完全复刻）**：中央从静止 2D 像素图 → **3D 像素方块结构·持续旋转**（core-experience-v2 §1.5）。**⚠ 待 owner/Lead 裁：旋转=装饰（render-only·省·安全）还是玩法（sim 确定性·转动改变暴露给环轨的面·环轨×旋转双层博弈）**。GD 倾向先 A 装饰出 MVP·B 作进阶。→ P3D 出 3D voxel 核心 + 旋转（render）；若裁 B，PE/Lead 定 sim 持旋转角确定性方案。强化选「全3D盒庭」。
