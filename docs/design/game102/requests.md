...
> **差异化·3D 旋转核心（owner 2026-07-24·不完全复刻）**：中央从静止 2D 像素图 → **3D 像素方块结构·持续旋转**（core-experience-v2 §1.5）。**⚖ Lead 2026-08-18 裁 A（架构面·sim/render 定性归主程）：旋转=装饰（render-only）先行**——理由：①MVP 不引入 sim 面新确定性负担（旋转角入 hash/lockstep/存档=全套代价）②GD 同倾向 ③可升不可降的正确方向：装饰升玩法=加确定性方案（届时按缺口裁决协议摆 A/B 给 owner），玩法降装饰=拆已入 hash 的状态（伤存档）。B 作进阶留档，owner 若要「转动改变暴露面」的双层博弈随时升。→ P3D 出 3D voxel 核心 + 旋转（render）；若裁 B，PE/Lead 定 sim 持旋转角确定性方案。强化选「全3D盒庭」。
