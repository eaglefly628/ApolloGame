...
> - **三条时序疑点的组合摆法**：队首递进=`zone-occupancy` 队首 + `event-when` 到位边沿；突破态 6→10 切换=`event-when` 条件树切容量数据（数据表两档·非新逻辑）；弹尽入槽=`event-when`（`group-count`剩余=0 边沿）→ `tray` 待命槽。**先按数据编排试**。
