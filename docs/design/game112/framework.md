...
> **owner 2026-09-24 第一批判词**：「只有能抽象成引擎通用能力的才立缺口；游戏专属规则直接在游戏逻辑里实现。」据此 **①② = 游戏专属 → 游戏层确定性 TS 模块**（`capability-plan.md` §4 例外④·game-c 摊牌/边池先例），**④ = B 先行·撤单**。下面 ①②④ 原文保留作实查记录，裁决以本段为准；余 ③⑤⑥⑦⑧⑨⑩ 逐条待判。
...
| ③ | 指针跟随 + 距离/速度叶 | A | engine（主程） | —（Loop-1） | ⬜ A ⬜ B |
...
| ⑧ | 片段→序列帧转换 + video 资产桥接 | **A** | engine 资产面 / PST 脚本 | —（Loop-1） | ⬜ A ⬜ B |
| ⑨ | VideoActor | **B（延后）** | engine（主程） | — | ⬜ A ⬜ B |
| ⑩ | Video 控件事件 | A | pui | — | ⬜ A ⬜ B |
