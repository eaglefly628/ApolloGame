...
> 当前阶段：**S1 立项卡已落板 · S2 机器门绿 + 复查 PASS（S2-reviewer-agent）· S3 机器门绿 + 复查 PASS（r3）· S4 机器门绿 + 复查 PASS（r2）· 文档增量 r4 指正后做「只看它」沉浸模式（第 3 轮·等复核）**（`node scripts/game-pipeline.mjs board game112`）。owner 2026-09-24 判：11 项缺口全裁（游戏专属进游戏层 / 引擎两款能力归 Lead）。
...
> **S3/S4 碰过的文件（边界栏·复查范围核查用）**：`games/game112/**`（新建）· `public/games/game112/{pipeline.json,art/**,probe/**}`（CLI/脚本/探针写）· `docs/design/game112/{acceptance,self-check}/**`（S4 剧本与自证）· `scripts/game112-playthrough.mjs`（S4 真浏览器试玩·28 断言·17 图）· `scripts/game112-spec-recursion.mjs`（S4 递归复核·11 条款打坏验红）· `docs/design/game112/**` · `src/launcher.tsx`（+1 条 GAMES 登记）· `src/launcher/game-runner.tsx`（+1 行 loader）· `scripts/game112-art-requirements.mjs`（新建·美术台账推导）· 另 `scripts/scoped-gate.mjs`+test（台账文件归类修复·owner 授权·主程 review 欠）· `docs/workflow/requests.md`（引擎池：+REQ-UIWALK-ENTRY·REQ-UI-STAGEMODE 立后撤·只动自己条目）。
> Claude Design：任务书 `claude-design-brief.md`（v1·约束与交稿格式）+ **逐屏规格 `claude-design-spec.md`（v2·2026-09-25·S5 起点）**；出稿放 `cloud-design/`。
> **星尾馆空间框架 + 美术设定**：`hall-framework.md`（owner 2026-09-25 直令「可扩展多房间 + 地图 + 先定外框」·十间房表·馆图·热点语言·art bible·风格包 `style-pack.starlit-lived-in.json`·出图清单）。台账 6 行（art-06 = 主厅定调图槽 `hall-stage` Panel.skin）。**等 owner 判 §8 四问**。
