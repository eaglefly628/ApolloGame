...
describe('steering — 群体分离 (separation·REQ-SURVIVOR群体①)', () => {
  ...
  it('分离：两个同群 agent 追同一目标 → 垂直斥开（不塌成一点）', () => {
    const w = world();
    target(w, 'p', 100, 0); // 目标正右方
    // A 在 B 正下方（相距 2·radius 10 内）→ 分离沿 y 轴把它们推开
    ...
    w.tick();
    expect(vel(w, 'A').vy).toBeLessThan(0); // A 被上方的 B 斥向下
    expect(vel(w, 'B').vy).toBeGreaterThan(0); // B 被下方的 A 斥向上
    expect(vel(w, 'A').vx).toBeGreaterThan(0); // 仍朝目标（右）前进
  });
  ...
  it('零回归：无 separation → 纯 seek（vx=speed·vy=0·不被邻居推）', () => {
    ...
    expect(vel(w, 'A').vy).toBe(0); // 无分离=不被 B 推
  });
  ...
  it('clamp：分离叠加后速度模长不超过 speed', () => {
    ...
    w.tick();
    const v = vel(w, 'A');
    expect(Math.hypot(v.vx, v.vy)).toBeCloseTo(3, 6); // 大 weight 也被 clamp 回 speed
  });

  it('tagMask：只被带该 Tag 的邻居斥、不被无 tag 邻居斥', () => {
    // A 用 tagMask=ENEMY；近处放一个"非 ENEMY"实体 → A 不应被推。
    const wNo = world();
    target(wNo, 'p', 100, 0);
    agent(wNo, 'A', 0, 0, { mode: 'seek', speed: 3, stopRange: 5, separation: { radius: 10, weight: 2, tagMask: ENEMY } }, 'p');
    wNo.createEntity('deco');
    wNo.addComponent('deco', xf(0, 2)); // 近·但无 Tag
    wNo.tick();
    expect(vel(wNo, 'A').vy).toBe(0); // 非 ENEMY 邻居不产生斥力

    // 换成带 ENEMY tag 的邻居 → A 被推。
    const wYes = world();
    target(wYes, 'p', 100, 0);
    ...
    wYes.tick();
    expect(vel(wYes, 'A').vy).toBeLessThan(0); // 被上方 ENEMY 斥向下
  });

  it('stopRange 环绕：到攻击距离(base v=0)仍分离、不叠一点', () => {
    const w = world();
    target(w, 'p', 100, 0);
    // 两个都已在 stopRange 内（离目标 <5·base seek=0），彼此相距 2 → 分离仍推开
    ...
    w.tick();
    expect(vel(w, 'A').vy).toBeLessThan(0); // base=0 但分离把 A 推离 B
