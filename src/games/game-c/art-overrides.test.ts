...
describe('game-c 美术台账 skinKey 契约（art-replace 写回别名依据）', () => {
  ...
  it('37 行·skinKey 全带且唯一（一行一素材·fill 写回的别名依据·此前全 null=生成写不回的根因）', () => {
    expect(led.rows.length).toBe(37);
    const keys = led.rows.map((r) => r.skinKey);
    expect(keys.every((k) => typeof k === 'string' && k!.length > 0)).toBe(true);
    expect(new Set(keys).size).toBe(37);
  });

  it('28 生成槽 game-c/ 命名空间（override 载入过滤依据）+ 9 vendor 筹码 chip/ 保其索引 id', () => {
    const keys = led.rows.map((r) => r.skinKey!);
    expect(keys.filter((k) => k.startsWith('game-c/')).length).toBe(28);
