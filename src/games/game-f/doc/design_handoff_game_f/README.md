...
### 3. 单机对局界面 (`Game F Skins.dc.html`) — hifi（1280×720,内部按 1920×1080 绘制后整体 `scale(0.6667)`）
- **顶 HUD**(高 84/缩放前):阶段-回合 + 回合类型 pips + 敌阵名 + 相位横幅 + 倒计时 + 连胜。
- **左栏 羁绊**:势力/职业条目,激活高亮 + 阶梯 ticks。
...
## 待办（本包未含,下一步）
**赤壁合作对战界面(平盘版)** —— 2 真人(蜀+吴)vs AI 魏,同视角同 UI:魏上 / 蜀左 / 吴右**同一张完全平的棋盘(无长江/桥)**;队友镜像条(可折叠)、团队共享血、双 ready、联盟羁绊"火烧赤壁"半亮/全亮、赠予飞行、⚑标记。规则见 `source-briefs/gamefcoopsunliu.md` 与 `gamefcoopuibrief.md`(注:河流已按用户最新决定取消,改平盘)。
