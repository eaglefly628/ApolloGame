...
| # | 任务 | 要点 |
|---|---|---|
| **A0 ⛔⛔ TOP** | **战斗实时 → 回合制 PVE 重构**（owner 2026-06-19 **立即派发** · **盖过下方 A1-A7 + CR 实时经济**） | 按 `doc24` §七 回合循环 + §八 refactor 清单：**删** 实时连续行军插值 / 圣水时间 regen / 读秒暂停银行 / rAF 实时循环；**建** 回合制状态机(圣水/手牌/slot占位/大本营 hp) + 离散 slot 棋盘(3 路 ×9 格·我4 中1 敌4) + 动作处理(抽/放[+可选改机关]/打天罡/弃·**互斥·同类无限·每回合 +1 圣水**) + 推进+遭遇结算(调 `clash-resolve`) + AI 回合(脚本) + 判负(大本营 hp=0)；**保留照用** `clash-resolve` / 天罡 apply(`aggregateTengang`) / 地支 inlay apply / 公平骨架 / sim(喂回合 setup)。**logic 先做、不等 UI**；战斗屏 UI 待 Cloud Design 稿(doc24 §九)。配测：回合状态机确定性 hash + 出帧(开局/部署/推进/掷命/破家)。**原 A1-A7(实时味)随之退役 / 重定。** |
