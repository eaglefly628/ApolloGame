...
> 正典：`doc 17/18/19`（战斗模型）+ `doc 20`（**天罡牌**养成层 · 原"天罡牌"已 owner 拍板正式定名「**天罡牌**」）+ `UI/Game G 大厅.dc.html`（菜单设计稿）。验收：design G 逐 commit 驻验（绿 + 出帧/行为断言 + 对愿景）。
> 🏷 **命名 + 牌组结构（owner 2026-06-18 拍板 · 乙照此重设计菜单）**：① 原"天罡牌"→ **天罡牌**、原"星球牌"→ **地支牌**。② **三独立牌组（不是"融入"）**：**扑克牌组**(52·上场打) / **天罡牌组**(局内法术) / **地支牌组**(第二轴·局外升档)——**改造坊分三区管理、取消"把点数牌融入天罡"旧概念**（doc20 §〇）。**菜单/UI/文案一律显「天罡牌」「地支牌」**；代码内部 `joker`/`planet` 标识符不强求改。
> ⛔ **局内经济大改（owner 2026-06-18 · 抄皇室战争 + 两牌库「前线」式 · 详见 `doc 21`）**：牌**不是时间到了就放** → **点数(召唤源泉)回复 → 花点数摸牌·玩家选库**。**两个独立牌库**：① 普通库(52 点数牌·cycle·可囤积 ~7) ② 天罡库(法术·**上限 5·打掉才补**)。打牌：点数牌部署三路慢行军遭遇掷命 / 天罡牌施法。**砍读秒暂停**(纯实时)·快节奏(点数 regen 快·出牌快)。先破 3 血大本营。**取代**战潮/天机能量。**甲：`A1 战潮` superseded → 做 CR 两牌库点数经济**(march/clash/续航/大本营**保留**)。**乙：改造坊三区·构筑普通库 + 天罡库**(各 cost)。cost/regen 数值待真机调。
...
| # | 任务 | 要点 |
|---|---|---|
| **A0 ⛔⛔ TOP** | **战斗实时 → 回合制 PVE 重构**（owner 2026-06-19 **立即派发** · **盖过下方 A1-A7 + CR 实时经济**） | 按 `doc24` §七 回合循环 + §八 refactor 清单：**删** 实时连续行军插值 / 召唤源泉时间 regen / 读秒暂停银行 / rAF 实时循环；**建** 回合制状态机(召唤源泉/手牌/slot占位/大本营 hp) + 离散 slot 棋盘(3 路 ×9 格·我4 中1 敌4) + 动作处理(抽/放[+可选改机关]/打天罡/弃·**互斥·同类无限·每回合 +1 召唤源泉**) + 推进+遭遇结算(调 `clash-resolve`) + AI 回合(脚本) + 判负(大本营 hp=0)；**保留照用** `clash-resolve` / 天罡 apply(`aggregateTengang`) / 地支 inlay apply / 公平骨架 / sim(喂回合 setup)。**logic 先做、不等 UI**；战斗屏 UI 待 Cloud Design 稿(doc24 §九)。配测：回合状态机确定性 hash + 出帧(开局/部署/推进/掷命/破家)。**原 A1-A7(实时味)随之退役 / 重定。** |
| ~~A1 ⭐~~〔退役·实时味〕 | ~~战潮抽牌·事件脉冲~~（A0 回合制取代"实时涌牌"语义） | 实时底流/脉冲概念在回合制下作废；"该来牌时哗一把"的心流改由回合制抽牌动作承载。|
...
| **A5** | **天罡局内打出执行** | 读 `save.jokers`（契约②）→ 抽到打出触发效果。**乙做"构筑选哪些进库"，你做"局内抽到打出生效"。**|
...
| **B3 ⭐ 重设计** | **改造坊三牌组（owner 纠偏·不融入）** | **取消"把点数牌融入天罡"！** 改造坊**分三区**：**扑克牌组**(52·看/改造) · **天罡牌组**(选 ≤5 进战库 `save.jokers`·契约②) · **地支牌组**(`save.planets`·升档)。三组**各自独立、互不融合**。+ §10.8 C 流派印记（集齐解锁轻被动）。UI 显「天罡牌/地支牌」。|
| **B4** | **公平骨架** | 退役"强化全军 favor 泵点数" → 牌组/收藏数值映射新模型(buff/天罡/附魔)。**保持 `prepareArmies→ArmyCard[]{favor}` 接口不变**（甲读）。|
...
> 理由：策划 `match-flow` 无 pre-battle 阵型/备战；「分配」= **局内 CR**（召唤源泉回复 → 花点数摸牌选库 → 派上/中/下三路），不再有局外布阵/备战屏。owner 实机只想 出战→直接打。
...
- **连携检测**：检测一卡 inlays 是否凑 三合(申子辰/寅午戌/巳酉丑/亥卯未) / 六合(6 对) → apply：
  - 三合（**有界确定**）：水=整局 1 次必重掷 / 火=赢后连推 1 次+破家小额伤 / 金=每场返 1 召唤源泉+击败敌限时移除 / 木=阵亡回手牌可再派+击杀返材料。
