...
> ⛔ **局内经济大改（owner 2026-06-18 · 抄皇室战争 · 详见 `doc 21`）**：牌**不是时间到了就放**——**点数(圣水)回复 + 手牌 4 循环 + 花 cost 打牌**（点数牌部署三路 / 天罡牌当法术）+ 先破 3 血大本营。**取代**战潮抽牌/天机能量经济。**甲：`A1 战潮脉冲` superseded → 改做 CR 点数经济**（regen 点数 + 手牌4 cycle + 花 cost 打牌；march/clash/lane/续航/大本营**保留**）。**乙：改造坊构筑战斗 deck**（点数牌 + ≤5 天罡 + 各 cost）。⚠️ deck 大小(8 CR 忠实/52 整副) + cost/regen 待 owner 拍（doc21 §五）。
