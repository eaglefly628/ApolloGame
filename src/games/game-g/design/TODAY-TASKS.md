...
> 正典：`doc 17/18/19`（战斗模型）+ `doc 20`（**天机牌**养成层 · 原"小丑牌"已 owner 拍板正式定名「**天机牌**」）+ `UI/Game G 大厅.dc.html`（菜单设计稿）。验收：design G 逐 commit 驻验（绿 + 出帧/行为断言 + 对愿景）。
> 🏷 **命名（owner 拍板）**：原"小丑牌"→ **天机牌**。**菜单/UI/文案一律显「天机牌」**（乙改造坊/牌组预览照此）；代码内部 `joker`/`GAME_G_JOKERS`/`save.jokers` 标识符不强求改。
