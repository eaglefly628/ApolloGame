...
> 🏷 **命名 + 牌组结构（owner 2026-06-18 拍板 · 乙照此重设计菜单）**：① 原"小丑牌"→ **天罡牌**、原"星球牌"→ **地支牌**。② **三独立牌组（不是"融入"）**：**扑克牌组**(52·上场打) / **天罡牌组**(规则·draft 激活) / **地支牌组**(第二轴·升档)——**改造坊分三区管理、取消"把点数牌融入小丑"旧概念**（doc20 §〇）。**菜单/UI/文案一律显「天罡牌」「地支牌」**；代码内部 `joker`/`planet`/`save.jokers/planets` 标识符不强求改。
