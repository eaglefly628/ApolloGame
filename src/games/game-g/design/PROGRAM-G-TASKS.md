...
## ▶ 派单（你 idle 后 · 2026-06-15）—— 优先 U6 / U5L1（加"有趣"）

| 任务 | 内容 | 规格 |
|---|---|---|
| **U6 · AI 反制布阵 ⭐** | 意图已钉：三轴反制（布阵反制/流派 Boss 反制/干预反制）+ 难度档（低均衡·中变阵·高三轴全开）→ AI 成真·猜心对手 | **`09` §四.5**（我刚补，照接）|
| **U5 L1 · 手感 ⭐** | 溃散级联波纹 / 斩首聚焦 hitstop / 逐路揭晓 / Boss 入场台词 | **`15`**（实装可先做；真机调感后置=本仓"无真浏览器帧"既有债）|
| foil 皮肤 | 数值附魔已回驳 → 仅 **foil/holo 表现皮肤**（纯收集、不进 hash）| `12` §一 |
| T-G1 大厅 GameShell | 手写壳→`GAME_G_LOBBY_UI` UILayout（并行·质量）| `08` §六 U1 |
| 次环克制校准 / playtest 调参 | 克制网次环、`14` 平衡风险 | 真机 playtest 后 |
