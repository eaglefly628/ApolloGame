...
| T-G3 开局布阵/分兵 | ✅ **数据核+预设+AI暗布阵完成**（拖拽微调下轮）| game-g 22 测绿(总 1204)；`Formation`/4 预设(均衡/锋矢/两翼/田忌)/`armyFromFormation`(无则回退蛇形=均衡,零迁移)/`laneEstimates` 纯数据零能力；大厅出征→**布阵屏**(4 预设一键切+三路预估条 军官数/主将/Σfavor)→AI 按难度暗布阵(低=均衡/中=变化/高=克制你上局,开战揭晓)；同阵型+seed 逐拍 hash 一致。**余(下轮)**：军官卡拖拽跨路微调(drag-place)+兵自动补平的可视交互 |
