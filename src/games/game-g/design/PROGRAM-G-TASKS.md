...
### T-G6 · 培养 / 小丑牌 / 流派 ⭐（本轮新派 · `12` 全文）🟡 **slice 1 完成（cycle#9）**
- **改造坊**：融**小丑牌(10)**/**星球牌(5)**/附魔，消材料、持久存档（`craft-recipe` + 经济）。〔🟡 改造坊 lite(融小丑·持久)已落；星球/附魔下轮〕
- **小丑牌**（`12` §二）：死士/不屈/旗手/同袍/赌徒/督粮/影武者/连环/枭雄/先登——每张 = 融牌面的改规则被动。〔🟡 **本批 4 张** build 时纯 favor 变换(同袍/赌徒/先登/不屈)已落生效；士气放大族(旗手/枭雄)+结局联动族(死士/连环/督粮/影武者)下轮(需 resolve 时钩子)〕
...
- **6 流派 + 克制网**（`12` §四）：斩首/牌型/将领/铺场/概率/弃一保二，石头剪刀布闭环。〔⬜ 待小丑铺齐后接〕
...
| T-G6 培养/小丑/流派 | 🟡 **slice 1：小丑数据底座 + applyJokers + 改造坊 lite（融小丑·持久）完成**| game-g **54 测绿(总 1243)**；`GAME_G_JOKERS` 本批 4 张(同袍 suit-synergy/赌徒 polarize/先登 lane-pref/不屈 diehard)=纯 build 时 favor 变换 + `applyJokers` 解释器(`applyInterventions` 同族,零新能力,揭晓前·outcome-first)；Save 加 `jokers[]`(**局外持久·跨 run 不清零**)、大厅 **改造坊** 融小丑(花材料·一次性)、出征 showMatch 先融小丑后干预。**D0 核 Game E jokers.ts**：其=运行时计分(chips/mult)、Game G 需 build 时军阵变换 → 复用声明式哲学不复用运行时(同同花未复用 evaluateHand)。**余**：士气放大族(旗手/枭雄,需 resolve 时 morale 钩子)、结局联动族(死士/连环/督粮/影武者)、星球牌/附魔、6 流派+克制网、三选一掺流派钥匙(reply#10) |
