...
- **小丑牌**（`12` §二）：死士/不屈/旗手/同袍/赌徒/督粮/影武者/连环/枭雄/先登——每张 = 融牌面的改规则被动。〔🟡 **6/10 落地生效**：favor 变换族(同袍/赌徒/先登/不屈, cycle#9) + **士气放大族(旗手/枭雄, cycle#10=将领流)**；余结局联动族(死士/连环/督粮)+影武者下轮(需 resolve 后/能量钩子)〕
...
| T-G6 培养/小丑/流派 | 🟡 **slice 1+2：6 小丑(favor 变换族 + 士气放大族=将领流) + applyJokers + 改造坊 lite 完成**| game-g **57 测绿(总 1248)**；`GAME_G_JOKERS` 6 张：cycle#9 favor 变换族(同袍/赌徒/先登/不屈)=`applyJokers`(揭晓前改 favor)；**cycle#10 士气放大族(旗手 ×1.5 全路/枭雄 ×2 顶级主将路)**=`jokerMoraleScale`+`resolveArmy(moraleScale)` 参数化 `06` 士气(**仅放大士气不放大溃散·缩放不改掷命次数→确定性不变**)，=**将领流**核心。Save `jokers[]` 局外持久·跨 run 不清零；改造坊融小丑、showMatch 先融小丑→干预→算士气倍率。**D0**：复用 Game E 声明式哲学不复用运行时(其=运行时计分)。**余**：结局联动族(死士/连环/督粮)+影武者(斩首重定向⚠️)、星球牌/附魔、6 流派+克制网、三选一掺流派钥匙(reply#10) |
