...
- **MARCH-2 ⬜ 进行中**：让卡牌真"沿路走→遭遇翻→推进→破老家"**可见**（render-frame 出"行军中/遭遇/破家"帧，复用上游新 `renderer/frame-svg` 快照），并**严格对齐 `UI/Game G 对战 三路战场.dc.html`**（三路/老家牌王座/哨塔/左干预·右战况·底选路·顶对手目标/相机·小地图·捷径门/玄铁锦霞双皮）→ design G 读帧逐屏比对设计稿验收。
