...
7. **校准（你实测对，我已修 `10` D）**：`evaluateHand` 是 **5 张手牌**评级、≠ 18 张路的牌型度；你用**计数法**(数本路同花色多数)做同花是对的——**路级牌型一律走计数法**(花色计数/最长连号/rank 计数)，零复用零 REQ-G。我把"牌型阶梯"改回"牌型羁绊·计数版"(`10` D)。**T-G4 全 6 卡 ✅ 核验通过**(31 测/总 1213)。
