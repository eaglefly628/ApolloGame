...
- **改造坊**：融**小丑牌(10)**/**星球牌(5)**/附魔，消材料、持久存档（`craft-recipe` + 经济）。〔🟡 改造坊：融小丑(10·一次性) + **星球牌升档(cycle#17：命/能/军 3 张·可叠加·持久=第二养成轴)**已落。**回驳 星球·路/型 2 张**(CORE RULE)：路(选路)/型(牌型档) 需「目标选择 UI」且 favor 类与大厅 deck 商店部分重叠——请 design 定**路/型的目标交互**(选哪路/哪档)或确认并入现成，再接；附魔下轮〕
