...
## ▶ 当前派单（design G 2026-06-16）· VIS-3 HUD 布局（让"整屏可操作"，非锦上添花）

> 战场画面已成（VIS-1/2/4 全 ✅ 港真渲染），但玩家的**操作层**（干预卡/派牌/战况）还没按 UI 稿成形 → 游戏"能看不能玩全屏"。这是把它变成**完整可玩屏**的关键，**收回"锦上添花"定性、列为当前任务**。

**做什么**（按 `UI/三路战场.dc.html` + `16` §四）：
1. **左栏 干预卡·能量◈**：手牌干预卡(每张名+⚡cost) + 当前能量◈/上限；点卡=出干预信号。
2. **右栏 三路战况**：上/中/下三路比分 + 当前对手(名/流派/克制提示)。
3. **底部 选路派牌**：选路 + DEPLOY；战斗相位门(备战可派/揭晓锁)。
4. **顶部** 当前对手 + 目标(占老家即胜) + 命线❤/关卡 N/5。
5. **中央** 3D 战场(已成)。
6. **⭐ 扩 render-frame 出"全屏帧"**（战场 + HUD overlay 一起投影到 SVG）→ **让 HUD 布局也能被 design G 读图评**（解"DOM headless 不可验"）。

**验收**：全屏帧显示 HUD 各区对齐 UI 稿；干预/派牌/战况可读；tsc+vitest+build 全绿；DOM 壳先手写对齐、后续可迁 GameShell(T-G1)。
**之后**（非阻塞）：VIS-4 余(斩首聚焦 hitstop/Boss 入场台词) → VIS-5 美术(玄铁/锦霞双皮)。
