...
> | **持棒方 BATON** | 🔵 **design G**（program G 乙完成 B1+B2+B4+B6 → 待 design G 驻片验）|
...
> | **甲·战斗轨进度（→ 待 design G 验）** | **A1 战潮抽牌·事件脉冲 ✅**（`b24c81a`）：`live-combat` 加纯函数 `tideDrawPulse(newClashes, homeAChipped, homeBChipped)`——遭遇+1/斩将(输方主将)+1/**告急(我家−1血)+2 绝境援牌**/破阵(敌家−1血)+1，负 chip 钳 0、确定性零 rng；`showMatch` 抽牌驱动改读它（与特写同源 `clashLog.slice` + 我家/敌家血差，底流叠脉冲，手牌满 HAND_MAX 自然节流）。测：单元(各事件张数/负钳) + **行为「看得见」**(真局整局涌牌>20 远超纯底流·峰值拍≥2·静拍存在=非线性「啪嗒」·同 seed 序列确定)。vitest 1453 绿。正交守住(仅动 live-combat+showMatch 抽牌段+live-combat.test)。**下一步 A2 心流 juice**。|
> | **乙·菜单轨进度（→ 待 design G 验）** | **B1+B2+B4+B6 已落地**。待 design G 验收 / 余 B3/B5 待派。|
> | **状态** | 双轨并行：甲 A1✅（战潮脉冲）/ 乙 B1+B2+B4+B6✅（大厅忠实港+布阵牌展+公平骨架+新手指导）。待 design G 双轨验收。|
...
> | **最后更新** | 2026-06-19 · by program G 乙 · B1+B2+B4+B6 落地 → 翻棒 design G 驻片验；余 B3/B5 待派 |
