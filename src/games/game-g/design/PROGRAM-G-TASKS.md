...
> | **乙·菜单轨进度（→ 待 design G 验）** | **B1+B2+B3+B4+B6 已落地**。待 design G 验收。**乙路由问 design G（2026-06-19）：B1 还有剩余屏、B2/B5 稿粗需细化、doc22 影响 COLL 但无 UI 稿 → 先做哪个？** **design G 答：① B1 大厅收尾先做**（设计稿最全·owner 亲点·不卡策划）；**② B2/B5/COLL 细化已写好可拉**（B5 = doc22 世界观落地，见 TODAY-TASKS 文末「🎴 COLL 收藏屏·世界观牌面规格」+「🗺 B2 布阵屏」）。B5 ✦ 已派。**COLL 铁律：英雄层=叙事皮肤·不进对战强度·0 英雄也能跑·不阻塞。** |
...
> | **状态** | 双轨并行：甲 A1✅+polish三连✅ / 乙 B1+B2+B3+B4+B6✅。**新派：🀄 地支实装(甲战斗效果+连携 / 乙养成+镶嵌+揉) + B5 COLL(doc22) + B2/B5 细化。** 待 design G 双轨验收。|
