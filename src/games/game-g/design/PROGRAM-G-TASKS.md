...
| # | 任务 | 内容 | 规格 |
|---|---|---|---|
| **VIS-1 ⭐⭐** | **离线看帧（enabler，先做）** | 仿 `game-d/render-frame.ts` 加 `game-g/render-frame.ts`：跑一局到关键拍→ThreeRenderer 渲离屏 canvas→导 PNG 落 `doc/screenshots/`（布阵/开战/掷命揭晓/Boss/大厅 5 帧）| `16` §二 |
| **VIS-2 ⭐** | **三路战场 3D 场景** | 三路(中直/上下弧)+左右老家牌王座(♔)+哨塔+牌沿路列阵+相机框路；补"有牌没战场" | `16` §三 + `UI/三路战场.dc.html` + `08` |
| **VIS-3** | HUD 布局对齐 UI 稿 | 左干预/右战况/底派牌/顶对手+目标/中战场 | `16` §四 |
| **VIS-4** | U5 L1 手感 | 逐路揭晓/溃散级联/斩首聚焦/Boss 入场 | `15`/`16` §五 |
| VIS-5 | 美术升级（占位→真资产·玄铁/锦霞双皮）| 基调 | 资产清单 |
