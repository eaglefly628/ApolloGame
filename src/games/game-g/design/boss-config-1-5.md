...
- **招牌被动**：**破釜沉舟** — 全军死战不退、霸王之勇主将无双、连胜滚雪球；**叠新被动「诅咒」**周期骚扰。
- **16 牌组**（全高点·莽一波·黑红双花霸王军）：
  `8♠ 9♠ 10♠ 10♥ J♠ J♥ Q♠ Q♥ K♠ K♥ K♦ A♠ A♥ A♦ Q♠ J♥`（主将=A♠ 项羽）
- **地煞 3 + 诅咒**：`burnboats`(破釜沉舟·全军+20%·绝不溃) + `overlord`(霸王之勇·主将+40%) + `winstreak`(九战九捷·每胜+4%·封顶20%) + **`curse`(诅咒·每3回合随机弹你一张兵回起点·见 REQ-G-诅咒地煞)**
- **留场P**：**0.75** · **bossFavorBias +4** · **homeHp 5**
- 注：原型无诅咒时实测 ~68%；诅咒（每3回合骚扰）补足到 ~65%。
...
| **curse**（新） | everyTurns / mode | — | **3 / toStart** | 5 |
