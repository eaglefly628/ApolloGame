...
## 一、关1 · 列奥尼达「温泉关死守」★ · 目标 ~94%（owner 2026-06-21 调·原98%偏易）
...
- **⚠ 公平模型下关1 ≈ 96%**（sim·Boss 全明牌装备·无偷源泉）——**而且这是当前 Boss AI 太菜的天花板**（镜像测试：同配置下 Boss-AI 仅 17.5% 不输 = 输 82.5%）。**真正的难度旋钮 = ① 修 Boss AI 到 ~50% 对手水平（派甲·见 `balance-philosophy-fairness.md`）② 然后用 3 张明牌地煞做公平加成。** 关1 在 AI 修好前保持 ~96% 教学易·公平。
