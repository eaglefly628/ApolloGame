...
- **⚠ 当前 sim ≈ 96%·目标 ~70%·差距 = 两个 AI 都没做好**（sim·Boss 全明牌装备·无偷源泉）。镜像测试：同配置下 Boss-AI 仅 17.5% 不输 = 输 82.5% → **当前 96% 是"Boss AI 太菜 + 玩家 AI 太蠢（贪心铺场）"双重失真的天花板，不是真实手感。**
  - **达到 ~70% 的路径（两件事都做完才能信 sim）**：
    1. **修 Boss AI**（派甲·P0·`boss-ai-spec.md`）：信息不对称让 Boss 打到 ~玩家水平（镜像 ~50%）。
    2. **建强玩家 AI**（派甲·P0·`player-ai-spec.md`）：sim 里的"玩家"现在是贪心脚本（铺场流），手感不准；要换成会田忌赛马/连携/集中破家的**强玩家 AI**，sim WR 才代表真实高手手感。
    3. 两个 AI 都到位后，**用 3 张明牌地煞 + bossFavorBias 微调**把强玩家从镜像 ~50% **往上**给到 **~70%**（关1 是教学关·略让玩家占优·但 Boss 的隘口死守 + 方阵要让玩家"有点费劲才赢"）。
  - 在两个 AI 落地前，关1 的 96% 仅供占位·**不作为定稿手感**。
