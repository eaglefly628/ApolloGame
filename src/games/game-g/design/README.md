...
- **愿景升级（2026-06-14，owner 口述童年原型）**：Game G 的灵魂 = 童年"扑克翻转大战"——抛牌**正面活/反面死**、用手法改概率、把牌按**三三制**编成**三路军**列阵对砍。掷命内核已在（`01`），本次新增**战场结构层**（`06`：编制/军衔=点数/三路/布阵/将领牵动）+ **培养与功能牌层**（`07`：天罡/地支融牌面 + 功能牌改战场）。**已拍板**：54/方（52+2王）· 三路×18 · 军衔=点数 · **首发 vs AI**。
...
| 07 | `07-cultivation-and-cards.md` | 培养牌组：天罡/地支融牌面 + 功能牌目录（进攻/埋伏/特殊出现）|
...
| 12 | `12-cultivation-jokers-archetypes.md` | 培养：天罡牌(10)/地支牌(5)/附魔 + 6 流派 + 克制网（长期重玩层）|
...
| 20 | `20-joker-catalog-150.md` | ⭐ 天罡牌全集（致命翻牌养成层）：10 维度数据 kind + 牌力评估 + 一期 20 张精选 + 储备池逐期加厚（owner：不堆 150、慢慢加厚、每张牌力）|
...
**已实现**：实时三路行军战斗 + 大厅 5 屏 + 军队/布阵/天罡/地支/流派/Boss/干预全量数据，vitest ~1454 全绿。  
**正在做**：B5（lobby 待派）·3D-READ 胜率可读·3D-JOKER 局内打出·3D-SIM 蒙卡仿真台·AI 反制布阵。  
**待定**：Phase-4 多人（outcome-first 已扫清浮点债）。路线见 `05`。
