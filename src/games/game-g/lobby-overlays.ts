...
export function helpBox(helpTab: 'intro' | 'tut' | 'manual', tier: 'easy' | 'mid' | 'hard'): string {
  ...
  const tutBody = `<h3>📖 新手指导 · 一局怎么打</h3>
    ...
    <div class="step"><b>开局</b>：三路 9 格、两端大本营各 3 血。每回合 <b>+1 召唤源泉</b>，四选一（抽/放/打天罡·三选一互斥；弃牌不互斥·弃后可追加一个动作）。</div>
  ...
  const easy = `<h3 style="color:#4ade80">🟢 初级 · 打赢第一场</h3>
    ...
    <p><b>回合制·每回合做一件事</b>：回合开始 <b>+1 召唤源泉</b>，然后<b>选一个动作</b>：<br>· <b>抽牌</b>· <b>放牌</b>（部署一兵到某路·可顺手开关机关门）· <b>打天罡</b>（三选一·互斥）<br>· <b>弃牌</b>（<b>不互斥</b>·弃后还能再做一个抽/放/打天罡）。<br>做完 → 棋盘走一格，两军碰头 → <b>掷命对决</b>。</p>
