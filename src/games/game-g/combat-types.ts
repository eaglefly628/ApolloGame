...
export interface ClashCard { id?: string; rank: string; suit: string; general: boolean; points: number; buff: number; morale: number; tengang: number; pEff: number; tgBreak?: [string, number][]; nearDef?: number; dishaEdge?: number; phalanx?: number; wins?: number; rollMod?: { bonus: number; floor: number; twice: number } } // rollMod：该侧掷骰系天罡改掷(bonus 鬼手改掷+N / floor 磐石掷下界 / twice 灌铅骰多掷取高)→显式进掷骰读数(owner/GD 2026-07-04·否则掷值超 [1~P] 看着像作弊)；phalanx：地煞·斯巴达方阵→每兵按自身相邻友兵数 +确定战力（owner 2026-07-03·改逻辑为真·每兵加战力·进拆解可见）；id：该兵 unitId（owner 2026-06-29·离场动画按 id 定位被撕/光荣离场的兵）；tgBreak：天罡逐张贡献 [天罡id, 加成]（owner 2026-06-21·对决明细溯源）；nearDef：地煞·隘口守军固守 +战力；dishaEdge：地煞·招牌气势折成的确定战力（owner 2026-07-01 确定制·仅 Boss 侧）；wins：连胜场数（owner 2026-07-01·每胜战力对折 0.5^wins·明细里显对折削减）
