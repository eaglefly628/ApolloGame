...
export interface HudState {
  ...
  muted: boolean;
  overlay: { kind: OverlayKind; amount: number; free: number } | null;
}
...
export function buildTopBar(s: HudState): LayoutNode {
  ...
  return {
    type: 'Panel', id: 'k-topbar', props: { glass: true },
    ...
    children: [
      ...
      { type: 'Panel', id: 'k-stats', props: { bare: true }, layout: { direction: 'row', align: 'center', gap: 22 }, children: stats },
      {
        ...
        children: [
          { type: 'Button', id: 'k-mute', props: { label: s.muted ? '🔇' : '🔊', kind: 'ghost', action: 'toggle_mute' } },
        ],
      },
    ],
  };
...
export function buildBottomBar(s: HudState): LayoutNode {
  ...
  const betCtl: LayoutNode = {
    ...
    children: [
      { type: 'Button', id: 'k-betdown', props: { label: '−', kind: 'primary', disabled: s.spinning || free || s.bet <= BET_MIN, action: 'betdown' } },
      {
      ...
      { type: 'Button', id: 'k-betup', props: { label: '+', kind: 'primary', disabled: s.spinning || free || s.bet >= BET_MAX, action: 'betup' } },
    ],
  };
  const spinBtn: LayoutNode = {
    type: 'Button', id: 'k-spin',
    props: { label: s.spinning ? '···' : free ? `FREE SPIN (${s.free})` : '↻ SPIN', kind: 'hero', disabled: !canSpin, action: 'spin' },
    layout: canSpin && !s.spinning ? { fx: [{ kind: 'glow', color: free ? 'warn' : 'jade' }] } : undefined,
  };
  return {
    type: 'Panel', id: 'k-bottombar', props: { glass: true },
