...
export interface FlowAgent extends Component {
  ...
  /**
   * ⚠ **软分离只在同一张场内生效**（按 `fieldId` 找邻居）。两队各跟一张场时**互相不推**——
   * 这是 M1 的既有语义，ORCA 落地时曾被顺手改成"按网格几何"，因为改的是既有世界的轨迹
   * （lockstep / 存档回放会分叉）而被独立复查打回，现已复原。要跨阵营互推 → 用 `orca`
   * （它按几何找邻居，跨场可见），或另提工单把这条做成显式开关。
   */
  /**
   * **ORCA 硬避让**（owner 2026-08-24 拍板「可以上」·移植自 RVO2·Apache-2.0）。
   * 与 `separation` 二选一（同时填 → ORCA 优先，另一个被忽略并留痕）：
   * · `separation` = **软**承诺：允许瞬时重叠，靠力弹开，便宜、观感"涌流"。
   * · `orca`       = **强**承诺：在 `timeHorizon` 拍内保证互不碰撞，每单位解一个二维线性规划。
   * 两者都**不改走位**——流场方向仍是期望速度，ORCA 只求"离期望最近且不会撞"的那个速度。
   */
  orca?: {
    /**
     * ⚠ **多个单位走向同一个目标点时必须配 `arriveRange`**：不给的话它们会一直往那个点里挤，
     * 线性规划直接无可行解，原码只能落到「尽量少撞」的兜底 ⇒ 真的会压进去（实测 5 个单位
     * 挤一个点，最近两心距 0.198 而半径和 0.70）。**这不是避让算法的锅**——一个点容不下五个人。
     */
    radius: number;          // 本单位的碰撞半径（世界单位）
    timeHorizon?: number;    // 前瞻多少拍内保证不撞（**缺省 8**·越大越早让、越礼貌）
    maxNeighbors?: number;   // 最多考虑几个邻居（缺省 8·按距离取最近的·封顶=最坏情况可算）
  };
  /**
   * ORCA 的两条**降级**（都会在 DebugTrace 里留痕·别指望它无声地全能）：
   * ① 邻居是纯流场/软分离单位（不还礼）→ 我独自让满，但对方**没有半径可言**（按 0 计），
   *    强承诺因此只到"我自己那个半径"（实测 1v1 迎面 0.603，而半径和 0.60）；
   * ② `radius` 填 0/负/NaN → 当作没开 ORCA（塌掉的约束比没有约束更难查）。
   */
}
