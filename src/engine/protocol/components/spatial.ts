...
export interface FlowAgent extends Component {
   ...
   * 实测（5v5 迎面对穿·半径和 0.70·整队起始位置扫一族）：纯流场 0.047~0.100（直接对穿）·
   * 软分离 0.061~0.224 · **ORCA 0.631~0.701**。即：把穿模从 ~90% 压到**最坏 10%**，不是 0，
   * 且最坏点在**中场对撞**（不是终点拥挤）。要"绝对不许穿模"→ 还得叠 `collision-resolve`。
   *
