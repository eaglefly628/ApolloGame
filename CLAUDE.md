...
- **例外·UI 基座 + 展示台 game-i 归 PUI（owner 2026-07-16 设立专职·草案）**：UI 库渲染线（`src/ui/**`——LayoutNode 控件闭集 + render/server 渲染器 + catalog 自描述 + 校验器 + 主题）+ `src/games/game-i/**`（展示台）+ `tools/ui-audit.mjs`+`tools/audits/**` + UI 手册（ui-playbook/ui.md/casual-toolkit.md）由专设的「UI 基座 + 展示台程序员（PUI）」主管（**P3D 的镜像**）。**别的 session 勿擅改 `src/ui/**`/game-i**——那是 PUI 的地盘；缺控件走 requests.md 报 PUI。完整三档边界契约见 `docs/roles/PUI.md §1`。（P3D 卡里 `src/ui/** 🔒 主程` 那行归属相应改读 PUI。）
- **UI 铁律（owner 2026-06-25 拍板）**：所有游戏 UI/HUD/菜单/面板必须用 `ui/components` 的 **`LayoutNode` 纯数据**（控件=闭集；**写世界=action 信号入队，handler 绝不塞自由逻辑/CSS·DOM**）；play-field 走 render 组件+引擎渲染器。**禁**：手写 React 屏/自由 DOM；直用 `ui/shell`/`ui/vn`（待退役·VN 零消费可随时退）。表达不了→requests.md 扩控件，绝不手写逃生。**Claude Design 稿铁律（owner 2026-07-17 拍板·三 PE 同坑复盘）：凡该屏有 .dc.html 设计稿在档，稿=1:1 复刻基准——开工前必须真渲染目击（领工声明附截图·review-gates 铁律），视觉规格（字体/渐变/纹样/配色/标题）全消费，「主题件现成观感」不得压过稿；表达不了的差异逐条列清单报 PUI 裁决，绝不悄悄降格成素坯。****做 UI 前必读 `docs/design/ui-playbook.md` + `docs/playbooks/ui.md`。** 现状：game-f 冻结勿删勿迁（owner 2026-06-25）；game-e 的 1163 行 React 屏=反面教材勿模仿（详见 engine-llm-readiness-review §3.3）。
- **推送门禁铁律**（分支归属见文件顶「第一准则·分支」）：`claude/mainbranch` **直推不开 PR**；每次提交前 `fetch → rebase → push`（多 session 并行）。**tsc + vitest + build 全绿才推**。**（owner 2026-07-21 提速·智能门禁）**用 `node scripts/scoped-gate.mjs --run` 按改动面缩范围：改动全在单游戏→只跑该游戏 vitest（+tsc+build）；纯文档→只跑文档守卫；碰引擎/共享/多游戏→仍跑全量（工具自动兜底判 full）。**rebase 后对 origin 重判一次**——自己 delta 仍非引擎面就不必全量重跑（陈旧基线测的绿不算绿指的是自己碰的面，别人已门禁过的提交不重复背）。**核对门禁用退出码、别拿 `vitest | grep` 吞掉失败码（会误判绿）。**
  **全库兜底=主程定时巡检（owner 2026-07-22 拍板·不做服务端 CI）**：各 session 推送前只须跑好**自己改动面**的测试（scoped-gate 自动判范围）；**全库所有测试每天北京时间凌晨 4 点由主程 session 的定时任务自动跑一遍**——发现红=主程查域开单派修、不改代码。
- 提交署名 `Claude <noreply@anthropic.com>`。提交信息以 session URL 结尾。不在产物里写模型标识。
- 需求池 `docs/workflow/requests.md`（**owner 2026-07-15 拍板：只管引擎本身·最多 10 硬槽·满了先清后加**，`context-budget-guard` 机器卡；Lead 评审→标状态·done 同提交迁归档腾槽）；**游戏级工单随游戏走**=`docs/design/<game>/requests.md`（游戏可暂停·不占槽）；**派工通道（owner 2026-07-02 拍板）**：Lead 评审通过的实现类需求，在条目上标「**指派：Opus**」+ 附实现 spec（组件/语义/测试要求写死），由 Opus 档 session/子代理领工照图施工；**Fable 主 session 只出图纸（spec/裁决）和验收（对抗性复核 diff），不亲手施工**；无 spec 的架构判断不得下放。**3D 渲染线 + Game Z 独立池 `docs/workflow/requests-3d.md`（P3D 域）**；已完结条目在 `requests-archive.md`（查旧单先 grep 它）；各角色开工清单在 `docs/workflow/finish/`（P3D/PF/PG/PS）。
...
- **角色启动协议（owner 2026-07-03 拍板·session 正规化）**：owner 开新 session 宣告「角色=X·任务=Y」→ 第一步读 `docs/roles/index.md` 找角色卡照办（域边界/必读/工具/通道以卡为准）；未宣告角色=通用 session 按本文件通例。名录 8 正式 + 1 草案：LEAD/GD-\<game\>/PE-\<game\>/P3D/**PUI**(草案·UI 基座+展示台)/PS/PA/PST/OPS（GD、PE 每游戏一员·只写自己游戏的目录）。
