...
- **角色启动协议（owner 2026-07-03 拍板·session 正规化）**：owner 开新 session 宣告「角色=X·任务=Y」→ 第一步读 `docs/roles/index.md` 找角色卡照办（域边界/必读/工具/通道以卡为准）；未宣告角色=通用 session 按本文件通例。名录 8 角色：LEAD/GD/PE-\<game\>/P3D/PS/PA/PST/OPS。
